...
**Problem:** The project currently uses a **flat orchestration model** — the Orchestrator spawns one agent per division directly, managing all 10+ agents itself. This creates a bottleneck: no division-internal quality gates, no specialist delegation, and the 5-agent concurrency limit caps total parallelism at 5.
...
graph TD
    ...
    QL --> QS1["UnitInteg-Eng"] & QS2["E2E-Eng"] & QS3["LoadCompat-Eng"] & QS4["Regression-Eng"]
...
```
You are the {DIVISION_NAME} Division Lead for {PROJECT_NAME}.
...
2. **"Am I about to use Bash to run build/test/deploy commands?"** — If YES: STOP. Spawn a Specialist.
...
1. **Every Lead prompt** MUST end with the CRITICAL REMINDER suffix (see project config Lead-Spawning Patterns)
...
## Live Example: Bug Fix Flow (BUG-NNN)

**Scenario:** A layout bug on a page — text overflows container.
...
sequenceDiagram
    ...
    par Fix + Security in parallel
        O->>FL: Brief: Fix layout overflow (upstream: QA reproducer)
        activate FL
        FL->>FL: Spawns Component-Architect + ResponsiveA11y-Eng
        FL-->>O: Report: Fixed, lint pass, typecheck pass
        deactivate FL
    and
        O->>SL: Brief: Verify no XSS via text injection
        activate SL
        SL->>SL: Spawns AppSec-Analyst
        SL-->>O: Report: No XSS risk, security invariants clean
        deactivate SL
    end
    ...
    par QA + Docs in parallel
        ...
        activate QL
        QL->>QL: Spawns E2E-Eng + UnitInteg-Eng
        QL-->>O: Report: Inverted test GREEN, all E2E pass, {TEST_USERS} auth OK
        deactivate QL
    and
        O->>DocL: Brief: Update OPEN_ISSUES.md + add regression guard
        activate DocL
        DocL->>DocL: Spawns UserGuide-Writer
        DocL-->>O: Report: Docs updated, BUG-NNN marked fixed
        deactivate DocL
    end

    O-->>U: BUG-NNN fixed — 4 Leads, ~10 specialists, 3 waves
```
