...
| **storage/data**      | `#ffccbc` | `#d84315` | Database, object storage, message bus, cache |
| **infra**             | `#f3e5f5` | `#6a1b9a` | Auth provider, tracing, monitoring           |
...
- **IDs**: UPPERCASE short names — `GATEWAY`, `DB`, `AUTH`, `QUEUE`
...
- Group by **architectural layer** (Client, Gateway, Services, Data, Infra)
...
| Labeled dotted | `-.label.->` | Token validation, traces, monitoring |
...
- **Always use aliases** for long names: `participant Gateway as API Gateway`
...
- Do NOT use generic node names like `A`, `B`, `C` for architecture — use `GATEWAY`, `DB`
