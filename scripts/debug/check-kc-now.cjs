...
async function main() {
  ...
  if (!adminTok) {
    console.log('FAIL admin token:', r1.b.slice(0, 300));
    return;
  }
  ...
  if (!Array.isArray(users)) {
    console.log('FAIL users:', r2.b.slice(0, 300));
    return;
  }
  console.log('\nKeycloak users:');
  ...
  console.log(
    idIsCorrect
      ? '✅ UUID is CORRECT (00000000-...002)'
      : '❌ UUID WRONG, expected 00000000-0000-0000-0000-000000000002, got ' +
          instructor.id
  );

  // Step 4: Get user token for instructor
  const tokenBody =
    'client_id=edusphere-web&username=instructor%40example.com&password=Instructor123%21&grant_type=password';
  const r3 = await post(
  ...
  if (!userTok) {
    console.log('FAIL user token:', r3.b.slice(0, 300));
    return;
  }
  ...
  const payload = JSON.parse(
    Buffer.from(userTok.split('.')[1], 'base64').toString()
  );
  console.log(
    'JWT sub:',
    payload.sub,
