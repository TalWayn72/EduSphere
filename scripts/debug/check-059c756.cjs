...
async function main() {
  ...
  const targetRuns = data.workflow_runs.filter(
    (r) => r.head_sha && r.head_sha.startsWith(TARGET_SHA)
  );
  ...
  console.log('');
  console.log(
    '=== All recent master runs (top 15) — SHA : workflow : status : conclusion ==='
  );
  ...
  const checksData = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/059c7568b60fb1bd81695d6f63deaa9741d39bc6/check-runs'
  );
  if (checksData.check_runs) {
    ...
    console.log(
      'Check runs API result:',
      JSON.stringify(checksData).substring(0, 300)
    );
  }
}
