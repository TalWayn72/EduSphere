...
(async () => {
  ...
  await sleep(3000);
  let allLinks = await page.$$eval('a[href]', links => links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 80) })));
  console.log('Links on /courses:', JSON.stringify(allLinks.filter(l => !l.href.includes('#')), null, 2));
  ...
  await sleep(3000);
  allLinks = await page.$$eval('a[href]', links => links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 80) })));
  const nonHash = allLinks.filter(l => !l.href.includes('#') && !l.href.endsWith('/courses/discovery') && !l.href.endsWith('/courses'));
  console.log('Non-trivial links on /discovery:', JSON.stringify(nonHash, null, 2));
  ...
  const cardContent = await page.$$eval('[class*="card" i], [class*="Card"]', cards =>
  ...
  console.log('\nAll /courses/ links:', JSON.stringify(coursePathLinks, null, 2));

  // Step 3: Try clicking a course card directly
  if (coursePathLinks.length > 0) {
    // Filter to ones that look like detail pages
    const detailLinks = coursePathLinks.filter(l =>
      !l.href.endsWith('/discovery') && !l.href.endsWith('/new') && !l.href.endsWith('/courses/')
    );
    console.log('\nDetail-ish links:', JSON.stringify(detailLinks, null, 2));

    if (detailLinks.length > 0) {
      console.log(`\nNavigating to: ${detailLinks[0].href}`);
      await page.goto(detailLinks[0].href, { waitUntil: 'domcontentloaded', timeout: 15000 });
      await sleep(3000);
      ...
      console.log('\nAll visible buttons on course page:', JSON.stringify(allBtns, null, 2));
      ...
      if (editBtn) {
        ...
        allBtnsEdit.forEach(b => {
          const isDeleteLike = /delete|מחיקה|remove|trash|הסר/i.test(b.text) || /delete/i.test(b.testId || '');
          console.log(`  ${isDeleteLike ? '🗑️' : '  '} [${b.testId || '-'}] "${b.text}"`);
        });

        // Search for any delete-related element (not just buttons)
        const deleteElements = await page.$$eval('[data-testid*="delete" i], [class*="delete" i], [aria-label*="delete" i]', els =>
          els.map(e => ({ tag: e.tagName, text: e.textContent?.trim()?.substring(0, 60), testId: e.getAttribute('data-testid'), visible: e.offsetParent !== null }))
        );
        console.log('\nDelete-related elements:', JSON.stringify(deleteElements, null, 2));
      }
    }
  }
  ...
  const studentCourseLinks = await page2.$$eval('a', links =>
    links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 80) }))
      .filter(l => l.href.includes('/courses/') && !l.href.includes('#') && !l.href.endsWith('/discovery') && !l.href.endsWith('/new'))
  );
  ...
  if (studentCourseLinks.length > 0) {
    ...
    console.log('Student action buttons:', JSON.stringify(studentBtns, null, 2));
  }
