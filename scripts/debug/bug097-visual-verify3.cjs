...
const PAGES_AFTER_LOGIN = [
  ...
  { name: 'social', path: '/social', file: 'bug097-v3-social-he.png' },
  {
    name: 'gamification',
    path: '/gamification',
    file: 'bug097-v3-gamification-he.png',
  },
...
async function checkPage(page, label, screenshotFile) {
  ...
  try {
    ...
    result.dir = await page.evaluate(
      () => document.documentElement.dir || '(empty)'
    );
    result.lang = await page.evaluate(
    ...
    if (bodySnippet.length < 20) {
      ...
      console.log(
        '    -> Body snippet: "' + bodySnippet.substring(0, 120) + '..."'
      );
    }
  } catch (err) {
...
(async () => {
  ...
  try {
    ...
    const loginBtns = await page
      .locator('button, a')
      .filter({ hasText: /sign.?in|כניסה|login|log.?in|התחברות/i })
      .all();
    ...
    if (currentUrl.includes('/realms/') || currentUrl.includes('keycloak')) {
      ...
      try {
        ...
        await page.waitForURL(
          function (url) {
            return url.toString().startsWith(BASE);
          },
          { timeout: 15000 }
        );
        await page.waitForTimeout(3000);
    ...
    for (const pg of PAGES_AFTER_LOGIN) {
      ...
      try {
        ...
        if (pageUrl.includes('/login') && pg.path !== '/login') {
          console.log(
            '  Redirected to login for ' + pg.name + ' (not authenticated)'
          );
          // Still check the login page for Hebrew
          results.push(
            await checkPage(page, pg.name + ' (login-redirect)', pg.file)
          );
          continue;
        }
        if (pageUrl.includes('/realms/')) {
          ...
          results.push({
            ...
            dir: 'N/A',
            lang: 'N/A',
            englishFound: ['REDIRECT_TO_KEYCLOAK'],
            ok: false,
          });
    ...
    try {
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, 'bug097-v3-ERROR.png'),
      });
    } catch (e) {}
  } finally {
    await browser.close();
