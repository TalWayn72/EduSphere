...
(async () => {
  ...
  page.on('request', (req) => {
    if (req.url().includes('/graphql')) {
      try {
        ...
        if (pd && pd.query && (pd.query.includes('updateConsent') || pd.query.includes('UpdateConsent'))) {
          mutationSent = true;
  ...
  if (signInVisible) {
    ...
    const devVisible = await devBtn.isVisible({ timeout: 5000 }).catch(() => false);
    if (devVisible) {
      await devBtn.click();
  ...
  const isVisible = await toggle.isVisible({ timeout: 10000 }).catch(() => false);
  ...
  const passed = mutationSent && mutationResult && mutationResult.data && mutationResult.data.updateConsent === true && !errorToast;
