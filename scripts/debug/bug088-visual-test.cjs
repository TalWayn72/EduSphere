...
(async () => {
  ...
  try {
    ...
    console.log(
      'DEV LOGIN: Could not find dev login button, trying without auth'
    );
  }
  ...
  try {
    await page.goto(
      BASE_URL + '/settings?highlight=ai-consent&returnTo=%2Fcourses%2Fnew',
      {
    ...
    const hasBackButton = await page
      ...
      .isVisible()
      .catch(() => false);
    const hasPrivacyCard = await page
      .locator('text=Privacy & AI')
      .first()
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'bug088-settings-returnto.png'),
      fullPage: true,
    });
    results.push({
      test: 'Settings+returnTo',
      backButton: hasBackButton,
      privacyCard: hasPrivacyCard,
      toggle: hasToggle,
      pass: hasBackButton && hasPrivacyCard && hasToggle,
    });
    console.log(
      'TEST 1:',
      hasBackButton && hasPrivacyCard && hasToggle ? 'PASS' : 'FAIL',
  ...
  try {
    ...
    if (visible) {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, 'bug088-toggle-clicked.png'),
        fullPage: true,
      });
