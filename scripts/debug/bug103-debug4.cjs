...
(async () => {
  ...
  await page.route('**/graphql', async (route) => {
    ...
    if (opName === 'CourseDetail' || opName === 'GetCourse' || opName === 'Course') {
      return route.fulfill({ status: 200, contentType: 'application/json',
        body: JSON.stringify({ data: { course: { __typename: 'Course', id: COURSE_ID, title: 'Course To Be Deleted', slug: 'course-to-delete', description: 'test', thumbnailUrl: null, estimatedHours: 5, isPublished: false, instructorId: 'dev-user-id', modules: [] } } })
      });
    }
    ...
    if (opName === 'Courses' || queryStr.includes('courses(')) {
      const courses = deleteWasCalled 
        ? [{ __typename: 'Course', id: 'keep1', title: 'Course That Stays', slug: 'stays', description: 'stays', isPublished: true, estimatedHours: 10, thumbnailUrl: null, instructorId: 'dev-user-id' }]
        : [
            { __typename: 'Course', id: COURSE_ID, title: 'Course To Be Deleted', slug: 'course-to-delete', description: 'test', isPublished: false, estimatedHours: 5, thumbnailUrl: null, instructorId: 'dev-user-id' },
            { __typename: 'Course', id: 'keep1', title: 'Course That Stays', slug: 'stays', description: 'stays', isPublished: true, estimatedHours: 10, thumbnailUrl: null, instructorId: 'dev-user-id' }
          ];
      return route.fulfill({ status: 200, contentType: 'application/json',
  ...
  await page.getByTestId('delete-course-confirm-input').fill('Course To Be Deleted');
  ...
  console.log('Contains "Course To Be Deleted":', mainText.includes('Course To Be Deleted'));
  console.log('Contains "Course That Stays":', mainText.includes('Course That Stays'));
  console.log('deleteWasCalled:', deleteWasCalled);
  
  await page.screenshot({ path: 'c:/Users/P0039217/.claude/projects/EduSphere/docs/screenshots/bug103-debug4.png' });
  await browser.close();
})();
