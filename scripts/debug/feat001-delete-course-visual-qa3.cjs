...
async function run() {
  ...
  const firstEditInfo = await page1.$$eval('button', els => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (link) return { linkHref: link.getAttribute('href'), btnText: btn.textContent.trim() };
  ...
  console.log('First Edit button context:', JSON.stringify(firstEditInfo, null, 2));
  ...
  if (editButtons.length > 0) {
    ...
    const navPromise = page1.waitForURL('**/edit**', { timeout: 10000 }).catch(() => null);
    ...
    if (page1.url().includes('/edit')) {
      ...
      if (deleteBtn) {
        ...
        console.log('Has destructive styling:', hasDestructive ? '✅ YES' : '⚠️  NO');
      } else {
        console.log('❌ Delete Course button NOT FOUND on edit page');
        const allBtns = await page1.$$eval('button', els =>
          els.map(e => ({ text: (e.textContent || '').trim().substring(0, 60), testid: e.getAttribute('data-testid') }))
        );
      ...
      if (deleteBtn2) {
        ...
        if (cancelBtn) {
          ...
          const dialogGone = !(await page1.$('[data-testid="delete-course-dialog"]:visible'));
  ...
  if (courseId) {
    ...
    console.log('Delete button visible to student:', studentDeleteBtn ? '❌ BUG — visible!' : '✅ NOT visible (correct)');

    const studentEditBtn = await page2.$('[data-testid="edit-course-btn"]');
    console.log('Edit button visible to student:', studentEditBtn ? '⚠️  Visible' : '✅ NOT visible');
    ...
    console.log('Redirected away from edit:', redirected ? '✅ YES (role guard works)' : '❌ NO (BUG)');

    if (!redirected) {
      const deleteBtnOnEdit = await page2.$('[data-testid="delete-course-btn"]');
      console.log('Delete button on forced edit page:', deleteBtnOnEdit ? '❌ BUG' : '✅ Not present');
    }
  } else {
    await page2.goto(`${BASE_URL}/courses`, { timeout: 15000 });
    await page2.waitForTimeout(3000);
    const studentDeleteBtn = await page2.$('[data-testid="delete-course-btn"]');
    console.log('Delete button visible to student:', studentDeleteBtn ? '❌ BUG' : '✅ NOT visible (correct)');
  }
