...
async function run() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-01-logged-in.png') });

  // ─── Step 2: Navigate to Settings ───
  console.log('[2/9] Navigating to Settings...');
  await page.goto(`${BASE}/settings`, { waitUntil: 'networkidle', timeout: 30000 });
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-02-settings.png') });
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-03-consent-toggled.png') });
  ...
  if (hasError) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-03-error-toast.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-04-courses-new.png') });
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-05-modal-open.png') });
  ...
  if (isDisabled) {
    console.error('[6/9] FAIL: Generate button is disabled! Consent may not have propagated.');
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-06-btn-disabled.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  const firstStatusText = statusCount > 0 ? await statusElements.first().textContent() : null;
  console.log(`[7/9] ProgressStatus count: ${statusCount}, text: "${firstStatusText}"`);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-07-progress.png') });
  ...
  for (let i = 0; i < 180; i++) {
    ...
    const statusStill = await statusElements.first().isVisible().catch(() => false);

    if (hasOutline || hasErr || (!statusStill && elapsed > 10)) {
      console.log(`[8/9] Done after ${elapsed}s (outline=${hasOutline}, error=${hasErr})`);
      completed = true;
      break;
    }
    ...
    if (i % 6 === 0) {
      ...
      await page.screenshot({ path: path.join(SCREENSHOT_DIR, `bug094-08-gen-${elapsed}s.png`) });
    }
  }

  if (!completed) {
    console.error('[8/9] TIMEOUT after 15 minutes');
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-08-timeout.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug094-09-result.png') });
