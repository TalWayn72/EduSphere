...
const PAGES_AFTER_LOGIN = [
  ...
  { name: 'gamification', path: '/gamification',  file: 'bug097-v2-gamification-he.png' },
...
(async () => {
  ...
  try {
    ...
    console.log('Step 1: Landing page with Hebrew locale pre-set via storageState...');
    ...
    try {
      ...
      console.log('  No sign-in button found on login page, trying direct Keycloak...');
    }

    const kcRedirect = await page.waitForURL('**/realms/**', { timeout: 10000 }).then(() => true).catch(() => false);
    ...
    for (const pg of PAGES_AFTER_LOGIN) {
      ...
      if (page.url().includes('/login') || page.url().includes('/realms/')) {
        ...
        results.push({ page: pg.name, dir: 'N/A', lang: 'N/A', englishFound: ['REDIRECT_TO_LOGIN'], ok: false });
        continue;
      }
    ...
    console.log(`${'Page'.padEnd(15)} ${'Dir'.padEnd(6)} ${'Lang'.padEnd(6)} ${'English Found'.padEnd(35)} Status`);
    console.log('-'.repeat(80));
    ...
    for (const r of results) {
      ...
      if (!r.ok) allOk = false;
      console.log(`${r.page.padEnd(15)} ${dirStr} ${langStr} ${engStr} ${status}`);
    }
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-v2-ERROR.png') }).catch(() => {});
  } finally {
    await browser.close();
  }
})();
