...
(async () => {
  ...
  if (courseHrefs.length === 0) {
    ...
    console.log(
      'All links on /courses:',
      JSON.stringify(
        allHrefs.filter((l) => l.href.includes('course')),
        null,
        2
      )
    );
  ...
  if (courseHrefs.length > 0) {
    ...
    const editBtn = await page.$(
      '[data-testid="edit-course-btn"], button:has-text("Edit Course"), a:has-text("Edit Course")'
    );
    if (editBtn) {
    ...
    const deleteBtn = await page.$(
      'button:has-text("Delete"), button:has-text("מחיקה"), button:has-text("Delete Course"), button:has-text("מחק"), [data-testid*="delete"]'
    );
    ...
    if (deleteBtn) {
      ...
      buttons.forEach((b) =>
        console.log(
          `  [${b.testId || 'no-testid'}] "${b.text}" ${b.disabled ? '(disabled)' : ''}`
        )
      );
    ...
    await sleep(2000);
    const deleteOnDetail = await page.$(
      'button:has-text("Delete"), button:has-text("מחיקה"), [data-testid*="delete"]'
    );
    if (deleteOnDetail) {
  ...
  if (studentCourseHrefs.length > 0) {
    await page2.goto(studentCourseHrefs[0], {
      waitUntil: 'domcontentloaded',
      timeout: 15000,
    });
    await sleep(2000);
    ...
    const sDeleteBtn = await page2.$(
      'button:has-text("Delete"), button:has-text("מחיקה"), [data-testid*="delete"]'
    );
    const sEditBtn = await page2.$(
      '[data-testid="edit-course-btn"], button:has-text("Edit Course"), a:has-text("Edit Course")'
    );

    console.log(
      `Student - Delete button: ${sDeleteBtn ? 'VISIBLE ⚠️' : 'hidden ✅'}`
    );
    console.log(
      `Student - Edit button: ${sEditBtn ? 'VISIBLE ⚠️' : 'hidden ✅'}`
    );
      ...
      btns
        ...
        .filter(
          (b) =>
            b.visible &&
            b.text &&
            (b.text.includes('Course') ||
              b.text.includes('Edit') ||
              b.text.includes('Delete') ||
              b.text.includes('Enroll') ||
              b.text.includes('Fork'))
        )
    );
    console.log(
      'Student course-related buttons:',
      JSON.stringify(studentButtons, null, 2)
    );
  }
