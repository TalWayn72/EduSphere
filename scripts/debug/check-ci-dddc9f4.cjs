...
async function check() {
  ...
  const myRuns = runs.workflow_runs.filter(r => r.head_sha.startsWith('dddc9f4'));
  ...
  const blockers = myRuns.filter(r => {
    if (r.conclusion === 'success' || r.conclusion === 'skipped' || r.conclusion === 'cancelled' || !r.conclusion) return false;
    return !infraOnly.some(name => r.name.includes(name));
  });
  if (blockers.length) {
    console.log('\nFAILED (non-infra):', blockers.map(r=>r.name).join(', '));
    process.exit(1);
