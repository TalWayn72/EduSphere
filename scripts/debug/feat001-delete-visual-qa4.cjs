...
(async () => {
  ...
  if (editBtn) {
    ...
    const uuidMatch = pageHtml.match(
  ...
  console.log(
    'Student course-action buttons:',
    JSON.stringify(studentBtns, null, 2)
  );

  const hasDelete = studentBtns.some(
    (b) => /delete|מחיקה/i.test(b.text) || /delete/i.test(b.testId || '')
  );
  const hasEdit = studentBtns.some(
    (b) => /edit|עריכה/i.test(b.text) || /edit/i.test(b.testId || '')
  );
