...
async function main() {
  ...
  console.log(
    '=== audit-export.yml run details for a59fbc7 (ID: 22474372412) ==='
  );
  const auditRun = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/22474372412'
  );
  if (auditRun.name) {
  ...
  const auditJobs = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/22474372412/jobs'
  );
  if (auditJobs.jobs) {
  ...
  var ciYml = await apiCall(
    '/repos/TalWayn72/EduSphere/contents/.github/workflows/ci.yml'
  );
  if (ciYml.content) {
