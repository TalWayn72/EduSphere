...
async function run() {
  ...
  if (editBtn) {
    ...
    console.log('Course-related anchors:', JSON.stringify(anchors.slice(0, 10), null, 2));
    ...
    if (currentUrl.includes('/courses/')) {
      ...
      if (courseId && courseId !== 'new' && courseId !== 'discovery') {
        ...
        await page1.goto(`${BASE_URL}/courses/${courseId}/edit`, { timeout: 15000 });
        await page1.waitForTimeout(3000);
      }
    }
  }
  ...
  if (deleteBtn) {
    ...
    const dialog = await page1.$('[data-testid="delete-course-dialog"], [role="alertdialog"], [role="dialog"]');
    ...
    if (confirmInput) {
      ...
      if (confirmBtn) {
        ...
        console.log('Confirm button disabled with wrong text:', isDisabled ? '✅ DISABLED (correct)' : '❌ ENABLED (BUG!)');
    ...
    if (cancelBtn) {
      ...
      const dialogAfterCancel = await page1.$('[data-testid="delete-course-dialog"]');
      const dialogVisible = dialogAfterCancel ? await dialogAfterCancel.isVisible() : false;
      console.log('Dialog closed after Cancel:', !dialogVisible ? '✅ CLOSED' : '❌ STILL OPEN');
    }
  } else {
  ...
  if (courseId) {
    ...
    console.log('Navigating to course detail:', `${BASE_URL}/courses/${courseId}`);
    ...
    console.log('Student sees delete button on detail page:', studentDeleteBtn ? '❌ BUG — visible!' : '✅ NOT visible (correct)');

    // Also check if student sees edit button (they shouldn't)
    const studentEditBtn = await page2.$('[data-testid="edit-course-btn"]');
    console.log('Student sees edit button:', studentEditBtn ? '⚠️  Visible' : '✅ NOT visible');
    ...
    console.log('Student redirected from edit page:', redirectedAway ? '✅ YES (correct — role guard works)' : '❌ NO (BUG — guard missing)');
    ...
    console.log('Delete button on student edit attempt:', deleteBtnOnEdit ? '❌ BUG' : '✅ Not present');
  } else {
    ...
    const studentDeleteBtn = await page2.$('[data-testid="delete-course-btn"]');
    console.log('Student sees delete button:', studentDeleteBtn ? '❌ BUG' : '✅ NOT visible (correct)');
  }
