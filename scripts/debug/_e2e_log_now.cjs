...
const fs = require('fs');
const TOKEN = fs
  .readFileSync(__dirname + '/check-e2e-result.cjs', 'utf8')
  .match(/TOKEN = '([^']+)'/)[1];
...
async function main() {
  const runs = await get(
    '/repos/TalWayn72/EduSphere/actions/runs?head_sha=' + SHA + '&per_page=15'
  );
  const ci = (runs.workflow_runs || []).find(
    (r) => r.name === 'Continuous Integration'
  );
  if (!ci) {
    console.log('CI not found');
    return;
  }

  const jobs = await get(
    '/repos/TalWayn72/EduSphere/actions/runs/' + ci.id + '/jobs?per_page=50'
  );
  ...
  if (!logResp.redirect) {
    console.log('No redirect for log:', JSON.stringify(logResp).slice(0, 200));
    return;
  }
  ...
  const summaryStart = lines.findIndex(
    (l) => l.includes('failed') && l.includes('passed')
  );
  if (summaryStart >= 0) {
    console.log('\n=== SUMMARY ===');
    lines
      .slice(Math.max(0, summaryStart - 2), summaryStart + 10)
      .forEach((l) =>
