...
(async () => {
  ...
  try {
    ...
    await page.waitForURL((url) => url.toString().includes('8080'), {
      timeout: 10000,
    });
    ...
    console.log(
      '  All hrefs:',
      allHrefs.filter((h) => h.includes('lesson') || h.includes('pipeline'))
    );
    ...
    if (!lessonId) {
      console.log(
        '  No lesson ID found via navigation, trying GraphQL query...'
      );
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug106-lesson-detail.png'),
      fullPage: true,
    });
    ...
    if (lessonId) {
      console.log(
        `[3] Pipeline page: /courses/${COURSE_ID}/lessons/${lessonId}/pipeline`
      );
      // Clear error tracking
      const errorsBefore = networkErrors.length;

      await page.goto(
        `${BASE_URL}/courses/${COURSE_ID}/lessons/${lessonId}/pipeline`,
        {
          waitUntil: 'networkidle',
          timeout: 15000,
        }
      ...
      pipelineErrors.forEach((e) =>
        console.log(`    ${e.status}: ${e.body.substring(0, 150)}`)
      );
    } else {
    ...
    const hasRawError =
      pageContent.includes('Error:') && pageContent.includes('Cannot');
    ...
    if (networkErrors.length === 0) {
      ...
      networkErrors.forEach((e) => {
        ...
        if (e.body.includes('Cannot query'))
    ...
    const screenshotPath = path.join(
      SCREENSHOT_DIR,
      'bug106-pipeline-fixed.png'
    );
    await page.screenshot({ path: screenshotPath, fullPage: true });
    console.log(`\nScreenshot saved: ${screenshotPath}`);

    const gqlErrors = networkErrors.filter(
      (e) => e.url.includes('graphql') || e.url.includes('4000')
    );
    console.log('\n========================================');
    console.log('SUMMARY');
    console.log('========================================');
    ...
    await page
      .screenshot({
        path: path.join(SCREENSHOT_DIR, 'bug106-pipeline-ERROR.png'),
        fullPage: true,
      })
      .catch(() => {});
  } finally {
