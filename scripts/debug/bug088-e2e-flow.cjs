...
(async () => {
  ...
  if (await signInBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    await page.waitForURL(/realms\/edusphere/, { timeout: 15000 }).catch(() => {});
    if (await page.locator('#username').isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await page.waitForURL(/localhost:5173/, { timeout: 20000 }).catch(() => {});
  ...
  await page.goto('http://localhost:5173/courses/new', { waitUntil: 'networkidle', timeout: 15000 });
  await page.waitForTimeout(2000);

  const launchBtn = page.locator('button, a').filter({ hasText: /Launch AI Builder|יוצר קורסים|AI Course/i }).first();
  ...
  const modalContent = await page.locator('[role="dialog"], .modal, [data-radix-dialog-content]').first();
  ...
  const warningVisible = await consentWarning.isVisible({ timeout: 5000 }).catch(() => false);
  console.log('Consent warning visible (inside modal):', warningVisible);

  // Look for any consent-related text
  const consentText = page.locator('text=/consent|הסכמ|AI features require/i').first();
  const consentTextVisible = await consentText.isVisible({ timeout: 3000 }).catch(() => false);
  console.log('Consent text visible:', consentTextVisible);

  // Look for the consent link
  const consentLink = page.locator('a[href*="settings"][href*="highlight"]').first();
  const consentLinkVisible = await consentLink.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  const generateBtn = page.locator('button').filter({ hasText: /Generate|צור קורס/i }).first();
  const genVisible = await generateBtn.isVisible({ timeout: 3000 }).catch(() => false);
  if (genVisible) {
    const genDisabled = await generateBtn.isDisabled();
    console.log('Generate button visible:', genVisible, 'disabled:', genDisabled);
  } else {
  ...
  if (consentLinkVisible) {
    ...
    await page.screenshot({ path: ssDir + '/bug088-flow-settings-from-modal.png' });
    ...
    await page.screenshot({ path: ssDir + '/bug088-flow-consent-toggled-on.png' });
    ...
    const launchBtn2 = page.locator('button, a').filter({ hasText: /Launch AI Builder|יוצר קורסים|AI Course/i }).first();
    ...
    const warningGone = !(await warningAfter.isVisible({ timeout: 3000 }).catch(() => false));
    ...
    const gen2 = page.locator('button').filter({ hasText: /Generate|צור קורס/i }).first();
    const gen2Vis = await gen2.isVisible({ timeout: 3000 }).catch(() => false);
    if (gen2Vis) {
      const gen2Disabled = await gen2.isDisabled();
      console.log('Generate button visible:', gen2Vis, 'disabled:', gen2Disabled);
    }

    await page.screenshot({ path: ssDir + '/bug088-flow-ready-to-generate.png' });
