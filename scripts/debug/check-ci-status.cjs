...
async function main() {
  ...
  const runsData = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs?branch=master&per_page=30'
  );

  if (!runsData.workflow_runs) {
    console.log(
      'ERROR fetching runs:',
      JSON.stringify(runsData).substring(0, 500)
    );
    return;
  }
  ...
  const targetRuns = allRuns.filter(function (r) {
    return r.head_sha === TARGET_SHA;
  });
  ...
  if (targetRuns.length === 0) {
    console.log(
      'No runs found for 264b869 yet. Showing most recent 15 runs across all SHAs:'
    );
  ...
  Object.keys(bySha).forEach(function (sha) {
    ...
    console.log(
      '\nSHA: ' +
        sha +
        (entry.sha_full === TARGET_SHA ? ' <-- TARGET HEAD' : '')
    );
    entry.runs.forEach(function (r) {
      ...
      console.log(
        '  [' + icon + '] ' + r.name + ' — ' + (r.conclusion || r.status)
      );
    });
  });
  ...
  const checksData = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/' +
      TARGET_SHA +
  ...
  const statusData = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/' + TARGET_SHA + '/status'
  );
  if (statusData.state !== undefined) {
    ...
    statuses.forEach(function (s) {
      console.log(
        '  [' + s.state + '] ' + s.context + ': ' + (s.description || '')
      );
    });
  } else {
    console.log(
  ...
  if (targetRuns.length > 0) {
    ...
    for (var i = 0; i < Math.min(targetRuns.length, 5); i++) {
      ...
      var jobsData = await apiCall(
        '/repos/TalWayn72/EduSphere/actions/runs/' +
          run.id +
          '/jobs?per_page=50'
      );
