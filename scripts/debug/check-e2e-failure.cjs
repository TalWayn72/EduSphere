...
async function main() {
  ...
  const runs = await apiCall(
    `/repos/TalWayn72/EduSphere/actions/runs?branch=feat/improvements-wave1&per_page=20`
  );
  const ciRun = runs.workflow_runs.find(
    (r) => r.head_sha.startsWith(sha) && r.name === 'Continuous Integration'
  );
  if (!ciRun) {
  ...
  const jobs = await apiCall(
    `/repos/TalWayn72/EduSphere/actions/runs/${ciRun.id}/jobs`
  );
  ...
  const annotations = await apiCall(
    `/repos/TalWayn72/EduSphere/check-runs/${e2eJob.check_run_url?.split('/').pop() || e2eJob.id}/annotations`
  );
