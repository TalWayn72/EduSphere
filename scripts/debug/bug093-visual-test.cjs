...
async function run() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-04-modal-open.png') });
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-05-ready.png') });
  ...
  console.log(`[6/8] ProgressStatus role="status" count: ${statusCount}, text: "${firstStatusText}"`);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-06-progress-visible.png') });
  ...
  const laterText = await statusElements.first().textContent().catch(() => null);
  console.log(`[6/8] After 6s, text: "${laterText}"`);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-06-progress-cycling.png') });
  ...
  for (let i = 0; i < 180; i++) { // 180 x 5s = 15 min max
    ...
    if (hasOutline || hasError || (!statusStill && elapsed > 10)) {
      console.log(`[7/8] Done after ${elapsed}s (outline=${hasOutline}, error=${hasError})`);
      completed = true;
      break;
    }
    ...
    if (i % 6 === 0) {
      ...
      await page.screenshot({ path: path.join(SCREENSHOT_DIR, `bug093-07-gen-${elapsed}s.png`) });
    }
  }

  if (!completed) {
    console.error('[7/8] TIMEOUT after 15 minutes');
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-07-timeout.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug093-08-result.png') });
  ...
  if (h3s.length > 0 && (modalText.includes('Module') || modalText.includes('Introduction'))) {
    ...
    const errorText = await modal.locator('.text-destructive').textContent().catch(() => '');
