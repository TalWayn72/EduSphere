...
async function run() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-01-login-page.png') });
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-01-logged-in.png') });
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-02-settings.png') });
  ...
  const toggleVisible = await aiToggle.isVisible({ timeout: 15000 }).catch(() => false);

  if (!toggleVisible) {
    console.log('[2/7] AI consent toggle not found, looking for Privacy section...');
    // Try scrolling down
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-02-scrolled.png') });
  }
  ...
  if (toggleVisible || await aiToggle.isVisible().catch(() => false)) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-02-consent-enabled.png') });
    console.log('[2/7] AI consent enabled');
  } else {
    console.log('[2/7] Cannot find toggle — proceeding anyway (consent may already be enabled via API)');
  }
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-03-courses-new.png') });
  ...
  const btnVisible = await launchBtn.isVisible({ timeout: 15000 }).catch(() => false);
  if (!btnVisible) {
    console.error('[4/7] Launch AI Builder button not found');
    const pageText = await page.textContent('body');
    console.log('Page content (500):', pageText.slice(0, 500));
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-04-no-button.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  if (modalVisible) {
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-04-modal-open.png') });
    console.log('[4/7] AI Builder modal opened');
  } else {
    console.log('[4/7] No modal — AI builder may be inline');
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-04-after-click.png') });
  }
  ...
  const generateBtn = container.locator('button').filter({
    hasText: /Generate|צור קורס|Generate Course/i
  }).first();
  await generateBtn.waitFor({ state: 'visible', timeout: 5000 });
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-05-ready.png') });
  ...
  for (let i = 0; i < 72; i++) { // 72 x 5s = 6 min max
    ...
    const outlineTitle = container.locator('h3.font-semibold, h3, [class*="title"]');
    const hasOutline = await outlineTitle.count() > 1; // Multiple titles = modules

    const errorAlert = container.locator('.text-destructive, [data-type="error"]');
    const hasError = await errorAlert.isVisible().catch(() => false);

    if (!spinnerVisible || hasOutline || hasError) {
      console.log(`[6/7] Completed after ${elapsed}s (spinner=${spinnerVisible}, outline=${hasOutline}, error=${hasError})`);
      completed = true;
      break;
    }
    ...
    if (i % 6 === 0) {
      ...
      await page.screenshot({ path: path.join(SCREENSHOT_DIR, `bug092-06-gen-${elapsed}s.png`) });
    }
  }

  if (!completed) {
    console.error('[6/7] TIMEOUT after 6 minutes');
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-06-timeout.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'bug092-07-result.png') });
  ...
  if (modalText.includes('Introduction') || modalText.includes('Mathematics') || modalText.includes('Module')) {
    console.log('STATUS: SUCCESS — Course outline visible in UI');
  } else if (modalText.includes('error') || modalText.includes('failed') || modalText.includes('שגיאה')) {
