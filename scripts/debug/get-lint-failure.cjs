    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          reject(
            new Error(
              e.message + ': ' + Buffer.concat(chunks).toString().slice(0, 200)
            )
          );
        }
      });
...
async function main() {
  const runs = await get(
    '/repos/TalWayn72/EduSphere/actions/runs?per_page=20&branch=master'
  );
  const ciRun = (runs.workflow_runs || []).find(
    (r) =>
      r.head_sha.startsWith(SHA) && r.name.includes('Continuous Integration')
  );
  if (!ciRun) {
    console.log(
      'CI run not found for',
      SHA,
      'Available:',
      (runs.workflow_runs || [])
        .map((r) => r.head_sha.slice(0, 7) + ' ' + r.name)
        .join('\n')
    );
    return;
  }
  ...
  const lintJob = (jobs.jobs || []).find((j) =>
    j.name.toLowerCase().includes('lint')
  );
  if (!lintJob) {
    console.log(
      'Lint job not found. Jobs:',
      (jobs.jobs || []).map((j) => j.name + ' ' + j.conclusion).join('\n')
    );
    return;
  }
  ...
  const failedSteps = (lintJob.steps || []).filter(
  ...
  const logsResp = await get(
    '/repos/TalWayn72/EduSphere/actions/jobs/' + lintJob.id + '/logs'
  );
  if (logsResp.message) {
  ...
  const logsUrl =
    'https://api.github.com/repos/TalWayn72/EduSphere/actions/jobs/' +
    lintJob.id +
    '/logs';
