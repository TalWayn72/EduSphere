...
async function runVisualQA() {
  ...
  try {
    ...
    await page1.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'bug103-01-logged-in.png'),
    });
    ...
    await page1.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'bug103-02-courses-list.png'),
    });
    ...
    const hasCourseLink = await courseLink
    ...
    if (hasCourseLink) {
      ...
      const courseTitle = page1
        .getByText(/Introduction|Course|Talmud/i)
        .first();
      const hasTitle = await courseTitle
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      if (hasTitle) {
        ...
        await page1.goto(`${BASE_URL}/courses/mock-course-1`, {
          waitUntil: 'load',
        });
      }
    }
    ...
    await page1.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'bug103-03-course-detail.png'),
    });
    ...
    for (const sel of deleteSelectors) {
      ...
      if (isVis) {
        console.log(
          `  [OK] Step 4: Delete element found with selector: ${sel}`
        );
        deleteFound = true;
        break;
      }
    ...
    if (!deleteFound) {
      ...
      for (const sel of deleteSelectors) {
        ...
        if (isVis) {
          console.log(
            `  [OK] Step 4: Delete element found after scroll with selector: ${sel}`
          );
          deleteFound = true;
          break;
        }
    ...
    if (deleteFound) {
      await page1.screenshot({
        path: path.join(SCREENSHOTS_DIR, 'bug103-04-delete-button-visible.png'),
      });
    } else {
      console.log(
        '  [WARN] Step 4: Delete button not found (may need SUPER_ADMIN/ORG_ADMIN role)'
      );
      await page1.screenshot({
        path: path.join(SCREENSHOTS_DIR, 'bug103-04-page-state.png'),
      });
      ...
      const hasLink2 = await courseLink2
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      if (hasLink2) {
        ...
        await page2.goto(`${BASE_URL}/courses/mock-course-1`, {
          waitUntil: 'load',
        });
      }
      ...
      for (const sel of deleteSelectors) {
        ...
        if (isVis) {
          console.log(
            `  [OK] Step 4: Delete element found as SUPER_ADMIN with: ${sel}`
          );
          deleteFound = true;
          await page2.screenshot({
            path: path.join(
              SCREENSHOTS_DIR,
              'bug103-04-delete-button-visible.png'
            ),
          });
          break;
        }
      ...
      if (!deleteFound) {
        ...
        await page2.screenshot({
          path: path.join(SCREENSHOTS_DIR, 'bug103-04-scrolled.png'),
        });
        ...
        for (const sel of deleteSelectors) {
          ...
          if (isVis) {
            console.log(
              `  [OK] Step 4: Delete element found after scroll as SUPER_ADMIN: ${sel}`
            );
            deleteFound = true;
            break;
          }
      ...
      if (!deleteFound) {
        ...
        const bodyText = await page2.evaluate(() =>
          document.body.innerText.substring(0, 500)
        );
        console.log(
          `  [DEBUG] Page content preview: ${bodyText.substring(0, 200)}`
        );
      }
      await ctx2.close();
    }
    ...
    for (const user of TEST_USERS) {
      ...
      try {
        ...
        const isAuthenticated =
          !url.includes('/login') && url.includes('localhost:5173');

        if (isAuthenticated) {
          console.log(
            `  [OK] ${user.role.padEnd(12)} (${user.email}) — authenticated`
          );
        } else {
          console.log(
            `  [FAIL] ${user.role.padEnd(12)} (${user.email}) — URL: ${url}`
          );
        }
        ...
        console.log(
          `  [FAIL] ${user.role.padEnd(12)} (${user.email}) — ${err.message.substring(0, 100)}`
        );
      } finally {
        await ctx.close();
      }
    ...
    if (consoleErrors.length === 0) {
      ...
      console.log(
        `  [INFO] ${consoleErrors.length} console error(s) detected:`
      );

      const dialogErrors = consoleErrors.filter(
        (e) => e.includes('DialogTitle') || e.includes('aria-')
      );
      const stripeErrors = consoleErrors.filter((e) =>
      ...
      if (dialogErrors.length > 0) {
        console.log(
          `  [WARN] DialogTitle a11y warnings: ${dialogErrors.length}`
        );
        dialogErrors
          .slice(0, 3)
        ...
        console.log(
          '  [OK] No DialogTitle a11y warnings (BUG-101 fix confirmed)'
        );
      }
