...
async function main() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-landing.png'),
      fullPage: false,
    });
    ...
    if (!clicked) {
      ...
      console.log(
        '   No login button found, navigating to /courses to trigger redirect...'
      );
    ...
    if (await usernameField.isVisible({ timeout: 8000 }).catch(() => false)) {
      ...
      try {
        ...
        await page.screenshot({
          path: path.join(SCREENSHOT_DIR, 'bug099-login-issue.png'),
        });
      }
    } else {
    ...
    const discEnglish =
      /(Discussions|Create Discussion|Recent Discussions)/i.test(discText);
    ...
    await page
      .screenshot({
        path: path.join(SCREENSHOT_DIR, 'bug099-ERROR.png'),
        fullPage: false,
      })
      .catch(() => {});
  } finally {
