...
async function run() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-visual-01-login-page.png') });
  ...
  if (signInBtn) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-visual-02-after-signin.png') });
  } else {
    console.log('No sign-in button found, checking for form inputs...');
    // Maybe the login page has username/password fields directly
    const hasUsernameField = await page.$('input[name="username"], input[type="email"], #username');
    if (hasUsernameField) {
  ...
  if (url.includes('8080') || url.includes('keycloak') || url.includes('realms')) {
    ...
    try {
      ...
      await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-visual-03-kc-filled.png') });
      ...
      await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-visual-04-after-kc-login.png') });
    } catch (e) {
      console.log('KC login error:', e.message);
      await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-visual-03-kc-error.png') });
    }
  } else {
    console.log('Not on Keycloak. Checking if already authenticated...');
    // Maybe the login page is a dev-mode page with a form
    const formInputs = await page.$$eval('input', els => els.map(e => ({ name: e.name, type: e.type, id: e.id, placeholder: e.placeholder })));
  ...
  const isOnApp = page.url().includes('localhost:5173') && !page.url().includes('/login');
  ...
  const pages = [
    { name: 'dashboard', path: '/dashboard', file: 'bug097-visual-dashboard-he.png' },
    { name: 'social', path: '/social', file: 'bug097-visual-social-he.png' },
    { name: 'gamification', path: '/gamification', file: 'bug097-visual-gamification-he.png' },
    { name: 'settings', path: '/settings', file: 'bug097-visual-settings-he.png' },
  ];
