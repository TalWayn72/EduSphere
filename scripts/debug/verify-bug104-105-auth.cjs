...
(async () => {
  ...
  if (
    ...
    const loginButton = page.locator(
      'button:has-text("Sign In"), button:has-text("Login"), a:has-text("Sign In"), a:has-text("Login"), button:has-text("התחבר")'
    );
  ...
  const screenshotPath =
    'C:/Users/P0039217/.claude/projects/EduSphere/docs/screenshots/bug104-105-AFTER-fix.png';
  ...
  const hasOfflineBanner = pageContent.includes(
    '\u05EA\u05D1\u05E0\u05D9\u05D5\u05EA \u05E1\u05D5\u05DB\u05DF \u05D0\u05D9\u05E0\u05DF \u05E0\u05D2\u05D9\u05E9\u05D5\u05EA'
  );
  ...
  console.log(
    'Page headings: ' +
      JSON.stringify(headings.map((h) => h.trim()).filter(Boolean))
  );

  // BUG-104: Invalid time value
  console.log('\n=== BUG-104 CHECK (Invalid time value) ===');
  const bug104Console = consoleErrors.filter((e) =>
  ...
  const bug105Console = consoleErrors.filter(
    (e) =>
      e.includes('Cannot return null') ||
      e.includes('templateType') ||
      e.includes('non-nullable')
  );
  const bug105Page = pageErrors.filter(
    (e) =>
      e.includes('Cannot return null') ||
      e.includes('templateType') ||
      e.includes('non-nullable')
  ...
  if (bug105Console.length === 0 && bug105Page.length === 0) {
    console.log(
      'BUG-105: FIXED - No "Cannot return null for non-nullable field" errors found'
    );
  } else {
    console.log('BUG-105: STILL BROKEN');
    bug105Console.forEach((e) =>
      console.log('  Console: ' + e.substring(0, 200))
    );
  ...
  const yellowBanners = await page
    .locator(
      '[class*="warning"], [class*="Warning"], [class*="alert"], [class*="Alert"], [role="alert"]'
    )
    .count();
  console.log('\n=== WARNING/ALERT BANNERS ===');
  console.log('Warning/alert elements found: ' + yellowBanners);
  if (yellowBanners > 0) {
    const bannerTexts = await page
      .locator(
        '[class*="warning"], [class*="Warning"], [class*="alert"], [class*="Alert"], [role="alert"]'
      )
      .allTextContents();
    bannerTexts.forEach((t, i) =>
      console.log('  Banner ' + i + ': ' + t.trim().substring(0, 200))
    );
  }
