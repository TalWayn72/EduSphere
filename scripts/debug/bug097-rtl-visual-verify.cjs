...
async function main() {
  ...
  try {
    ...
    await sleep(2000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-01-landing.png'), fullPage: false });
    ...
    await sleep(2000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-02-after-locale.png'), fullPage: false });

    // Check dir attribute
    const htmlDir = await page.evaluate(() => document.documentElement.dir);
    const htmlLang = await page.evaluate(() => document.documentElement.lang);
    report('Locale', htmlDir === 'rtl' ? 'PASS' : 'FAIL', `dir="${htmlDir}" lang="${htmlLang}"`);

    // Step 3: Try to log in
    console.log('--- Step 3: Login ---');
    // Look for login elements
    const loginButton = await page.$('button:has-text("Login"), button:has-text("כניסה"), a:has-text("Login"), a:has-text("כניסה"), [data-testid="login-button"]');
    if (loginButton) {
      await loginButton.click();
      await sleep(3000);
      await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-03-login-page.png'), fullPage: false });
      ...
      await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-04-after-login.png'), fullPage: false });
    ...
    const consentBtn = await page.$('button:has-text("Accept"), button:has-text("אישור"), button:has-text("Consent"), [data-testid="consent-accept"]');
    if (consentBtn) {
    ...
    report('Social', 'INFO', `Page heading text: "${socialHeading || 'not found'}"`);

    const socialDir = await page.evaluate(() => {
      const main = document.querySelector('main') || document.querySelector('[role="main"]') || document.body;
      return getComputedStyle(main).direction;
    });
    report('Social RTL', socialDir === 'rtl' ? 'PASS' : 'FAIL', `CSS direction: ${socialDir}`);

    // Check sidebar position (RTL = sidebar should be on right)
    const sidebarInfo = await page.evaluate(() => {
      const sidebar = document.querySelector('aside, nav[role="navigation"], [data-testid="sidebar"]');
    ...
    report('Dashboard RTL', dashDir === 'rtl' ? 'PASS' : 'FAIL', `CSS direction: ${dashDir}`);

    const dashHeading = await page.textContent('h1, h2, [data-testid="page-title"]').catch(() => null);
    report('Dashboard', 'INFO', `Page heading: "${dashHeading || 'not found'}"`);
    ...
    const langInfo = await page.evaluate(() => {
      ...
      const hebrewLabel = document.querySelector('[data-value="he"], [aria-label*="Hebrew"], [aria-label*="עברית"]');
    ...
    const gamHeading = await page.textContent('h1, h2, [data-testid="page-title"]').catch(() => null);
    report('Gamification', 'INFO', `Page heading: "${gamHeading || 'not found'}"`);

    // Check tab labels
    const tabInfo = await page.evaluate(() => {
      const tabs = document.querySelectorAll('[role="tab"], .tab, button[data-state]');
      if (tabs.length === 0) return 'No tabs found';
      return Array.from(tabs).map(t => t.textContent.trim()).join(' | ');
    });
    ...
    report('Gamification RTL', gamDir === 'rtl' ? 'PASS' : 'FAIL', `CSS direction: ${gamDir}`);
    ...
    const englishPatterns = ['Dashboard', 'Social Feed', 'Settings', 'Gamification', 'Courses', 'Profile', 'Logout', 'Login'];
    const bodyText = await page.evaluate(() => document.body.innerText);
    for (const pattern of englishPatterns) {
      if (bodyText.includes(pattern)) {
        report('English Text', 'WARN', `Found English text: "${pattern}" (should be Hebrew)`);
      }
    }
