...
async function main() {
  ...
  const ciJobs = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/' + ciRunId + '/jobs'
  );
  if (ciJobs.jobs) {
    ciJobs.jobs.forEach((j) => {
      const steps = (j.steps || [])
        .map(
          (s) => s.name + '(' + s.status + '/' + (s.conclusion || 'none') + ')'
        )
        .join(', ');
      console.log(
        '  Job: ' +
  ...
  const cdJobs = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/' + cdRunId + '/jobs'
  );
  if (cdJobs.jobs) {
  ...
  const checksData = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/059c7568b60fb1bd81695d6f63deaa9741d39bc6/check-runs?per_page=100'
  );
