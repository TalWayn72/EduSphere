  ...
  return new Promise((res, rej) => {
    ...
    https
      .get(opts, (r) => {
        ...
        r.on('end', () => {
          try {
            res({ status: r.statusCode, body: JSON.parse(d) });
          } catch (e) {
...
async function main() {
  ...
  var ciJobs = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/' + CI_RUN_ID + '/jobs?per_page=50'
  );
  var lintJobId = null;
  ...
  if (ciJobs.body && ciJobs.body.jobs) {
    ...
    console.log(
      'CI jobs response:',
      JSON.stringify(ciJobs.body).substring(0, 400)
    );
  }

  if (lintJobId) {
    console.log(
      '\n=== Logs for first failed CI job: ' +
        lintJobName +
        ' (id=' +
        lintJobId +
        ') ==='
    ...
    try {
      ...
      if (relevantLines.length > 0 && relevantLines.length < 200) {
        relevantLines.forEach(function (idx) {
  ...
  var cdJobs = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs/' + CD_RUN_ID + '/jobs?per_page=50'
  );
    ...
    cdJobs.body.jobs.forEach(function (job) {
      ...
      if (job.conclusion === 'failure') {
        if (job.steps) {
          job.steps.forEach(function (step) {
            console.log(
              '  Step [' + (step.conclusion || step.status) + ']: ' + step.name
            );
          });
        }
  ...
  if (firstFailedCdJobId) {
    console.log(
      '\n=== Logs for first failed CD job: ' +
        firstFailedCdJobName +
        ' (id=' +
        firstFailedCdJobId +
        ') ==='
    );
    try {
      ...
      for (var i = 0; i < cdLines.length; i++) {
        ...
        if (
          ...
          for (
            ...
            j <= Math.min(cdLines.length - 1, i + 4);
            j++
          ) {
      ...
      errLines.sort(function (a, b) {
        return a - b;
      });
      if (errLines.length > 0 && errLines.length < 300) {
        errLines.forEach(function (idx) {
          console.log(cdLines[idx]);
        });
      } else {
