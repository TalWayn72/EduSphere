...
async function login(page, email, password, label) {
  ...
  const loginLink = await page.$('a[href*="login"], a:has-text("Sign In"), a:has-text("כניסה"), a:has-text("Login")');
  if (loginLink) {
  ...
  const kcBtn = await page.$('button:has-text("Sign In with Keycloak"), button:has-text("Keycloak"), button:has-text("כניסה")');
  if (kcBtn) {
  ...
  if (page.url().includes('keycloak') || page.url().includes(':8080')) {
    ...
    // Maybe direct form
    const usernameInput = await page.$('#username, input[name="username"], input[name="email"]');
    if (usernameInput) {
...
async function logout(page, label) {
  ...
  const avatar = await page.$('[data-testid="user-menu"], [data-testid="avatar"], button:has-text("Logout"), button:has-text("Sign Out"), button:has-text("יציאה")');
  if (avatar) {
    await avatar.click();
    await sleep(500);
    const logoutItem = await page.$('button:has-text("Logout"), button:has-text("Sign Out"), a:has-text("Logout"), button:has-text("יציאה"), a:has-text("יציאה")');
    if (logoutItem) {
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-courses-list.png'), fullPage: false });

    // Find course links
    const allLinks = await page.$$eval('a[href]', links => links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 60) })));
    const courseDetailLinks = allLinks.filter(l => {
      const m = l.href.match(/\/courses\/([^/]+)$/);
      return m && m[1] !== 'new' && m[1] !== 'discovery';
    });
    console.log(`[INSTRUCTOR] Course detail links: ${courseDetailLinks.length}`);
    courseDetailLinks.slice(0, 5).forEach(l => console.log(`  - ${l.text} → ${l.href}`));
    ...
    if (courseDetailLinks.length > 0) {
      ...
      await page.goto(firstCourse.href, { waitUntil: 'domcontentloaded', timeout: 15000 });
      await sleep(2000);
      console.log(`[INSTRUCTOR] Course detail URL: ${page.url()}`);

      await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-course-detail.png'), fullPage: false });

      // Look for edit link/button
      const editLink = await page.$('a[href*="edit"], button:has-text("Edit"), button:has-text("עריכה")');
      if (editLink) {
        ...
        console.log(`[INSTRUCTOR] No edit button, trying direct URL: ${editUrl}`);
      ...
      console.log('[INSTRUCTOR] No course links found. Checking page content...');
      const bodyText = await page.$eval('body', b => b.textContent?.substring(0, 500));
      console.log('[INSTRUCTOR] Page content preview:', bodyText?.substring(0, 300));
      ...
      const links2 = await page.$$eval('a[href]', links => links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 60) })));
      const courseLinks2 = links2.filter(l => {
        const m = l.href.match(/\/courses\/([^/]+)$/);
        return m && m[1] !== 'new' && m[1] !== 'discovery';
      });
      console.log(`[INSTRUCTOR] Discovery course links: ${courseLinks2.length}`);

      if (courseLinks2.length > 0) {
        await page.goto(courseLinks2[0].href + '/edit', { waitUntil: 'domcontentloaded', timeout: 15000 });
        await sleep(2000);
        foundEditPage = true;
      }
    }
    ...
    const deleteBtn = await page.$('button:has-text("Delete"), button:has-text("מחיקה"), button:has-text("Delete Course"), button:has-text("מחק קורס"), [data-testid*="delete"]');
    ...
    if (deleteBtn) {
      ...
      await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-edit-page-with-delete.png'), fullPage: false });
      ...
      await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-delete-dialog.png'), fullPage: false });
      console.log('[INSTRUCTOR] Screenshot: delete confirmation dialog saved');

      // Check dialog content
      const dialogText = await page.$eval('[role="dialog"], [role="alertdialog"], [data-testid*="dialog"], .modal, [class*="Dialog"]',
        el => el.textContent?.substring(0, 300)).catch(() => 'no dialog element found');
      ...
      console.log('[INSTRUCTOR] Visible buttons:', JSON.stringify(buttons, null, 2));

      await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-edit-page-no-delete.png'), fullPage: false });
      console.log('[INSTRUCTOR] Screenshot: edit page saved');
    }

  } catch (err) {
    console.error('[INSTRUCTOR] Error:', err.message);
    await page.screenshot({ path: path.join(SCREENSHOTS, 'feat001-instructor-error.png'), fullPage: false }).catch(() => {});
  }
  ...
  try {
    ...
    // Navigate to courses
    await page2.goto(`${BASE}/courses`, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await sleep(3000);
    console.log(`[STUDENT] Courses URL: ${page2.url()}`);

    // Find a course
    const studentLinks = await page2.$$eval('a[href]', links => links.map(l => ({ href: l.href, text: l.textContent?.trim()?.substring(0, 60) })));
    ...
    if (studentCourseLinks.length > 0) {
      await page2.goto(studentCourseLinks[0].href, { waitUntil: 'domcontentloaded', timeout: 15000 });
      await sleep(2000);
      ...
      await sleep(2000);
      const dLinks = await page2.$$eval('a[href]', links => links.map(l => l.href));
      const dCourseLinks = dLinks.filter(l => {
        const m = l.match(/\/courses\/([^/]+)$/);
        return m && m[1] !== 'new' && m[1] !== 'discovery';
      });
      if (dCourseLinks.length > 0) {
        await page2.goto(dCourseLinks[0], { waitUntil: 'domcontentloaded', timeout: 15000 });
        await sleep(2000);
      }
    }

    // Check for delete/edit buttons (should NOT exist for student)
    const deleteBtn2 = await page2.$('button:has-text("Delete"), button:has-text("מחיקה"), button:has-text("Delete Course"), [data-testid*="delete"]');
    const editBtn2 = await page2.$('a[href*="edit"], button:has-text("Edit"), button:has-text("עריכה"), [data-testid*="edit"]');

    if (deleteBtn2) {
      console.log('[STUDENT] ⚠️ WARNING: Delete button visible to student! SECURITY ISSUE!');
    } else {
    ...
    await page2.screenshot({ path: path.join(SCREENSHOTS, 'feat001-student-no-delete.png'), fullPage: false });
    console.log('[STUDENT] Screenshot saved');

  } catch (err) {
    console.error('[STUDENT] Error:', err.message);
    await page2.screenshot({ path: path.join(SCREENSHOTS, 'feat001-student-error.png'), fullPage: false }).catch(() => {});
  }
