...
async function main() {
  ...
  try {
    ...
    let pageText = await page
      .locator('body')
      .innerText()
      .catch(() => '');
    if (!pageText.includes('Agent') && !pageText.includes('agent')) {
      console.log(
        '[BUG-104/105] Client-side nav may not have worked, trying sidebar link...'
      );
      ...
      if ((await agentLink.count()) > 0) {
        ...
        console.log(
          '[BUG-104/105] No sidebar link found, using direct goto...'
        );
    ...
    if (pageText.includes('Initializing authentication')) {
      console.log(
        '[BUG-104/105] Page still initializing, waiting 10 more seconds...'
      );
    ...
    // Take screenshot
    const agentsScreenshot = path.join(
      SCREENSHOTS_DIR,
      'bug104-105-agents-page.png'
    );
    ...
    // BUG-104 check
    const invalidTimeErrors = consoleErrors.filter((e) =>
      e.includes('Invalid time value')
    );
    console.log('\n=== BUG-104: "Invalid time value" ===');
    console.log(
      invalidTimeErrors.length === 0
        ? '  PASS'
        : `  FAIL (${invalidTimeErrors.length})`
    );
    invalidTimeErrors.forEach((e) => console.log(`    ${e.substring(0, 200)}`));

    // BUG-105 check
    const nullFieldErrors = consoleErrors.filter((e) =>
      e.includes('Cannot return null')
    );
    console.log('\n=== BUG-105: "Cannot return null" ===');
    console.log(
      nullFieldErrors.length === 0
        ? '  PASS'
        : `  FAIL (${nullFieldErrors.length})`
    );
    ...
    const bannerEn = await page
      .locator('text=Agent templates are not accessible')
      .count();
    console.log('\n=== Disconnected Banner ===');
    console.log(
      bannerHe === 0 && bannerEn === 0
        ? '  PASS: No banner'
        : '  FAIL: Banner visible'
    );
    ...
    for (const tpl of [
      ...
      console.log(
        `  ${c > 0 ? 'PASS' : 'WARN'}: "${tpl}" ${c > 0 ? `(${c})` : 'not found'}`
      );
    }
    ...
    const hasContent =
      pageText.length > 100 &&
      !pageText.includes('Initializing authentication');
    console.log('\n======================================');
    console.log('  VERIFICATION SUMMARY');
    console.log('======================================');
    console.log(
      `  BUG-104 (Invalid time value):  ${invalidTimeErrors.length === 0 ? 'FIXED' : 'STILL PRESENT'}`
    );
    console.log(
      `  BUG-105 (Cannot return null):   ${nullFieldErrors.length === 0 ? 'FIXED' : 'STILL PRESENT'}`
    );
    console.log(
      `  No disconnected banner:         ${bannerHe === 0 && bannerEn === 0 ? 'OK' : 'BANNER SHOWING'}`
    );
    console.log(
      `  No uncaught errors:             ${pageErrors.length === 0 ? 'OK' : 'ERRORS FOUND'}`
    );
    console.log(
      `  Page rendered:                  ${hasContent ? 'YES' : 'NO (auth stuck or blank)'}`
    );
    console.log('======================================\n');
  } catch (err) {
    ...
    try {
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, 'bug104-105-ERROR.png'),
        fullPage: true,
      });
    } catch {
      /* ignore */
