...
async function main() {
  ...
  try {
    ...
    report('Landing RTL', htmlDir === 'rtl' ? 'PASS' : 'FAIL', `dir="${htmlDir}" lang="${htmlLang}"`);

    // Capture landing page in Hebrew
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-01-landing-he.png'), fullPage: false });

    // Check landing page Hebrew text
    const landingText = await page.evaluate(() => document.body.innerText);
    const hasHebrew = landingText.includes('ברוכים הבאים') || landingText.includes('כניסה');
    report('Landing Hebrew', hasHebrew ? 'PASS' : 'FAIL', `Hebrew text present: ${hasHebrew}`);

    // Step 2: Login via Keycloak
    console.log('--- Step 2: Keycloak Login ---');
    // Click the Keycloak login button
    const kcButton = await page.$('button:has-text("Keycloak"), a:has-text("Keycloak"), button:has-text("כניסה עם Keycloak")');
    if (kcButton) {
      await kcButton.click();
      report('Login', 'INFO', 'Clicked Keycloak login button');
    } else {
      // Try any login button
      const anyLogin = await page.$('button:has-text("Log In"), button:has-text("Sign In"), a:has-text("Log In")');
      if (anyLogin) {
    ...
    await sleep(5000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-02-keycloak.png'), fullPage: false });

    const loginUrl = page.url();
    report('Login', 'INFO', `Login URL: ${loginUrl}`);

    // Fill Keycloak form
    if (loginUrl.includes('keycloak') || loginUrl.includes('auth') || loginUrl.includes('8080')) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-03-after-login.png'), fullPage: false });
    report('Login', 'INFO', `After login URL: ${page.url()}`);

    // Handle consent dialog if present
    await sleep(2000);
    const consentBtn = await page.$('button:has-text("Accept"), button:has-text("אישור"), button:has-text("I Agree"), [data-testid="consent-accept"]');
    if (consentBtn) {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-04-home-he.png'), fullPage: false });

    const postLoginDir = await page.evaluate(() => document.documentElement.dir);
    report('Post-Login RTL', postLoginDir === 'rtl' ? 'PASS' : 'FAIL', `dir="${postLoginDir}"`);

    const postLoginText = await page.evaluate(() => document.body.innerText.substring(0, 800));
    report('Post-Login Content', 'INFO', `First 800 chars:\n${postLoginText}`);

    // Check sidebar position
    const sidebarCheck = await page.evaluate(() => {
      const sidebar = document.querySelector('aside, nav:not([role="menubar"]), [class*="sidebar"], [class*="Sidebar"]');
      if (!sidebar) return 'no sidebar element found';
      const rect = sidebar.getBoundingClientRect();
      const windowWidth = window.innerWidth;
      const position = rect.left > windowWidth / 2 ? 'RIGHT (correct for RTL)' : 'LEFT';
    ...
    await sleep(3000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-social-he.png'), fullPage: false });
    ...
    await sleep(3000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-dashboard-he.png'), fullPage: false });
    ...
    await sleep(3000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-settings-he.png'), fullPage: false });
    ...
    const settingsInfo = await page.evaluate(() => {
      ...
      const selectInfo = Array.from(selectEls).map(s => `${s.tagName}:value="${s.value || s.textContent?.trim().substring(0, 30)}"`);
      return { hasHebrew, selects: selectInfo, textSample: allText.substring(0, 400) };
    });
    report('Settings', 'INFO', JSON.stringify(settingsInfo, null, 2));

    // Step 7: Navigate to gamification
    console.log('--- Step 7: Gamification ---');
    await page.goto(`${BASE_URL}/gamification`, { waitUntil: 'networkidle', timeout: 30000 });
    await sleep(3000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-gamification-he.png'), fullPage: false });

    const gamInfo = await page.evaluate(() => {
      const headings = Array.from(document.querySelectorAll('h1, h2, h3')).map(h => h.textContent.trim());
      const tabs = Array.from(document.querySelectorAll('[role="tab"], button[data-state]')).map(t => t.textContent.trim());
    ...
    await sleep(3000);
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-courses-he.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'bug097-rtl-error.png'), fullPage: false }).catch(() => {});
  } finally {
    await browser.close();
  }
