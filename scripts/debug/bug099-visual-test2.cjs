...
async function main() {
  ...
  try {
    ...
    const signInBtn = page
      .locator(
        'button:has-text("Sign In with Keycloak"), a:has-text("Sign In with Keycloak")'
      )
      .first();
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-keycloak-form.png'),
      fullPage: false,
    });
    ...
    if (await usernameField.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      try {
        ...
        await page.screenshot({
          path: path.join(SCREENSHOT_DIR, 'bug099-login-timeout.png'),
        });
      }
    } else {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-after-login.png'),
      fullPage: false,
    });
    ...
    const isLoggedIn =
      !bodyText.includes('Sign In with Keycloak') &&
      !bodyText.includes('Welcome to EduSphere');
    ...
    const challengeHandler = (response) => {
      if (response.status() >= 400)
        challengeErrors.push(response.status() + ' ' + response.url());
    };
    ...
    await page.waitForTimeout(2000);
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-challenges-he.png'),
      fullPage: false,
    });
    ...
    const chEnglish =
      /(Group Challenges|Create Challenge|Active Challenges)/i.test(
        challengesText
      );
    console.log(
      '   Hebrew: ' + chHebrew + ' | English UI strings: ' + chEnglish
    );
    console.log(
      '   400 errors on this page: ' +
        challengeErrors.filter((e) => e.startsWith('400')).length
    );
    console.log(
      '   RESULT: ' +
        (chHebrew && !chEnglish
          ? 'PASS'
          : chHebrew
            ? 'PARTIAL (some English)'
            : 'NOT AUTHENTICATED (showing login page)')
    );

    // 4. Navigate to peer-matching
    console.log('\n4. Navigating to /peer-matching...');
    await page.goto(BASE_URL + '/peer-matching', {
      waitUntil: 'networkidle',
      timeout: 15000,
    });
    await page.waitForTimeout(2000);
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-peer-matching-he.png'),
      fullPage: false,
    });
    ...
    await page.waitForTimeout(2000);
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, 'bug099-discussions-he.png'),
      fullPage: false,
    });
    ...
    await page
      .screenshot({ path: path.join(SCREENSHOT_DIR, 'bug099-ERROR.png') })
      .catch(() => {});
  } finally {
    await browser.close();
  }
