...
(async () => {
  ...
  await page.route('**/graphql', async (route) => {
    ...
    if (opName === 'Courses' || (parsed.query && parsed.query.includes('courses('))) {
      const response = JSON.stringify({ data: { courses: [
        { id: 'test1', title: 'MOCK COURSE XYZ', slug: 'mock', description: 'test', isPublished: true, estimatedHours: 5, thumbnailUrl: null, instructorId: 'dev-user-id', __typename: 'Course' }
      ] } });
  ...
  console.log('--- Contains MOCK COURSE XYZ:', html.includes('MOCK COURSE XYZ'));
  ...
  try {
    const result = await page.evaluate(() => {
      ...
      return document.querySelector('main')?.textContent?.substring(0, 200) || 'NO MAIN FOUND';
    });
  ...
  await page.screenshot({ path: 'c:/Users/P0039217/.claude/projects/EduSphere/docs/screenshots/bug103-debug2.png' });
