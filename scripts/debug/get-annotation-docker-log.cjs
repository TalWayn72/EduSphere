...
async function main() {
  const runs = await get(
    '/repos/TalWayn72/EduSphere/actions/runs?per_page=12&branch=master'
  );
  const cdRun = runs.workflow_runs.find(
    (r) =>
      r.head_sha.startsWith('d628f26') && r.name === 'CD — Deploy to Kubernetes'
  );

  const jobs = await get(
    `/repos/TalWayn72/EduSphere/actions/runs/${cdRun.id}/jobs`
  );
  const failedJob = jobs.jobs.find(
    (j) => j.name === 'Build & Push — subgraph-annotation'
  );

  console.log('Job ID:', failedJob.id);
  const logs = await getLogs(
    `/repos/TalWayn72/EduSphere/actions/jobs/${failedJob.id}/logs`
  );
