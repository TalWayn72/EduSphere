...
async function main() {
  ...
  const dockerRun = runs.workflow_runs.find(
    (r) => r.head_sha.startsWith(sha) && r.name === 'Docker Image Builds'
  );
  if (!dockerRun) {
    console.log('Docker run not found');
    return;
  }
  ...
  const jobs = await apiCall(
    `/repos/TalWayn72/EduSphere/actions/runs/${dockerRun.id}/jobs`
  );
  jobs.jobs.forEach((j) => {
