...
async function main() {
  ...
  const commitStatus = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/a59fbc7/status'
  );
  ...
  const checkRuns = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/a59fbc7/check-runs?per_page=50'
  );
  ...
  const suites = await apiCall(
    '/repos/TalWayn72/EduSphere/commits/a59fbc7/check-suites?per_page=20'
  );
  if (suites.check_suites) {
    ...
    suites.check_suites.forEach(function (s) {
      console.log(
        '  App:',
        s.app ? s.app.name : 'N/A',
        '| Status:',
        s.status,
        '| Conclusion:',
        s.conclusion,
        '| Head SHA:',
        s.head_sha ? s.head_sha.substring(0, 7) : 'N/A'
      );
    });
  } else {
  ...
  if (pr4.head) {
    console.log(
      'HEAD SHA:',
      pr4.head.sha ? pr4.head.sha.substring(0, 7) : 'N/A'
    );
  ...
  const allRuns = await apiCall(
    '/repos/TalWayn72/EduSphere/actions/runs?branch=feat%2Fimprovements-wave1&event=pull_request&per_page=30'
  );
