...
async function main() {
  ...
  const runs = await get(
    '/repos/TalWayn72/EduSphere/actions/runs?head_sha=' + sha + '&per_page=20'
  );
  ...
  const endTime = e2eJob.completed_at
    ? new Date(e2eJob.completed_at)
    : new Date();
  const elapsed = started
    ? Math.round((endTime.getTime() - started.getTime()) / 60000)
    : 0;
  console.log(
    'E2E:',
    e2eJob.status,
    '|',
    e2eJob.conclusion || 'running',
    '| elapsed:',
    elapsed,
    'min'
  );
