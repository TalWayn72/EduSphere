...
const DESIRED_USERS = [
  {
    ...
    username: 'super.admin@edusphere.dev',
...
async function getAdminToken() {
  ...
  if (!data.access_token)
    throw new Error('Admin token failed: ' + r.body.slice(0, 200));
...
async function main() {
  ...
  const existingCorrectIds = new Set(
    existingUsers.filter((u) => correctIds.has(u.id)).map((u) => u.id)
  );
  ...
  if (toImport.length > 0) {
    ...
    console.log(
      `  partialImport → HTTP ${result.status}: ${result.body.slice(0, 200)}`
    );
  } else {
  ...
  for (const u of DESIRED_USERS) {
    ...
    if (data.access_token) {
      ...
      console.log(
        `  ${subOk ? '✅' : '❌'} ${u.email} | sub=${payload.sub} | tenant_id=${payload.tenant_id}`
      );
    } else {
