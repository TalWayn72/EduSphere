...
function httpGet(path, token) {
  return new Promise((resolve, reject) => {
    ...
    if (token) headers['Authorization'] = 'Bearer ' + token;
    const req = http.request(
      { hostname: 'localhost', port: 8080, path, method: 'GET', headers },
      (res) => {
        let d = '';
        res.on('data', (c) => (d += c));
        res.on('end', () => resolve({ s: res.statusCode, b: d }));
...
async function main() {
  ...
  if (!tok) {
    console.error('No admin token:', r1.b.slice(0, 200));
    process.exit(1);
  }
  ...
  for (const a of assignments) {
    ...
    if (!role) {
      console.log('❌ Role not found:', a.role);
      continue;
    }
  ...
  for (const u of testUsers) {
    ...
    const payload = JSON.parse(
      Buffer.from(ldata.access_token.split('.')[1], 'base64url').toString()
    );
    const roles =
