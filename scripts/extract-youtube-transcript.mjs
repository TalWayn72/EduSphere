...
(async () => {
  ...
  try {
    const acceptBtn = page.locator(
      'button:has-text("Accept all"), button[aria-label*="Accept"], form button:first-of-type'
    );
  ...
  await page.screenshot({
    path: join(
      projectRoot,
      'docs/screenshots/youtube-transcript-extraction.png'
    ),
    fullPage: false,
  });
  ...
  try {
    ...
    if (captionMatch) {
      ...
      console.log(
        'Raw tracks (first 800 chars):',
        tracksJson.substring(0, 800)
      );

      // Try to parse track URLs
      const urlMatches = [...captionMatch[1].matchAll(/"baseUrl":"(.*?)"/g)];
      const langMatches = [
        ...captionMatch[1].matchAll(/"languageCode":"(.*?)"/g),
      ];
      ...
      if (chosenTrack) {
        console.log(
          `Using track: ${chosenTrack.lang} — ${chosenTrack.url.substring(0, 100)}...`
        );
    ...
    try {
      ...
      if (data.events) {
        ...
        if (textEvents.length > 0) {
          finalTranscript = textEvents
            .map((e) => `[${e.start}s] ${e.text}`)
            .join('\n');
      ...
      // Try XML format
      const xmlTexts = [
        ...captionContentA.matchAll(/<text[^>]*>(.*?)<\/text>/gs),
      ]
        .map((m) =>
          m[1]
            .replace(/&amp;/g, '&')
