...
async function main() {
      ...
      try {
        ...
        if (postData.includes('resigned')) {
          ...
          presignedResponses.push({
            ...
            authHeader: (resp.request().headers()['authorization'] || 'NONE').substring(0, 50),
          });
        }
      } catch {}
    }
  ...
  if (page.url().includes('realms/edusphere')) {
    ...
    await page.waitForURL('**/localhost:5173/**', { timeout: 15000 }).catch(() => {});
  ...
  writeFileSync('C:/tmp/test-upload.pdf', '%PDF-1.0\n1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj\n%%EOF');
  ...
  const uploadBtn = page.locator('button').filter({ hasText: /העלה|Upload/ }).first();
  ...
  console.log('\nConsole errors:', consoleMessages.filter(m => m.includes('[error]')).slice(-5));

  // Check token state
  const tokenInfo = await page.evaluate(() => {
    const devLoggedIn = window.sessionStorage.getItem('edusphere_dev_logged_in');
