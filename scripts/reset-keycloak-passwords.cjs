...
function httpReq(method, path, body, token, contentType) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(
      { hostname: KC_HOST, port: KC_PORT, path, method, headers },
      (res) => {
        let d = '';
        res.on('data', (c) => (d += c));
...
async function getAdminToken() {
  const body =
    'client_id=admin-cli&username=admin&password=admin123&grant_type=password';
...
async function main() {
  console.log(
    '[keycloak-setup] Starting idempotent Keycloak user bootstrap...'
  );
  ...
  const r1 = await httpReq(
    'GET',
    `/admin/realms/${REALM}/users?max=20`,
    null,
    token
  );
  ...
  const toDelete = existing.filter(
    (u) => !correctIds.has(u.id) && USERS.some((d) => d.email === u.email)
  );
  for (const u of toDelete) {
    const r = await httpReq(
      'DELETE',
      `/admin/realms/${REALM}/users/${u.id}`,
      null,
      token
    );
    console.log(
  ...
  const existingCorrectIds = new Set(
    existing.filter((u) => correctIds.has(u.id)).map((u) => u.id)
  );
  ...
  const toImport = USERS.filter((u) => {
    ...
    const hadCorrectId =
      existingCorrectIds.has(u.id) && !alreadyDeleted.has(u.id);
    return !hadCorrectId;
  });
  ...
  if (toImport.length > 0) {
    ...
    try {
      ...
      console.log(
        '[keycloak-setup] ✅ Partial import: added=' +
          result.added +
          ' skipped=' +
          result.skipped
      );
      ...
      console.log(
        '[keycloak-setup] partialImport HTTP',
        r.status,
        r.body.slice(0, 200)
      );
    }
  ...
  for (const u of USERS) {
    ...
    if (!role) {
      console.log('[keycloak-setup] ❌ Role not found:', u.role);
      continue;
    }
    const r = await httpReq(
      'POST',
    ...
    if (r.status === 204 || r.status === 409) {
      ...
      console.log(
        '[keycloak-setup] ❌ Role assign failed:',
        u.email,
        'HTTP',
        r.status,
        r.body.slice(0, 100)
...
main().catch((err) => {
  console.error('[keycloak-setup] FATAL:', err.message);
  process.exit(1);
});
