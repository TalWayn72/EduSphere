...
describe('Incident response controls (G-18)', () => {
  describe('Incident response procedure document', () => {
    it('INCIDENT_RESPONSE.md exists', () => {
      expect(existsSync(resolve(ROOT, 'docs/security/INCIDENT_RESPONSE.md'))).toBe(true);
    });

    it('defines 72-hour GDPR Art.33 notification requirement', () => {
      const src = readFileSync(resolve(ROOT, 'docs/security/INCIDENT_RESPONSE.md'), 'utf8');
    ...
    it('lists supervisory authorities by EU region', () => {
      ...
      const hasDPA = src.includes('BfDI') || src.includes('CNIL') || src.includes('DPC') || src.includes('supervisory');
  ...
  describe('Wazuh breach detection rules', () => {
    it('Wazuh rules file exists', () => {
      expect(existsSync(resolve(ROOT, 'infrastructure/wazuh/rules/edusphere-breach.xml'))).toBe(true);
    });
  ...
  describe('Falco runtime detection rules', () => {
    it('Falco rules file exists', () => {
      expect(existsSync(resolve(ROOT, 'infrastructure/falco/edusphere-rules.yaml'))).toBe(true);
    });

    it('detects psql direct access bypassing ORM and RLS', () => {
      const src = readFileSync(resolve(ROOT, 'infrastructure/falco/edusphere-rules.yaml'), 'utf8');
