...
describe('Keycloak per-tenant isolation (G-20)', () => {
  describe('Provisioning script', () => {
    it('provisioning script exists', () => {
      expect(existsSync(resolve(ROOT, 'scripts/provision-tenant-realm.sh'))).toBe(true);
    });

    it('script enables brute force protection on new realms (G-12 compliance for all tenants)', () => {
      const src = readFileSync(resolve(ROOT, 'scripts/provision-tenant-realm.sh'), 'utf8');
  ...
  describe('Tenant domains schema (G-21)', () => {
    it('tenant_domains table exists', () => {
      expect(existsSync(resolve(ROOT, 'packages/db/src/schema/tenantDomains.ts'))).toBe(true);
    });

    it('domain field has unique constraint (no two tenants share a domain)', () => {
      const src = readFileSync(resolve(ROOT, 'packages/db/src/schema/tenantDomains.ts'), 'utf8');
    ...
    it('domain schema exported from db schema index', () => {
      const src = readFileSync(resolve(ROOT, 'packages/db/src/schema/index.ts'), 'utf8');
  ...
  describe('Tenant provisioning documentation', () => {
    it('provisioning guide exists', () => {
      expect(existsSync(resolve(ROOT, 'docs/deployment/TENANT_PROVISIONING.md'))).toBe(true);
    });

    it('documents G-20 identity isolation model', () => {
      const src = readFileSync(resolve(ROOT, 'docs/deployment/TENANT_PROVISIONING.md'), 'utf8');
