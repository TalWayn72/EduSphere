...
describe('Zod: createOrganization input validation', () => {
  it('org validation schemas file exists', () => {
    const exists =
      ...
      existsSync(resolve(ROOT, 'apps/subgraph-core/src/org/org-provisioning.schemas.ts')) ||
  ...
  it('createOrganization schema validates slug format (alphanumeric + hyphens)', () => {
    const content =
      ...
      read('apps/subgraph-core/src/org/org-provisioning.schemas.ts') ||
...
describe('Zod: updateBranding input validation', () => {
  it('branding validation schema validates color hex format', () => {
    const content =
      read('apps/subgraph-core/src/org/org.schemas.ts') ||
      read('apps/subgraph-core/src/tenant/tenant.schemas.ts') ||
      read('apps/subgraph-core/src/org/validation.ts');
    // Must validate hex color format (#RRGGBB or #RGB)
    expect(content).toMatch(/hex|#[0-9a-fA-F]|color.*regex|regex.*color/i);
  });

  it('branding validation schema validates URL format for logo', () => {
    const content =
      read('apps/subgraph-core/src/org/org.schemas.ts') ||
      read('apps/subgraph-core/src/tenant/tenant.schemas.ts') ||
      read('apps/subgraph-core/src/org/validation.ts');
  ...
  it('branding validation restricts file types for logo upload', () => {
    const content =
      read('apps/subgraph-core/src/org/org.schemas.ts') ||
      read('apps/subgraph-core/src/tenant/tenant.schemas.ts') ||
      read('apps/subgraph-core/src/org/validation.ts');
    // Should restrict to image types (png, jpg, svg, webp)
    expect(content).toMatch(/png|jpg|jpeg|svg|webp|image|mime/i);
  });
});
...
describe('Zod: marketplace mutations input validation', () => {
  ...
  it('publishToMarketplace validation schema validates pricing', () => {
    const content =
      read('apps/subgraph-content/src/marketplace/marketplace.schemas.ts') ||
      read('apps/subgraph-core/src/marketplace/marketplace.schemas.ts') ||
      read('apps/subgraph-core/src/org/validation.ts');
    if (content) {
      expect(content).toMatch(/price|pricing|pricingModel/);
    }
  });
});
...
describe('GraphQL Authorization: Org onboarding mutations', () => {
  it('org.graphql SDL file exists in subgraph-core', () => {
    const exists =
      ...
      existsSync(resolve(ROOT, 'apps/subgraph-core/src/org/org-provisioning.graphql'));
    expect(exists).toBe(true);
  });
  ...
  it('inviteUser mutation requires ORG_ADMIN role', () => {
    const sdl =
      read('apps/subgraph-core/src/org/org.graphql') ||
      read('apps/subgraph-core/src/org/org-provisioning.graphql');
    if (!sdl) return;
    ...
    const surroundingText = lines
      .slice(inviteLine, inviteLine + 2)
      .join('\n');
    expect(surroundingText).toContain('@authenticated');
    expect(surroundingText).toContain('@requiresRole');
    expect(surroundingText).toContain('ORG_ADMIN');
  });

  it('createApiKey mutation requires ORG_ADMIN role', () => {
    const sdl =
      read('apps/subgraph-core/src/org/org.graphql') ||
      read('apps/subgraph-core/src/org/org-provisioning.graphql');
    if (!sdl) return;
  ...
  it('createWebhook mutation requires ORG_ADMIN role', () => {
    const sdl =
      read('apps/subgraph-core/src/org/org.graphql') ||
      read('apps/subgraph-core/src/org/org-provisioning.graphql');
    if (!sdl) return;
    ...
    const surroundingText = lines
      .slice(webhookLine, webhookLine + 2)
      .join('\n');
    expect(surroundingText).toContain('@authenticated');
    expect(surroundingText).toContain('@requiresRole');
    expect(surroundingText).toContain('ORG_ADMIN');
  ...
  it('revokeApiKey mutation requires ORG_ADMIN role', () => {
    const sdl =
      read('apps/subgraph-core/src/org/org.graphql') ||
      read('apps/subgraph-core/src/org/org-provisioning.graphql');
    if (!sdl) return;
    ...
    const surroundingText = lines
      .slice(revokeLine, revokeLine + 2)
      .join('\n');
    expect(surroundingText).toContain('@authenticated');
    expect(surroundingText).toContain('ORG_ADMIN');
  });

  it('updateGamificationConfig mutation requires ORG_ADMIN role', () => {
    const sdl =
      read('apps/subgraph-core/src/org/org.graphql') ||
      read('apps/subgraph-core/src/org/org-provisioning.graphql');
    if (!sdl) return;
    ...
    if (gamLine === -1) return;
    const surroundingText = lines.slice(gamLine, gamLine + 2).join('\n');
    expect(surroundingText).toContain('@authenticated');
    expect(surroundingText).toContain('ORG_ADMIN');
  });
