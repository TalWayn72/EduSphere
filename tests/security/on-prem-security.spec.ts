...
describe('On-premises security documentation', () => {
  describe('Security hardening checklist', () => {
    it('SECURITY_HARDENING.md exists', () => {
      expect(existsSync(resolve(ROOT, 'docs/deployment/SECURITY_HARDENING.md'))).toBe(true);
    });

    it('covers network security: TLS for all inter-service comms', () => {
      const src = readFileSync(resolve(ROOT, 'docs/deployment/SECURITY_HARDENING.md'), 'utf8');
  ...
  describe('Air-gapped installation guide', () => {
    it('AIR_GAPPED_INSTALL.md exists', () => {
      expect(existsSync(resolve(ROOT, 'docs/deployment/AIR_GAPPED_INSTALL.md'))).toBe(true);
    });

    it('covers container image export procedure', () => {
      const src = readFileSync(resolve(ROOT, 'docs/deployment/AIR_GAPPED_INSTALL.md'), 'utf8');
