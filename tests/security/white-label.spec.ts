...
describe('White-label branding isolation (G-19)', () => {
  describe('TenantBrandingService', () => {
    it('branding service file exists', () => {
      const exists = (() => {
        try {
          readFileSync(resolve(ROOT, 'apps/subgraph-core/src/tenant/tenant-branding.service.ts'));
          return true;
        } catch { return false; }
      })();
    ...
    it('service uses per-tenant lookup with tenantId filter', () => {
      const src = readFileSync(
        resolve(ROOT, 'apps/subgraph-core/src/tenant/tenant-branding.service.ts'),
        'utf8',
      );
  ...
  describe('Frontend branding helper', () => {
    ...
    it('applies primary color as CSS custom property', () => {
      const src = readFileSync(resolve(ROOT, 'apps/web/src/lib/branding.ts'), 'utf8');
