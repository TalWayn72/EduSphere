...
const API_URL =
  __ENV.GATEWAY_URL?.replace('/graphql', '') || 'http://localhost:4000';
...
export default function () {
  ...
  const uploadRes = http.post(`${API_URL}/graphql`, UPLOAD_MUTATION, {
    headers,
  });
  const uploadOk = check(uploadRes, {
    ...
    'job created or auth error': (r) => {
      try {
        return acceptedBody(JSON.parse(r.body), 'requestTranscription');
      } catch {
        return false;
      }
    },
  ...
  let jobId;
  try {
    jobId = JSON.parse(uploadRes.body).data?.requestTranscription?.jobId;
  } catch {
    sleep(1);
    return;
  }
  ...
  for (let i = 0; i < 2; i++) {
    const pollRes = http.post(`${API_URL}/graphql`, statusQuery(jobId), {
      headers,
    });
    check(pollRes, {
      ...
      'status field present': (r) => {
        try {
          return (
            JSON.parse(r.body).data?.transcriptionJob?.status !== undefined
          );
        } catch {
          return false;
        }
    ...
    const status = (() => {
      try {
        return JSON.parse(pollRes.body).data?.transcriptionJob?.status;
      } catch {
        return null;
      }
    })();
