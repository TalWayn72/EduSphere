...
let UserService = UserService_1 = class UserService {
    ...
    mapUser(user) {
        ...
        return {
            ...
            preferences: (0, user_preferences_service_1.parsePreferences)(user['preferences']),
        };
    }
    ...
    async bulkImportUsers(csvData, authContext) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const row = Object.fromEntries(headers.map((h, idx) => [h, rawValues[idx] ?? '']));
            if (!row['email']) {
        ...
        return (0, db_1.withTenantContext)(this.db, tenantCtx, async (tx) => {
            const rows = await (opts.role
                ? tx
                    ...
                    .limit(opts.limit)
                    .offset(opts.offset)
                : tx.select().from(db_1.schema.users).limit(opts.limit).offset(opts.offset));
