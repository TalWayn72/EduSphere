...
type Mutation {
  ...
  ): LiveSession! @authenticated @requiresScopes(scopes: ["sessions:write"])
  endLiveSession(sessionId: ID!): LiveSession!
    @authenticated
    @requiresScopes(scopes: ["sessions:write"])
  joinLiveSession(sessionId: ID!): LiveSession! @authenticated
...
type Query {
  getSessionAttendees(sessionId: ID!): [User!]! @authenticated
}
```
````
...
type Mutation {
  createCheckoutSession(courseId: ID!): CheckoutSession! @authenticated
}
```
...
type Mutation {
  uploadModel3D(courseId: ID!, filename: String!, format: String!): MediaAsset!
    @authenticated
