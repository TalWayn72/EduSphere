...
1. `mcp__coordination-bridge__cb_register_agent({ id: "L1-DB-lead", division: "Database", role: "Lead" })`
2. `mcp__coordination-bridge__cb_update_status({ id: "L1-DB-lead", status: "running" })`
3. `mcp__vector-memory__vm_search({ query: "<task keywords>", n_results: 5 })` — check prior art
4. `mcp__vector-memory__vm_search_decisions({ query: "database", n_results: 5 })` — check past decisions
5. `mcp__coordination-bridge__cb_get_pending_help({ division: "Database" })` — answer pending requests
...
3. `mcp__coordination-bridge__cb_publish({ channel: "db:complete", ... })`
4. `mcp__coordination-bridge__cb_update_status({ id: "L1-DB-lead", status: "complete" })`
