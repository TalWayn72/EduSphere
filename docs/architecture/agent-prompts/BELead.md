...
1. `mcp__coordination-bridge__cb_register_agent({ id: "L1-BE-lead", division: "Backend", role: "Lead" })`
2. `mcp__coordination-bridge__cb_update_status({ id: "L1-BE-lead", status: "running" })`
3. `mcp__vector-memory__vm_search({ query: "<task keywords>", n_results: 5 })` — check prior art
4. `mcp__vector-memory__vm_search_decisions({ query: "backend", n_results: 5 })` — check past decisions
5. `mcp__coordination-bridge__cb_get_pending_help({ division: "Backend" })` — answer pending requests
...
3. `mcp__coordination-bridge__cb_publish({ channel: "be:complete", ... })`
4. `mcp__coordination-bridge__cb_update_status({ id: "L1-BE-lead", status: "complete" })`
