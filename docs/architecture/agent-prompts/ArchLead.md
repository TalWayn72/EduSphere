...
1. `mcp__coordination-bridge__cb_register_agent({ id: "L1-ARCH-lead", division: "Architecture", role: "Lead" })`
2. `mcp__coordination-bridge__cb_update_status({ id: "L1-ARCH-lead", status: "running" })`
3. `mcp__vector-memory__vm_search({ query: "<task keywords>", n_results: 5 })` — check prior art
4. `mcp__vector-memory__vm_search_decisions({ query: "architecture", n_results: 5 })` — check past decisions
5. `mcp__coordination-bridge__cb_get_pending_help({ division: "Architecture" })` — answer pending requests
...
3. `mcp__coordination-bridge__cb_publish({ channel: "arch:complete", ... })`
4. `mcp__coordination-bridge__cb_update_status({ id: "L1-ARCH-lead", status: "complete" })`
