...
export function getOrCreatePool(connectionString?: string): Pool {
  ...
  if (!_pools.has(key)) {
    _pools.set(
      key,
      new Pool({
        connectionString: key !== 'default' ? key : process.env.DATABASE_URL,
        max: 10,
...
- `apps/web/src/lib/urql-client.ts`:
  ...
  if (typeof window !== 'undefined')
    window.addEventListener('beforeunload', () => wsClient.dispose());
  ```
...
export class LangGraphService implements OnModuleInit, OnModuleDestroy {
  ...
  private scheduleMemoryTrim(): void {
    ...
    this.cleanupInterval = setInterval(
      () => {
        const storage = (
          this.checkpointer as unknown as { storage?: Map<string, unknown> }
        ).storage;
        if (storage && storage.size > MAX) {
          [...storage.keys()]
            .slice(0, storage.size - MAX)
...
export class SessionCleanupService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    this.interval = setInterval(() => void this.cleanup(), 30 * 60 * 1000);
  }
  onModuleDestroy() {
    ...
    await this.db
      ...
      .where(
        and(
          ...
          lt(schema.agentSessions.createdAt, cutoff)
        )
      );
  }
}
...
_Generated: 2026-02-22 | Issues found: 28 (5 Critical, 10 High, 13 Medium) | Files to modify: 45 | New files: 14_
