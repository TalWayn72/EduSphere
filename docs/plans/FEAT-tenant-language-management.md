...
```ts
const KNOWN_LOCALES = ['en','zh-CN','hi','es','fr','bn','pt','ru','id','he'] as const;
export const UpdateTenantLanguageSettingsSchema = z.object({
...
```ts
const [tenantLangResult] = useQuery({ query: MY_TENANT_LANGUAGE_SETTINGS_QUERY });
const availableLocales = (tenantLangResult.data?.myTenantLanguageSettings?.supportedLanguages
  ?? SUPPORTED_LOCALES) as SupportedLocale[];
const tenantDefault = (tenantLangResult.data?.myTenantLanguageSettings?.defaultLanguage ?? 'en') as SupportedLocale;

// Auto-fallback: if user's saved locale was disabled by admin, switch to tenant default
useEffect(() => {
  if (availableLocales.length > 0 && !availableLocales.includes(currentLocale)) {
