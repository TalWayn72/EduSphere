...
extend type Query {
  ...
  myCoursePipelines(courseId: ID!): [LessonPipeline!]!
    @authenticated
    @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
}
extend type Mutation {
  createLessonPipeline(input: CreatePipelineInput!): LessonPipeline!
    @authenticated
    @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
  addPipelineStep(input: AddStepInput!): LessonPipeline!
    @authenticated
    @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
  reorderPipelineSteps(pipelineId: ID!, stepIds: [ID!]!): LessonPipeline!
    @authenticated
    @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
  publishLessonPipeline(id: ID!): LessonPipeline!
    @authenticated
    @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
}
```
