...
- GraphQL SDL: `apps/subgraph-content/src/content-import/content-import.graphql`
  ...
  extend type Mutation {
    importFromYoutube(input: YoutubeImportInput!): ImportJob!
      @authenticated
      @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
    importFromWebsite(input: WebsiteImportInput!): ImportJob!
      @authenticated
      @requiresRole(roles: [INSTRUCTOR, ORG_ADMIN, SUPER_ADMIN])
...
extend type Mutation {
  ...
  ingestContentBatch(
    files: [Upload!]!
    courseId: ID!
  ): [ContentIngestionResult!]! @authenticated
}
