...
export class SrsService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.warn({ err }, 'SrsService: NATS connection failed — digest disabled');
    }
  ...
  private mapCard(row: typeof schema.spacedRepetitionCards.$inferSelect): SRSCard {
    return {
      id: row.id,
  ...
  async getDueReviews(userId: string, tenantId: string, limit = 20): Promise<SRSCard[]> {
    return withTenantContext(this.db, this.ctx(userId, tenantId), async (tx) => {
      const now = new Date();
      const rows = await tx
        .select()
  ...
  async createCard(userId: string, tenantId: string, conceptName: string): Promise<SRSCard> {
    return withTenantContext(this.db, this.ctx(userId, tenantId), async (tx) => {
      const [row] = await tx
      ...
      this.logger.debug({ userId, tenantId, conceptName }, 'SRS card created');
      return this.mapCard(row);
    });
  }

  async submitReview(cardId: string, userId: string, tenantId: string, quality: number): Promise<SRSCard> {
  ...
  private scheduleDailyDigest(): void {
    ...
    this.digestIntervalHandle = setInterval(runDigest, DAILY_DIGEST_INTERVAL_MS);
  ...
  private async publishDailyDigest(): Promise<void> {
    if (!this.nats) return;
    try {
      const payload = JSON.stringify({ event: 'srs.review.due', timestamp: new Date().toISOString() });
      this.nats.publish(SRS_REVIEW_DUE_SUBJECT, new TextEncoder().encode(payload));
      this.logger.log({ subject: SRS_REVIEW_DUE_SUBJECT }, 'SRS daily digest published');
    } catch (err) {
