...
vi.mock("@edusphere/db", () => ({
  ...
  schema: {
    scimTokens: { id: "id", tenantId: "tenant_id", tokenHash: "token_hash", isActive: "is_active", lastUsedAt: "last_used_at" },
  },
...
describe("ScimTokenService", () => {
  ...
  beforeEach(async () => {
    ...
    mockFrom = vi.fn().mockReturnValue({ where: vi.fn().mockReturnValue({ limit: mockLimit }), limit: mockLimit });
    ...
    withTenantContextMock.mockImplementation(async (_db, _ctx, fn) => fn({
      insert: mockInsert, update: mockUpdate, select: mockSelect,
      execute: vi.fn().mockResolvedValue({ rows: [] }),
    }));

    service = new ScimTokenService();
    Object.assign(service, { db: { select: mockSelect, update: mockUpdate, insert: mockInsert } });
  });
  ...
  it("validateToken returns tenantId for valid token", async () => {
    mockFrom.mockReturnValueOnce({ where: vi.fn().mockReturnValue({ limit: vi.fn().mockResolvedValue([BASE_ROW]) }) });
    const result = await service.validateToken("some-valid-raw-token-padded-to-64chars-exactly-here");
  ...
  it("validateToken rejects expired token", async () => {
    ...
    mockFrom.mockReturnValueOnce({ where: vi.fn().mockReturnValue({ limit: vi.fn().mockResolvedValue([expired]) }) });
    const result = await service.validateToken("some-raw-token-expired-padded-to-64-chars-exactly");
    expect(result).toBeNull();
  });

  it("validateToken rejects inactive token", async () => {
    const inactive = { ...BASE_ROW, isActive: false };
    mockFrom.mockReturnValueOnce({ where: vi.fn().mockReturnValue({ limit: vi.fn().mockResolvedValue([inactive]) }) });
    const result = await service.validateToken("some-raw-token-inactive-padded-to-64-chars-exactly");
    expect(result).toBeNull();
  });

  it("validateToken rejects unknown token", async () => {
    mockFrom.mockReturnValueOnce({ where: vi.fn().mockReturnValue({ limit: vi.fn().mockResolvedValue([]) }) });
    const result = await service.validateToken("unknown-token-not-in-database-padded-to-64-chars-x");
    expect(result).toBeNull();
  });
  ...
  it("listTokens never exposes raw token — only metadata fields", async () => {
    withTenantContextMock.mockImplementationOnce(async (_db, _ctx, fn) => fn({
      insert: mockInsert, update: mockUpdate,
      select: () => ({ from: () => ({ where: () => Promise.resolve([BASE_ROW]) }) }),
    }));
