...
import { withTenantContext, createDatabaseConnection, schema, eq, desc } from "@edusphere/db";
...
export class ScimResolver {
  ...
  private requireAdmin(ctx: GqlCtx): AuthContext {
    if (!ctx.authContext?.userId) throw new UnauthorizedException("Authentication required");
  ...
  @Query("scimSyncLog")
  async getScimSyncLog(@Args("limit") limit: number | undefined, @Context() ctx: GqlCtx) {
    const auth = this.requireAdmin(ctx);
    const tenantCtx: TenantContext = { tenantId: auth.tenantId, userId: auth.userId, userRole: "ORG_ADMIN" };
    const rows = await withTenantContext(this.db, tenantCtx, async (tx) =>
      tx.select().from(schema.scimSyncLog)
  ...
  async generateScimToken(
    ...
    const auth = this.requireAdmin(ctx);
    return this.tokenService.generateToken(auth.tenantId, auth.userId, input.description, input.expiresInDays);
  }
