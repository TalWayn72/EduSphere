...
export class ScimUserService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.log('ScimUserService: NATS connected');
    } catch (err) {
      this.logger.warn({ err }, 'ScimUserService: NATS unavailable');
    }
  }
  ...
  async listUsers(
    ...
    return withTenantContext(this.db, ctx, async (tx) => {
      ...
      const countRows = await tx.execute<{ count: string }>(
        sql`SELECT COUNT(*)::text AS count FROM users WHERE tenant_id = ${tenantId}::uuid`
      );
  ...
  async getUser(tenantId: string, userId: string): Promise<ScimUser | null> {
    ...
    const rows = await withTenantContext(this.db, ctx, async (tx) =>
      tx
        ...
        .from(schema.users)
        .where(
          and(eq(schema.users.id, userId), eq(schema.users.tenant_id, tenantId))
        )
        .limit(1)
    );
  ...
  async createUser(tenantId: string, scimUser: ScimUser): Promise<ScimUser> {
    ...
    const rows = await withTenantContext(this.db, ctx, async (tx) => {
      const inserted = await tx
        ...
        .values({
          ...
          role:
            (ext?.role as (typeof schema.users.$inferSelect)['role']) ??
            'STUDENT',
          tenant_id: tenantId,
        })
        .returning();
    ...
    if (ext?.courseIds?.length)
      this.publishEvent('EDUSPHERE.scim.enrollment', {
        userId: created.id,
        tenantId,
        courseIds: ext.courseIds,
      });
  ...
  private patchToUpdates(
    op: ScimPatchOp
  ): Partial<typeof schema.users.$inferInsert> {
    const updates: Partial<typeof schema.users.$inferInsert> = {};
    if (op.path === 'name.givenName')
      updates.first_name = String(op.value ?? '');
    if (op.path === 'name.familyName')
      updates.last_name = String(op.value ?? '');
