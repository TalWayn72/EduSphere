...
describe("ScimUserService", () => {
  ...
  beforeEach(async () => {
    ...
    withTenantContextMock.mockImplementation(async (_db, _ctx, fn) => fn({
      ...
      select: () => ({ from: () => ({ where: () => ({ limit: () => Promise.resolve([MOCK_USER_ROW]) }) }) }),
      execute: vi.fn().mockResolvedValue({ rows: [{ count: "5" }] }),
    }));

    mockNats = { drain: vi.fn().mockResolvedValue(undefined), publish: vi.fn() };
    vi.mocked(connect).mockResolvedValue(mockNats as Parameters<typeof connect>[0] extends infer _T ? unknown : never);
  ...
  it("deleteUser soft-deletes and publishes NATS deprovisioned event", async () => {
    ...
    expect(mockNats.publish).toHaveBeenCalledWith("EDUSPHERE.user.deprovisioned", expect.any(Uint8Array));
    expect(mockInsert).toHaveBeenCalledTimes(1);
  });

  it("patchUser with op=replace updates the specified field", async () => {
    const ops: ScimPatchOp[] = [{ op: "replace", path: "name.givenName", value: "Bob" }];
  ...
  it("patchUser with op=add handles add operation (same path as replace)", async () => {
    const ops: ScimPatchOp[] = [{ op: "add", path: "name.familyName", value: "Johnson" }];
  ...
  it("listUsers respects startIndex pagination", async () => {
    ...
    withCtxMock.mockImplementationOnce(async (_db, _ctx, fn) => fn({
      select: () => ({ from: () => ({ where: () => ({ limit: () => ({ offset: vi.fn().mockResolvedValue([MOCK_USER_ROW]) }) }) }) }),
  ...
  it("createUser with group courseIds triggers SCIM enrollment NATS event", async () => {
    ...
    expect(mockNats.publish).toHaveBeenCalledWith("EDUSPHERE.user.created", expect.any(Uint8Array));
    expect(mockNats.publish).toHaveBeenCalledWith("EDUSPHERE.scim.enrollment", expect.any(Uint8Array));
  });
});