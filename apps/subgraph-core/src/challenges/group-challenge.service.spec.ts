...
describe('GroupChallengeService', () => {
  ...
  it('joinChallenge — throws when challenge is full', async () => {
    ...
    mockSelect.mockReturnValueOnce({
      from: vi.fn(() => ({
        where: vi.fn(() => ({
          limit: vi.fn().mockResolvedValue([{ maxParticipants: 2 }]),
        })),
      })),
    });
    ...
    await expect(
      service.joinChallenge(TENANT, USER, CHALLENGE)
  ...
  it('joinChallenge — throws on duplicate join', async () => {
    mockSelect.mockReturnValueOnce({
      from: vi.fn(() => ({
        where: vi.fn(() => ({
          limit: vi.fn().mockResolvedValue([{ maxParticipants: 50 }]),
        })),
      })),
    });
    mockSelect.mockReturnValueOnce({
    ...
    mockSelect.mockReturnValueOnce({
      from: vi.fn(() => ({
        where: vi.fn(() => ({
          limit: vi.fn().mockResolvedValue([{ id: 'p-uuid' }]),
        })),
      })),
    });
  ...
  it('publishScoreEvent — publishes NATS event when connection available', async () => {
    ...
    await expect(
      service.publishScoreEvent(TENANT, USER, CHALLENGE, 85)
    ).resolves.not.toThrow();
  });

  it('getMyParticipations — returns participations for userId', async () => {
    const rows = [
      { id: 'p1', userId: USER, challengeId: CHALLENGE, score: 75 },
    ];
    mockWithTenantContext.mockImplementationOnce(
      async (_db, _ctx, fn: (tx: unknown) => unknown) =>
        fn({
          select: vi.fn(() => ({
