...
describe('WebhookService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    const deliverySvc = new WebhookDeliveryService() as InstanceType<
  ...
  describe('createWebhook', () => {
    it('should validate URL and events then insert', async () => {
      ...
      const result = await service.createWebhook(
        { url: 'https://example.com/hook', events: ['course.completed'] },
        ctx
      );
    ...
    it('should throw when org has max webhooks', async () => {
      ...
      await expect(
        service.createWebhook(
          { url: 'https://x.com/h', events: ['course.completed'] },
          ctx
        )
  ...
  describe('updateWebhook', () => {
    it('should update webhook and return mapped result', async () => {
      ...
      const result = await service.updateWebhook(
        'w1',
        { url: 'https://new.com/hook', events: ['badge.issued'] },
        ctx
      );
  ...
  describe('testWebhook', () => {
    ...
    it('should throw when webhook not found', async () => {
      mockExecute.mockResolvedValue({ rows: [] });
      await expect(service.testWebhook('missing', ctx)).rejects.toThrow(
        'Webhook not found'
      );
    });
  });
