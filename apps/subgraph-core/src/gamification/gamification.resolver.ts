...
export class GamificationResolver {
  ...
  private requireAuth(ctx: GraphQLContext) {
    ...
    return { userId: ctx.authContext.userId, tenantId: ctx.authContext.tenantId || '' };
  }
  ...
  @Query('leaderboard')
  async getLeaderboard(@Args('limit') limit: number | undefined, @Context() ctx: GraphQLContext) {
  ...
  async createBadge(@Args("input") input: Parameters<BadgeService["createBadge"]>[0], @Context() ctx: GraphQLContext) {
    const { tenantId } = this.requireAuth(ctx);
    return this.badgeService.createBadge(input, tenantId);
  }

  @Mutation("updateBadge")
  async updateBadge(@Args("id") id: string, @Args("input") input: Parameters<BadgeService["updateBadge"]>[1], @Context() ctx: GraphQLContext) {
