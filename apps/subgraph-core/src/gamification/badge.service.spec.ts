...
const { mockExecute, mockSelect, mockInsert, mockTx, mockNatsConn, _mockNatsSubscription } =
  vi.hoisted(() => {
    const mockExecute = vi.fn();
    const mockSelect = vi.fn();
    const mockInsert = vi.fn();
    const mockTx = { execute: mockExecute, select: mockSelect, insert: mockInsert };
    ...
    return { mockExecute, mockSelect, mockInsert, mockTx, mockNatsConn, _mockNatsSubscription };
  });
...
vi.mock('@edusphere/db', () => ({
  ...
  schema: {
    ...
    userBadges: { id: 'id', userId: 'user_id', badgeId: 'badge_id', earnedAt: 'earned_at' },
...
vi.mock('./badge-definitions.js', () => ({
  PLATFORM_BADGES: [
    { name: 'First Step', description: 'd', icon: '🎓', category: 'COMPLETION',
      pointsReward: 100, conditionType: 'courses_completed', conditionValue: 1 },
  ],
...
describe('BadgeService', () => {
  ...
  it('checkAndAwardBadges awards badge when condition met', async () => {
    ...
    mockSelect
      .mockReturnValueOnce({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([badge]) }) })
  ...
  it('checkAndAwardBadges is idempotent — does not award badge twice', async () => {
    ...
    mockSelect
      .mockReturnValueOnce({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([badge]) }) })
      .mockReturnValueOnce({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([{ id: 'ub1' }]) }) });
    await service.checkAndAwardBadges('u1', 't1', 'courses_completed', 1);
    expect(mockInsert).not.toHaveBeenCalled();
  });
  ...
  it('leaderboard returns entries with correct rank', async () => {
    const rows = [
      { user_id: 'u1', first_name: 'Alice', last_name: 'A', total_points: 500, badge_count: '3' },
      { user_id: 'u2', first_name: 'Bob', last_name: 'B', total_points: 200, badge_count: '1' },
    ];
