...
import { createDatabaseConnection, schema, eq, and, sql, withTenantContext, closeAllPools } from '@edusphere/db';
...
export interface LeaderboardEntry {
  rank: number; userId: string; displayName: string; totalPoints: number; badgeCount: number;
}
interface NatsCoursePayload { userId: string; tenantId: string; courseId?: string }
interface NatsAnnotationPayload { userId: string; tenantId: string }
interface NatsStreakPayload { userId: string; tenantId: string; streakDays: number }
const MAX_SUBS = 10;
...
export class BadgeService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    } catch (err) { this.logger.warn({ err }, 'BadgeService: NATS unavailable'); }
  }
  ...
  private async subscribeToEvents(): Promise<void> {
    ...
    this.subs.push(courseSub); void this.handleCourseEvents(courseSub);
    const annotSub = nc.subscribe('EDUSPHERE.annotation.created');
    this.subs.push(annotSub); void this.handleAnnotationEvents(annotSub);
    const streakSub = nc.subscribe('EDUSPHERE.streak.milestone');
    this.subs.push(streakSub); void this.handleStreakEvents(streakSub);
  }

  private async handleCourseEvents(sub: Subscription): Promise<void> {
    for await (const msg of sub) {
      try {
        const p = JSON.parse(new TextDecoder().decode(msg.data)) as NatsCoursePayload;
        const pts = POINT_AWARDS['course.completed'] ?? 0;
        await this.awardPoints(p.userId, p.tenantId, 'course.completed', pts, 'Course completed');
        const count = await this.countUserCourses(p.userId, p.tenantId);
        await this.checkAndAwardBadges(p.userId, p.tenantId, 'courses_completed', count);
      } catch (err) { this.logger.error({ err }, 'course.completed event error'); }
    }
  }
  ...
  private async handleAnnotationEvents(sub: Subscription): Promise<void> {
    for await (const msg of sub) {
      try {
        const p = JSON.parse(new TextDecoder().decode(msg.data)) as NatsAnnotationPayload;
        const pts = POINT_AWARDS['annotation.created'] ?? 0;
        await this.awardPoints(p.userId, p.tenantId, 'annotation.created', pts, 'Annotation created');
        const count = await this.countUserAnnotations(p.userId, p.tenantId);
        await this.checkAndAwardBadges(p.userId, p.tenantId, 'annotations_created', count);
      } catch (err) { this.logger.error({ err }, 'annotation.created event error'); }
    }
  }
  ...
  private async handleStreakEvents(sub: Subscription): Promise<void> {
    for await (const msg of sub) {
      try {
        const p = JSON.parse(new TextDecoder().decode(msg.data)) as NatsStreakPayload;
        const key = 'streak.milestone.' + String(p.streakDays);
        const pts = (Object.prototype.hasOwnProperty.call(POINT_AWARDS, key)
          ? POINT_AWARDS[key as keyof typeof POINT_AWARDS]
          : undefined) ?? 0;
        if (pts > 0) await this.awardPoints(p.userId, p.tenantId, key, pts, String(p.streakDays) + '-day streak');
        await this.checkAndAwardBadges(p.userId, p.tenantId, 'streak_days', p.streakDays);
      } catch (err) { this.logger.error({ err }, 'streak.milestone event error'); }
    }
  }

  async checkAndAwardBadges(
    userId: string, tenantId: string, conditionType: string, newValue: number,
  ): Promise<void> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    await withTenantContext(this.db, ctx, async (tx) => {
      const eligible = await tx.select().from(schema.badges).where(
        and(eq(schema.badges.conditionType, conditionType),
          sql`${schema.badges.conditionValue} <= ${newValue}`),
      );
      for (const badge of eligible) {
        const already = await tx.select({ id: schema.userBadges.id }).from(schema.userBadges)
          .where(and(eq(schema.userBadges.userId, userId), eq(schema.userBadges.badgeId, badge.id)));
        if (already.length > 0) continue;
        await tx.insert(schema.userBadges).values({ userId, badgeId: badge.id, tenantId });
        await tx.insert(schema.pointEvents).values({
          userId, tenantId, eventType: 'badge.earned', points: badge.pointsReward,
  ...
  async awardPoints(
    userId: string, tenantId: string, eventType: string, points: number, description: string,
  ): Promise<void> {
    if (points <= 0) return;
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    await withTenantContext(this.db, ctx, async (tx) => {
      await tx.insert(schema.pointEvents).values({ userId, tenantId, eventType, points, description });
      await this.upsertPointTotal(tx, userId, tenantId, points);
    });
  }

  private async upsertPointTotal(tx: Database, userId: string, tenantId: string, delta: number): Promise<void> {
  ...
  async myBadges(userId: string, tenantId: string): Promise<Array<{ id: string; badge: typeof schema.badges.$inferSelect; earnedAt: Date }>> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    return withTenantContext(this.db, ctx, async (tx) => {
      const rows = await tx.select({
        id: schema.userBadges.id, earnedAt: schema.userBadges.earnedAt, badge: schema.badges,
      }).from(schema.userBadges)
        .innerJoin(schema.badges, eq(schema.userBadges.badgeId, schema.badges.id))
        .where(eq(schema.userBadges.userId, userId));
      return rows.map((r) => ({ id: r.id, badge: r.badge, earnedAt: r.earnedAt }));
    });
  }

  async leaderboard(tenantId: string, limit: number): Promise<LeaderboardEntry[]> {
    const ctx: TenantContext = { tenantId, userId: 'system', userRole: 'SUPER_ADMIN' };
    return withTenantContext(this.db, ctx, async (tx) => {
      const rows = await tx.execute<{
        user_id: string; first_name: string; last_name: string; total_points: number; badge_count: string;
      }>(sql`
      ...
      return rows.rows.map((r, i) => ({
        rank: i + 1, userId: r.user_id,
        displayName: (r.first_name + ' ' + r.last_name).trim(),
        totalPoints: Number(r.total_points), badgeCount: Number(r.badge_count),
      }));
    });
  }
  ...
  async myTotalPoints(userId: string, tenantId: string): Promise<number> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    return withTenantContext(this.db, ctx, async (tx) => {
      const rows = await tx.select({ total: schema.userPoints.totalPoints })
        .from(schema.userPoints).where(eq(schema.userPoints.userId, userId));
      return rows[0]?.total ?? 0;
    });
  }

  private async countUserCourses(userId: string, tenantId: string): Promise<number> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    return withTenantContext(this.db, ctx, async (tx) => {
  ...
  private async countUserAnnotations(userId: string, tenantId: string): Promise<number> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    return withTenantContext(this.db, ctx, async (tx) => {
      const rows = await tx.select({ count: sql`COUNT(*)` })
        .from(schema.annotations)
        .where(and(eq(schema.annotations.user_id, userId), eq(schema.annotations.tenant_id, tenantId)));
  ...
  private async seedPlatformBadges(): Promise<void> {
    ...
    } catch (err) { this.logger.warn({ err }, 'Badge seeding skipped'); }
  }

  async adminBadges(tenantId: string): Promise<typeof schema.badges.$inferSelect[]> {
    const rows = await this.db.select().from(schema.badges).where(
  ...
  async createBadge(
    input: {
      name: string; description: string; iconEmoji: string; category: string;
      pointsReward: number; conditionType: string; conditionValue: number;
    },
    tenantId: string,
  ...
  async updateBadge(
    ...
    const [badge] = await this.db.update(schema.badges)
      .set(input)
