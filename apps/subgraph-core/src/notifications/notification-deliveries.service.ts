...
export class NotificationDeliveriesService implements OnModuleDestroy {
  ...
  async recordDelivery(
    ...
    const channelVal =
      channel.toLowerCase() as (typeof schema.notificationDeliveries.$inferInsert)['channel'];
  ...
  async getHistory(
    ...
    return {
      ...
      pageInfo: {
        ...
        startCursor: sliced[0]
          ? Buffer.from(sliced[0].createdAt.toISOString()).toString('base64')
          : null,
  ...
  private aggregateAnalytics(
    rows: Array<typeof schema.notificationDeliveries.$inferSelect>
  ) {
    ...
    for (const r of rows) {
      ...
      const ch = byChannel.get(r.channel) ?? {
        sent: 0,
        delivered: 0,
        failed: 0,
      };
      ch.sent++;
      if (r.status === 'delivered') ch.delivered++;
      if (r.status === 'failed' || r.status === 'bounced') ch.failed++;
      byChannel.set(r.channel, ch);

      const tp = byType.get(r.notificationType) ?? {
        sent: 0,
        delivered: 0,
        failed: 0,
      };
      tp.sent++;
      if (r.status === 'delivered') tp.delivered++;
      if (r.status === 'failed' || r.status === 'bounced') tp.failed++;
    ...
    return {
      ...
      totalDelivered,
      totalFailed,
      byChannel: Array.from(byChannel.entries()).map(([channel, s]) => ({
        channel,
        ...s,
      })),
      byType: Array.from(byType.entries()).map(([notificationType, s]) => ({
        notificationType,
        ...s,
      })),
    };
  ...
  private mapRow(
    r: typeof schema.notificationDeliveries.$inferSelect
  ): DeliveryDto {
    return {
      id: r.id,
