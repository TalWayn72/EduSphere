...
vi.mock('@edusphere/db', () => ({
  ...
  schema: {
    customRoles: { id: 'id', tenantId: 'tenant_id', name: 'name', isSystem: 'is_system', createdAt: 'created_at' },
    userRoleDelegations: { id: 'id', tenantId: 'tenant_id', userId: 'user_id', roleId: 'role_id', isActive: 'is_active' },
  },
  eq: vi.fn((a, b) => ({ eq: [a, b] })),
  and: vi.fn((...a) => ({ and: a })),
}));
...
const TENANT_CTX = { tenantId: 'tenant-1', userId: 'admin-1', role: 'ORG_ADMIN' };

const makeRole = (overrides = {}) => ({
  id: 'role-1', tenantId: 'tenant-1', name: 'Content Manager',
  description: 'Manages courses', permissions: ['courses:view'],
  isSystem: false, createdBy: 'admin-1', createdAt: new Date(), updatedAt: new Date(),
  ...overrides,
});

const makeDelegation = (overrides = {}) => ({
  id: 'del-1', userId: 'user-1', roleId: 'role-1', delegatedBy: 'admin-1',
  validUntil: null, isActive: true, createdAt: new Date(), ...overrides,
});

const setupSelectLimit = (rows) => mockTx.select.mockReturnValue({
  from: vi.fn().mockReturnValue({ where: vi.fn().mockReturnValue({ limit: vi.fn().mockResolvedValue(rows) }) }),
});
const setupSelectOrderBy = (rows) => mockTx.select.mockReturnValue({
  from: vi.fn().mockReturnValue({ where: vi.fn().mockReturnValue({ orderBy: vi.fn().mockResolvedValue(rows) }) }),
});
const setupInsert = (result) => mockTx.insert.mockReturnValue({
  values: vi.fn().mockReturnValue({ returning: vi.fn().mockResolvedValue(result) }),
});
const setupUpdateNoReturn = () => mockTx.update.mockReturnValue({
  set: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(undefined) }),
});
const setupUpdateWithReturn = (result) => mockTx.update.mockReturnValue({
  set: vi.fn().mockReturnValue({ where: vi.fn().mockReturnValue({ returning: vi.fn().mockResolvedValue(result) }) }),
});
const setupDelete = () => mockTx.delete.mockReturnValue({ where: vi.fn().mockResolvedValue(undefined) });
...
describe('CustomRoleService', () => {
  ...
  it('should throw NotFoundException for unknown role', async () => {
    setupSelectLimit([]);
    await expect(service.getRole('unknown-id', TENANT_CTX)).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('should create and return new role', async () => {
    ...
    const result = await service.createRole(
      { name: 'Content Manager', description: 'Manages courses', permissions: ['courses:view'] },
      TENANT_CTX,
    );
  ...
  it('should throw BadRequestException when updating system role', async () => {
    ...
    await expect(service.updateRole('role-1', { name: 'X' }, TENANT_CTX)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('should update and return custom role', async () => {
    ...
    const result = await service.updateRole('role-1', { name: 'Updated' }, TENANT_CTX);
    expect(result.name).toBe('Updated');
  });

  it('should throw BadRequestException when deleting system role', async () => {
    setupSelectLimit([makeRole({ isSystem: true })]);
    await expect(service.deleteRole('role-1', TENANT_CTX)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('should throw NotFoundException when delegating non-existent role', async () => {
    setupSelectLimit([]);
    await expect(service.delegateRole('user-1', 'bad-role', null, TENANT_CTX)).rejects.toBeInstanceOf(NotFoundException);
  });

  it('should create delegation and return delegation data', async () => {
    setupSelectLimit([makeRole()]);
    setupInsert([makeDelegation()]);
    const result = await service.delegateRole('user-1', 'role-1', null, TENANT_CTX);
