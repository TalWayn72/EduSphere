  ...
  async addGoal(
    ...
    return withTenantContext(this.db, this.ctx(userId, tenantId), async (tx) => {
      const [row] = await tx
        ...
        .values({ userId, tenantId, targetConceptName, targetLevel: targetLevel ?? null })
        .returning();
      this.logger.debug({ userId, tenantId, targetConceptName }, 'competency goal added');
      return this.mapGoal(row!);
    });
  }

  async removeGoal(goalId: string, userId: string, tenantId: string): Promise<boolean> {
    return withTenantContext(this.db, this.ctx(userId, tenantId), async (tx) => {
