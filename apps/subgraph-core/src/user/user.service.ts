...
import { createDatabaseConnection, schema, eq, withTenantContext, closeAllPools } from "@edusphere/db";
...
export class UserService implements OnModuleDestroy {
  ...
  private toTenantContext(authContext: AuthContext): TenantContext {
    return { tenantId: authContext.tenantId || "", userId: authContext.userId, userRole: authContext.roles[0] || "STUDENT" };
  }
  ...
  async findById(id: string, authContext?: AuthContext) {
    if (authContext && authContext.tenantId) {
      ...
      return withTenantContext(this.db, tenantCtx, async (tx) => {
        const [user] = await tx.select().from(schema.users).where(eq(schema.users.id, id)).limit(1);
  ...
  async create(
    input: { email: string; firstName: string; lastName: string; role: string; tenantId: string },
    authContext: AuthContext,
  ) {
    const tenantCtx = this.toTenantContext(authContext);
    return withTenantContext(this.db, tenantCtx, async (tx) => {
      const displayName = [input.firstName || "", input.lastName || ""].join(" ").trim();
      const insertValues = {
        tenant_id: input.tenantId || authContext.tenantId || "",
        email: input.email, first_name: input.firstName || "",
        last_name: input.lastName || "", display_name: displayName,
        ...(input.role && { role: input.role as UserRole }),
      };
      const [user] = await tx.insert(schema.users).values(insertValues).returning();
      return this.mapUser(user);
    });
  }

  async update(id: string, input: { firstName?: string; lastName?: string; role?: string }, authContext: AuthContext) {
    const tenantCtx = this.toTenantContext(authContext);
    return withTenantContext(this.db, tenantCtx, async (tx) => {
      ...
      if (input.firstName || input.lastName) {
        updateData.display_name = [input.firstName||"", input.lastName||""].join(" ").trim();
      }
      if (input.role) updateData.role = input.role as UserRole;
      const [user] = await tx.update(schema.users).set(updateData).where(eq(schema.users.id, id)).returning();
  ...
  async deactivateUser(id: string, authContext: AuthContext): Promise<boolean> {
    const tenantCtx = this.toTenantContext(authContext);
    return withTenantContext(this.db, tenantCtx, async (tx) => {
      await tx.update(schema.users).set({ updated_at: new Date() }).where(eq(schema.users.id, id));
      this.logger.log({ userId: id, tenantId: tenantCtx.tenantId }, "User deactivated");
      return true;
    });
  }
  ...
  async resetUserPassword(userId: string, authContext: AuthContext): Promise<boolean> {
    ...
    this.logger.log({ userId, tenantId: authContext.tenantId }, "Password reset requested");
    return true;
  }

  async bulkImportUsers(
    csvData: string, authContext: AuthContext,
  ): Promise<{ created: number; updated: number; failed: number; errors: string[] }> {
    const tenantCtx = this.toTenantContext(authContext);
    const lines = csvData.trim().split("\n");
    const headers = lines[0]?.split(",").map((h) => h.trim()) ?? [];
    let created = 0; const updated = 0; let failed = 0; const errors: string[] = [];
    for (let i = 1; i < lines.length; i++) {
      const rawValues = lines[i]?.split(",").map((v) => v.trim()) ?? [];
      const row: Record<string, string> = {};
      headers.forEach((h, idx) => { row[h] = rawValues[idx] ?? ""; });
      if (!row["email"]) { errors.push("Row " + String(i) + ": missing email"); failed++; continue; }
      try {
        await this.create({
          email: row["email"],
        ...
        errors.push("Row " + String(i) + ": " + String(err)); failed++;
      }
    }
    this.logger.log({ created, updated, failed, tenantId: tenantCtx.tenantId }, "Bulk import completed");
  ...
  async adminUsers(
    ...
    return withTenantContext(this.db, tenantCtx, async (tx) => {
      const rows = await (opts.role
        ? tx.select().from(schema.users).where(eq(schema.users.role, opts.role as UserRole)).limit(opts.limit).offset(opts.offset)
        : tx.select().from(schema.users).limit(opts.limit).offset(opts.offset));
      const countRows = await tx.select({ id: schema.users.id }).from(schema.users);
      return {
        users: rows.map((u) => this.mapUser(u)).filter((u): u is NonNullable<MappedUser> => u !== null),
