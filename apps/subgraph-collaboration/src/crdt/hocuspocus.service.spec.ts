...
vi.mock('@edusphere/auth', () => ({
  ...
  JWTValidator: vi.fn().mockImplementation(function () {
    return { validate: mocks.mockValidate };
  }),
}));
...
type ConfiguredServer = {
  onAuthenticate: (data: OnAuthData) => Promise<unknown>;
};
...
describe('HocuspocusService', () => {
  ...
  describe('onModuleInit()', () => {
    ...
    it('uses HOCUSPOCUS_PORT env var when set', () => {
      ...
      service.onModuleInit();
      expect(
        (mocks.mockConfigure.mock.calls[0][0] as { port: number }).port
      ).toBe(9999);
  ...
  describe('onAuthenticate hook', () => {
    function getHook(): ConfiguredServer['onAuthenticate'] {
      service.onModuleInit();
      return (mocks.mockConfigure.mock.calls[0][0] as ConfiguredServer)
        .onAuthenticate;
    }

    it('validates JWT token from query param', async () => {
      mocks.mockValidate.mockResolvedValue({
        userId: 'user-1',
        tenantId: 'tenant-1',
      });
      const hook = getHook();
      await hook({
        requestParameters: new URLSearchParams('token=valid-jwt'),
        token: undefined,
        documentName: 'doc-1',
        context: {},
      });
    ...
    it('throws Unauthorized when JWT is invalid/expired', async () => {
      ...
      await expect(
        hook({
          requestParameters: new URLSearchParams('token=bad'),
          token: undefined,
          documentName: 'doc-1',
          context: {},
        })
    ...
    it('document name is namespaced — passed through correctly', async () => {
      ...
      await hook({
        requestParameters: new URLSearchParams('token=t'),
        token: undefined,
        documentName: 'tenant-123/doc-456',
        context: ctx,
      });
