...
const { _mockPublish, mockNatsClient } = vi.hoisted(() => {
  const _mockPublish = vi.fn().mockResolvedValue(undefined);
  const mockNatsClient = { publish: _mockPublish, close: vi.fn() };
  return { _mockPublish, mockNatsClient };
});
