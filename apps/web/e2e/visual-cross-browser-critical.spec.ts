...
test.describe('Visual Cross-Browser -- Critical Pages @visual @xbrowser', () => {
  ...
  for (const { name, path } of CRITICAL_PAGES) {
    ...
    test(`${name} -- header`, async ({ page }) => {
      ...
      await page.waitForTimeout(500);
      const header = page
        .locator('header, [data-testid="page-header"], h1')
        .first();
      const headerOpts = {
      ...
      if (await header.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(header).toHaveScreenshot(
          `xbrowser-${name}-header.png`,
          headerOpts
        );
      } else {
        await expect(page).toHaveScreenshot(`xbrowser-${name}-header.png`, {
          ...headerOpts,
          fullPage: true,
        });
      }
    ...
    test(`${name} -- main content area`, async ({ page }) => {
      ...
      if (await main.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(main).toHaveScreenshot(
          `xbrowser-${name}-main.png`,
          mainOpts
        );
      } else {
        await expect(page).toHaveScreenshot(`xbrowser-${name}-main.png`, {
          ...mainOpts,
          fullPage: true,
        });
      }
    ...
    test(`${name} -- sidebar or nav`, async ({ page }) => {
      ...
      await page.waitForTimeout(500);
      const sidebar = page
        .locator('aside, [role="complementary"], nav, .sidebar')
        .first();
      const sidebarOpts = {
        animations: 'disabled' as const,
        maxDiffPixelRatio: 0.01,
      ...
      if (await sidebar.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(sidebar).toHaveScreenshot(
          `xbrowser-${name}-sidebar.png`,
          sidebarOpts
        );
      } else {
        await expect(page).toHaveScreenshot(`xbrowser-${name}-sidebar.png`, {
          ...sidebarOpts,
          fullPage: true,
        });
      }
    ...
    test(`${name} -- footer area`, async ({ page }) => {
      ...
      await page.waitForTimeout(500);
      const footer = page
        .locator('footer, [data-testid="footer"], [role="contentinfo"]')
        .first();
      const footerOpts = {
        animations: 'disabled' as const,
        maxDiffPixelRatio: 0.01,
      ...
      if (await footer.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(footer).toHaveScreenshot(
          `xbrowser-${name}-footer.png`,
          footerOpts
        );
      } else {
        await expect(page).toHaveScreenshot(`xbrowser-${name}-footer.png`, {
          ...footerOpts,
          fullPage: true,
        });
      }
