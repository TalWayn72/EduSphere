...
test('SA-01 Content Viewer (Default landing)', async ({ page }) => {
  ...
  await page.waitForTimeout(2000);
  await page.screenshot({ path: `${SCREENSHOTS}/sa-01-content-viewer.png`, fullPage: true });
  console.log('URL:', page.url());
  const navItems = await page.locator('nav a, nav button').allInnerTexts();
  console.log('Navigation items:', navItems);
  const h1 = await page.locator('h1, h2').first().innerText().catch(() => 'no heading');
...
test('SA-11 User Menu - Check admin options', async ({ page }) => {
  ...
  const userMenu = page.locator('[aria-label*="user" i], [aria-label*="account" i], button:has-text("SA"), .avatar, .user-menu').first();
  ...
  if (menuVisible) {
    ...
    await page.screenshot({ path: `${SCREENSHOTS}/sa-11-user-menu.png`, fullPage: false });
    const menuText = await page.locator('body').innerText();
    console.log('After menu click:', menuText.substring(0, 500));
  } else {
    await page.screenshot({ path: `${SCREENSHOTS}/sa-11-dashboard-no-menu.png`, fullPage: false });
