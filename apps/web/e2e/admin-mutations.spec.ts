...
test.describe('admin-mutations — Admin Dashboard', () => {
  ...
  test('admin dashboard loads stats without raw GraphQL errors', async ({
    page,
  }) => {
  ...
  test('admin dashboard shows stat cards with numbers', async ({ page }) => {
    ...
    await expect(page.getByText('1247').or(page.getByText('1,247')))
      .toBeVisible({
        timeout: 10_000,
      })
      .catch(() => {
  ...
  test('admin dashboard loading state does not show raw errors', async ({
    page,
  }) => {
...
test.describe('admin-mutations — User Management', () => {
  ...
  test('user management page loads users list', async ({ page }) => {
    ...
    await expect(page.getByText(/alice@example|Alice Smith/i))
  ...
  test('user management page shows no raw GraphQL errors on fetch failure', async ({
    page,
  }) => {
    await routeGraphQL(page, (opName) => {
      if (opName === 'ListUsers' || opName === 'AdminUsers') {
        return JSON.stringify({
          ...
          errors: [
            {
              message:
                'relation "users" does not exist — table missing in schema',
    ...
    await expect(
      page.getByText('relation "users" does not exist')
...
test.describe('admin-mutations — Announcements', () => {
  ...
  test('createAnnouncement success shows confirmation, not raw GraphQL', async ({
    page,
  }) => {
    ...
    const createBtn = page
      .getByRole('button', { name: /create|add|new announcement/i })
      .first();
    ...
    await expect(page).toHaveScreenshot(
      'admin-announcements-after-create.png',
      {
        maxDiffPixelRatio: 0.05,
      }
    );
  ...
  test('createAnnouncement error shows friendly message (not raw server error)', async ({
    page,
  }) => {
    await routeGraphQL(page, (opName) => {
      if (opName === 'ListAnnouncements' || opName === 'GetAnnouncements') {
      ...
      if (opName === 'CreateAnnouncement') {
        return JSON.stringify({
          ...
          errors: [
            {
              message:
                'null value in column "tenant_id" of relation "announcements" violates not-null constraint',
...
test.describe('admin-mutations — At-Risk Dashboard', () => {
  test('at-risk dashboard page loads without raw errors', async ({ page }) => {
    await routeGraphQL(page, (opName) => {
      if (
        opName === 'AtRiskUsers' ||
        opName === 'GetAtRiskLearners' ||
        opName === 'OrgAtRiskLearners'
      ) {
        return JSON.stringify({
          data: {
            orgAtRiskLearners: [
