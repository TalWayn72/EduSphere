...
test.describe('B2B Landing Page — Hero Section', () => {
  ...
  test('hero headline contains "AI-Native LMS"', async ({ page }) => {
    ...
    await expect(
      hero.getByRole('heading', { name: /AI-Native LMS/i })
    ).toBeVisible({ timeout: 10_000 });
  });

  test('hero badge mentions Canvas replacement', async ({ page }) => {
    const hero = page.locator('[data-testid="hero-section"]');
    await expect(hero.getByText(/Canvas/i).first()).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('pricing anchor "$12,000/year" is visible in hero', async ({ page }) => {
    const hero = page.locator('[data-testid="hero-section"]');
    await expect(hero.getByText(/\$12,000\/year/i)).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('hero has "Request Demo" CTA link', async ({ page }) => {
    const hero = page.locator('[data-testid="hero-section"]');
    await expect(hero.getByRole('link', { name: /Request Demo/i })).toBeVisible(
      { timeout: 10_000 }
    );
  });
  ...
  test('hero has "Start 90-Day Pilot" CTA link', async ({ page }) => {
    const hero = page.locator('[data-testid="hero-section"]');
    await expect(
      hero.getByRole('link', { name: /Start 90-Day Pilot/i })
    ).toBeVisible({ timeout: 10_000 });
  });
...
test.describe('B2B Landing Page — Trust Bar', () => {
  ...
  test('trust bar section is visible', async ({ page }) => {
    await expect(page.locator('[data-testid="trust-bar"]')).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('WCAG 2.2 AA compliance badge is present in trust bar', async ({
    page,
  }) => {
    const bar = page.locator('[data-testid="trust-bar"]');
...
test.describe('B2B Landing Page — Compliance Section', () => {
  ...
  test('"Air-Gapped Ready" compliance card is visible', async ({ page }) => {
    const section = page.locator('[data-testid="compliance-badges-section"]');
    await expect(section.getByText('Air-Gapped Ready')).toBeVisible({
      timeout: 10_000,
    });
  });
...
test.describe('B2B Landing Page — VS Competitors', () => {
  ...
  test('table has Canvas, D2L, Blackboard, Docebo column headers', async ({
    page,
  }) => {
    ...
    await expect(section.getByText('Blackboard')).toBeVisible({
      timeout: 10_000,
    });
  ...
  test('EduSphere column is highlighted', async ({ page }) => {
    const section = page.locator('[data-testid="vs-competitors-section"]');
    await expect(section.getByText('EduSphere')).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('Visual Anchoring Sidebar row shows EduSphere winning', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="vs-competitors-section"]');
    const table = section.locator('table');
    const anchorRow = table.getByRole('row', {
      name: /Visual Anchoring Sidebar/i,
    });
...
test.describe('B2B Landing Page — ROI Calculator', () => {
  ...
  test('ROI calculator section is visible', async ({ page }) => {
    await expect(
      page.locator('[data-testid="roi-calculator-section"]')
    ).toBeVisible({ timeout: 10_000 });
  });

  test('ROI calculator heading "Calculate Your ROI" is present', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="roi-calculator-section"]');
    await expect(
      section.getByRole('heading', { name: /Calculate Your ROI/i })
    ).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('default state shows non-NaN ROI percentage', async ({ page }) => {
    ...
    const roiText = await section
      .locator('text=/Net ROI/i')
      .first()
      .locator('..')
      .textContent();
  ...
  test('default state shows dollar value saved (non-zero)', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="roi-calculator-section"]');
    await section.scrollIntoViewIfNeeded();
    const savedEl = section
      .getByText('Dollar value saved')
      .locator('..')
      .locator('..')
      .locator('p')
      .last();
  ...
  test('EduSphere annual cost displays as formatted currency', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="roi-calculator-section"]');
    await section.scrollIntoViewIfNeeded();
    // With 1000 students → tier $28,000
    const costEl = section
      .getByText('EduSphere annual cost')
      .locator('..')
      .locator('..')
      .locator('p')
      .last();
  ...
  test('"Get Your Custom ROI Report" CTA links to pilot form', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="roi-calculator-section"]');
    const cta = section.getByRole('link', {
      name: /Get Your Custom ROI Report/i,
    });
...
test.describe('B2B Landing Page — Pricing', () => {
  ...
  test('"White-label INCLUDED" badge appears on all paid tiers', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pricing-section"]');
  ...
  test('YAU explanation tooltip trigger is present', async ({ page }) => {
    const section = page.locator('[data-testid="pricing-section"]');
    await expect(section.getByText('What is YAU?')).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('NO consumer pricing tiers (Free, Pro, Basic) visible', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pricing-section"]');
    const text = (await section.textContent()) ?? '';
  ...
  test('pricing FAQs section is present', async ({ page }) => {
    const section = page.locator('[data-testid="pricing-section"]');
    await expect(section.getByText('Pricing FAQs')).toBeVisible({
      timeout: 10_000,
    });
  });
...
test.describe('B2B Landing Page — Pilot Form Validation', () => {
    ...
    await page.route('**/graphql', async (route) => {
      ...
      if (body?.operationName === 'SubmitPilotRequest') {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            errors: [{ message: 'Should not reach server' }],
          }),
        });
        return;
      }
  ...
  test('pilot CTA section is visible', async ({ page }) => {
    await expect(page.locator('[data-testid="pilot-cta-section"]')).toBeVisible(
      { timeout: 10_000 }
    );
  });
  ...
  test('submitting empty form shows validation errors on required fields', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();

    const submitBtn = section.getByRole('button', {
      name: /Apply for Free Pilot/i,
    });
  ...
  test('orgName field shows validation error when too short', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await expect(
      section.getByText(/Organization name is required/i)
    ).toBeVisible({ timeout: 3_000 });
  });

  test('email field shows validation error for invalid email', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await expect(section.getByText(/Valid email required/i)).toBeVisible({
      timeout: 3_000,
    });
  });

  test('use case field shows validation error when too short', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await expect(section.getByText(/min 10 chars/i)).toBeVisible({
      timeout: 3_000,
    });
  });
});
...
test.describe('B2B Landing Page — Pilot Form Success Flow', () => {
  ...
  test('successfully submits pilot form and shows confirmation', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await section
      .locator('#useCase')
      .fill(
        'We want to modernize our LMS and replace Canvas with AI-native capabilities for 2,500 students.'
      );
    ...
    await expect(section.getByText(/Application received/i)).toBeVisible({
      timeout: 10_000,
    });
    // Form should no longer be visible after submission
    await expect(
      section.locator('[aria-label="Pilot application form"]')
    ).not.toBeVisible({ timeout: 5_000 });
  });

  test('success state has role="status" and aria-live="polite"', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await section
      .locator('#useCase')
      .fill('Replace Blackboard for our nursing department with AI tutoring.');
  ...
  test('success state does NOT show raw GraphQL response', async ({ page }) => {
    ...
    await section
      .locator('#useCase')
      .fill('Improve engagement for our online computer science courses.');
...
test.describe('B2B Landing Page — Pilot Form Error Handling', () => {
  ...
  test('shows user-friendly error when GraphQL submission fails', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await section
      .locator('#useCase')
      .fill('Testing error handling for the pilot submission form.');
  ...
  test('raw GraphQL error message NOT visible to user after failed submission', async ({
    page,
  }) => {
    const section = page.locator('[data-testid="pilot-cta-section"]');
    await section.scrollIntoViewIfNeeded();
    ...
    await section
      .locator('#useCase')
      .fill('Testing that we never show raw GraphQL errors to end users.');
...
test.describe('B2B Landing Page — Navigation', () => {
  ...
  test('nav has Features, Pricing, Compliance, Pilot anchor links', async ({
    page,
  }) => {
    const nav = page.locator('[data-testid="landing-nav"]');
    await expect(
    ...
    await expect(
      nav.getByRole('link', { name: /Compliance/i }).first()
    ).toBeVisible();
    await expect(
      nav.getByRole('link', { name: /Pilot/i }).first()
    ).toBeVisible();
  });
