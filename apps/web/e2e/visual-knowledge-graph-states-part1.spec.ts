...
async function mockKnowledgeGraph(page: Page): Promise<void> {
  await page.route(`${GRAPHQL_URL}`, async (route) => {
    ...
    if (op.includes('TopicCluster') || query.includes('topicCluster')) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { topicClusters: TOPIC_CLUSTERS } }) });
      return;
    }
    ...
    if (op.includes('SearchGraph') || query.includes('searchGraph')) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { searchGraph: { results: GRAPH_NODES.slice(0, 4), totalCount: 4 } } }) });
      return;
    }
...
test.describe('Visual Regression — Knowledge Graph States Part 1 @visual-kg', () => {
  ...
  test('knowledge graph — graph canvas area', async ({ page }) => {
    ...
    await screenshotElOrPage(canvas, page, 'kg-canvas-area.png', { ...LOOSE_OPTS, fullPage: false });
  });
  ...
  test('knowledge graph — legend / node type indicators', async ({ page }) => {
    ...
    await screenshotElOrPage(legend, page, 'kg-legend.png', { ...LOOSE_OPTS, fullPage: false });
  });
  ...
  test('knowledge graph — zoomed in via controls', async ({ page }) => {
    ...
    if (await zoomIn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await zoomIn.click(); await page.waitForTimeout(500);
      await zoomIn.click(); await page.waitForTimeout(500);
      await zoomIn.click(); await page.waitForTimeout(1000);
    }
  ...
  test('knowledge graph — zoomed out via controls', async ({ page }) => {
    ...
    if (await zoomOut.isVisible({ timeout: 3000 }).catch(() => false)) {
      await zoomOut.click(); await page.waitForTimeout(500);
      await zoomOut.click(); await page.waitForTimeout(1000);
    }
  ...
  test('knowledge graph — filtered by AI cluster', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const filterBtn = page.locator('[data-testid="filter-cluster"], [data-testid="cluster-AI"], button:has-text("AI"), .cluster-filter').first();
    if (await filterBtn.isVisible({ timeout: 3000 }).catch(() => false)) { await filterBtn.click(); await page.waitForTimeout(1500); }
    await expect(page).toHaveScreenshot('kg-filtered-ai-cluster.png', LOOSE_OPTS);
  });

  test('knowledge graph — filtered by node type Concept', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const typeFilter = page.locator('[data-testid="filter-type-concept"], [data-testid="type-filter"] >> text=Concept, button:has-text("Concept")').first();
    if (await typeFilter.isVisible({ timeout: 3000 }).catch(() => false)) { await typeFilter.click(); await page.waitForTimeout(1500); }
    await expect(page).toHaveScreenshot('kg-filtered-concept-type.png', LOOSE_OPTS);
  });

  test('knowledge graph — filtered by Person type', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const typeFilter = page.locator('[data-testid="filter-type-person"], button:has-text("Person"), .type-filter:has-text("Person")').first();
    if (await typeFilter.isVisible({ timeout: 3000 }).catch(() => false)) { await typeFilter.click(); await page.waitForTimeout(1500); }
    await expect(page).toHaveScreenshot('kg-filtered-person-type.png', LOOSE_OPTS);
  });

  test('knowledge graph — node hover state', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const node = page.locator('[data-testid="graph-node"], .graph-node, circle, [data-node-id]').first();
    if (await node.isVisible({ timeout: 3000 }).catch(() => false)) { await node.hover(); await page.waitForTimeout(800); }
  ...
  test('knowledge graph — node selected / detail panel', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const node = page.locator('[data-testid="graph-node"], .graph-node, circle, [data-node-id]').first();
    if (await node.isVisible({ timeout: 3000 }).catch(() => false)) { await node.click(); await page.waitForTimeout(1000); }
  ...
  test('knowledge graph — node detail sidebar', async ({ page }) => {
    ...
    const node = page.locator('[data-testid="graph-node"], .graph-node, circle, [data-node-id]').first();
    if (await node.isVisible({ timeout: 3000 }).catch(() => false)) { await node.click(); await page.waitForTimeout(1000); }
    const sidebar = page.locator('[data-testid="node-detail"], [data-testid="graph-sidebar"], aside, [role="complementary"]').first();
    await screenshotElOrPage(sidebar, page, 'kg-node-detail-sidebar.png', { ...LOOSE_OPTS, fullPage: false });
  });
  ...
  test('knowledge graph — personal mastery scores overlay', async ({ page }) => {
    await goTo(page, '/knowledge-graph/personal');
    const mastery = page.locator('[data-testid="mastery-overlay"], [data-testid="mastery-scores"], .mastery-indicator').first();
    if (await mastery.isVisible({ timeout: 3000 }).catch(() => false)) {
      await expect(mastery).toHaveScreenshot('kg-personal-mastery.png', { ...LOOSE_OPTS, fullPage: false });
    } else {
      await expect(page).toHaveScreenshot('kg-personal-mastery.png', LOOSE_OPTS);
    }
  });

  test('knowledge graph — search panel open', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const searchToggle = page.locator('[data-testid="graph-search"], [aria-label*="Search"], input[type="search"], [placeholder*="Search"]').first();
    if (await searchToggle.isVisible({ timeout: 3000 }).catch(() => false)) { await searchToggle.click(); await page.waitForTimeout(500); }
  ...
  test('knowledge graph — search with results', async ({ page }) => {
    ...
    if (await searchInput.isVisible({ timeout: 3000 }).catch(() => false)) { await searchInput.fill('Machine Learning'); await page.waitForTimeout(1500); }
