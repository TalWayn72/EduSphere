...
test.describe('Features Page — AEO', () => {
  ...
  test('renders page with correct title containing Features and EduSphere', async ({
    page,
  }) => {
    await expect(page).toHaveTitle(/Features.*EduSphere/i);
  });

  test('has meta description mentioning AI tutoring and knowledge graph', async ({
    page,
  }) => {
  ...
  test('has HowTo JSON-LD for AI Tutoring with 4 HowToStep items', async ({
    page,
  }) => {
    ...
    const howToSchemas = schemas.filter(
      (s: any) => s['@type'] === 'HowTo'
    ) as any[];
    ...
    const aiTutorHowTo = howToSchemas.find(
      (s: any) =>
        s.name?.toLowerCase().includes('ai tutoring') ||
        s.name?.toLowerCase().includes('chavruta')
    );
  ...
  test('has SoftwareApplication JSON-LD with EducationalApplication category', async ({
    page,
  }) => {
    const ldScripts = await page
      .locator('script[type="application/ld+json"]')
    ...
    const app = schemas.find(
      (s: any) => s['@type'] === 'SoftwareApplication'
    ) as any;
  ...
  test('SoftwareApplication JSON-LD has offers array for pricing plans', async ({
    page,
  }) => {
    const ldScripts = await page
      .locator('script[type="application/ld+json"]')
  ...
  test('renders main heading "Everything You Need to Learn Smarter"', async ({
    page,
  }) => {
    await expect(
      page.getByRole('heading', {
        name: /Everything You Need to Learn Smarter/i,
      })
  ...
  test('shows all 6 feature section headings', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: 'AI Tutoring (Chavruta)' })
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: 'Knowledge Graph' })
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: 'Gamification' })
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: 'Enterprise Grade' })
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: 'Multi-Language' })
    ).toBeVisible();
    await expect(
  ...
  test('How it works steps are visible for AI Tutoring feature', async ({
    page,
  }) => {
    await expect(page.getByText('Assessment')).toBeVisible();
    await expect(
      page.getByText('Socratic dialogue deepens understanding')
    ).toBeVisible();
  });
  ...
  test('CTA banner Get Started Free link points to /login', async ({
    page,
  }) => {
    // The bottom CTA banner link
    const ctaBannerLink = page
      .getByRole('link', { name: /Get Started Free/i })
      .last();
  ...
  test('page does not expose internal ports or localhost references', async ({
    page,
  }) => {
