...
test.describe('Visual Regression — Scenarios & Roleplay @visual-new', () => {
  ...
  test('scenarios page — grid of scenario cards renders correctly', async ({
    ...
    await page
      .locator(
        'h3.font-semibold, text=No scenarios available yet, .animate-pulse'
      )
      .first()
  ...
  test('roleplay simulator — chat interface layout (via scenario click)', async ({
    ...
    await expect(page).toHaveScreenshot('roleplay-simulator-chat.png', {
      ...LOOSE_OPTS,
      mask: [...dynamicMasks(page), page.locator('.whitespace-pre-wrap')],
    });
  });
  ...
  test('roleplay simulator — header bar renders correctly', async ({
    ...
    if (headerVisible) {
      ...
      await expect(page).toHaveScreenshot(
        ...
        { ...STABLE_OPTS, mask: dynamicMasks(page) }
      );
    }
  });
