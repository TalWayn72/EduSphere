...
test.describe('YouTube Lesson Verification - Student View', () => {
  ...
  async function ensureLoggedIn(page: import('@playwright/test').Page) {
    ...
    if (bodyText.includes('Log In') || bodyText.includes('Sign In')) {
      ...
      try {
        const loginBtn = page.locator('a:has-text("Log In"), button:has-text("Log In"), a:has-text("Sign In"), button:has-text("Sign In")').first();
        const visible = await loginBtn.isVisible({ timeout: 3000 }).catch(() => false);
        if (visible) {
          await loginBtn.click();
          await page.waitForURL(/keycloak|8080/, { timeout: 10000 });
          await page.fill('input[name="username"], #username', 'student@edusphere.ac.il');
  ...
  test('student can see lesson with transcript and summary', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    await page.screenshot({ path: 'docs/screenshots/student-01-after-login.png' });
    ...
    if (previewBodyText.includes('Sign In (Dev Mode)') || previewBodyText.includes('Log In')) {
      console.log('Redirected to login on lesson preview — logging in again...');
      await ensureLoggedIn(page);
      await page.waitForTimeout(2000);
    ...
    await page.screenshot({ path: 'docs/screenshots/student-02-lesson-preview.png', fullPage: true });

    // Read page content
    const pageText = await page.textContent('body') ?? '';
    console.log('LESSON PREVIEW TEXT (first 3000 chars):', pageText.substring(0, 3000));

    // Check for Hebrew content
    const hasHebrew = pageText.includes('ספירת העומר') || pageText.includes('מוחין') || pageText.includes('קבלה') || pageText.includes('שיעור');
    console.log('Has Hebrew lesson content:', hasHebrew);

    // Check for transcript/summary
    const hasTranscript = pageText.includes('transcript') || pageText.includes('Transcript') || pageText.includes('תמלול');
    const hasSummary = pageText.includes('summary') || pageText.includes('Summary') || pageText.includes('סיכום');
    ...
    const ytNocookie = await page.locator('iframe[src*="youtube-nocookie"]').count();
    console.log('YouTube iframe count:', ytIframe + ytNocookie);

    // Check for video player of any kind
    const videoEls = await page.locator('video, iframe[src*="youtube"], iframe[src*="vimeo"]').count();
    console.log('Video/iframe elements:', videoEls);

    // Check for error messages
    const hasError = pageText.includes('Error') || pageText.includes('error') || pageText.includes('שגיאה') || pageText.includes('404') || pageText.includes('Not Found');
    ...
    await page.screenshot({ path: 'docs/screenshots/student-03-lesson-scrolled.png', fullPage: true });
    ...
    await page.screenshot({ path: 'docs/screenshots/student-04-lesson-detail.png', fullPage: true });
    const detailText = await page.textContent('body') ?? '';
    console.log('DETAIL PAGE TEXT (first 2000 chars):', detailText.substring(0, 2000));
