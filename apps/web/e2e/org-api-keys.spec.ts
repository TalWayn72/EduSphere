...
async function mockApiKeyAPIs(
  page: import('@playwright/test').Page
): Promise<void> {
  await routeGraphQL(page, (op) => {
    if (
      op === 'ListApiKeys' ||
      op.toLowerCase().includes('listapi') ||
      op.toLowerCase().includes('apikey')
    ) {
...
test.describe('API Keys — List', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}${API_KEYS_ROUTE}`, {
  ...
  test('displays list of API keys', async ({ page }) => {
    await expect(page.getByText('BI Export Key')).toBeVisible({
      timeout: 10_000,
    });
  ...
  test('shows active/revoked status', async ({ page }) => {
    await expect(
      page.locator('[data-testid="key-status-active"]').first()
    ).toBeVisible({ timeout: 10_000 });
    await expect(
      page.locator('[data-testid="key-status-revoked"]')
    ).toBeVisible();
  });

  test('shows scope badges for each key', async ({ page }) => {
    await expect(page.getByText('bi:export').first()).toBeVisible({
      timeout: 10_000,
    });
  ...
  test('generate key button is present', async ({ page }) => {
    await expect(page.locator('[data-testid="btn-generate-key"]')).toBeVisible({
      timeout: 10_000,
    });
  });
});
...
test.describe('API Keys — Generate', () => {
  test('clicking generate opens form', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="generate-key-modal"]')
    ).toBeVisible({ timeout: 5_000 });
  });

  test('generated key is displayed only once with copy button', async ({
    page,
  }) => {
    test.skip(!RUN_WRITE_TESTS, 'Write tests disabled');
    await mockApiKeyAPIs(page);
    ...
    await page.locator('[data-testid="btn-generate-key"]').click();
    await page
      .locator('[data-testid="generate-key-modal"]')
      .waitFor({ timeout: 5_000 });

    // Fill form
    await page
      .locator('[data-testid="input-key-description"]')
      .fill('New Test Key');
    ...
    await expect(page.locator('[data-testid="raw-key-display"]')).toBeVisible({
      timeout: 10_000,
    });
