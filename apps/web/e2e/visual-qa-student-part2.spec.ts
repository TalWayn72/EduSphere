...
test.afterAll(() => { fs.writeFileSync(path.join(RESULTS_DIR, 'qa-report-part2.json'), JSON.stringify(report, null, 2)); });
...
test('04 — Course List page — courses grid', async ({ page }) => {
  const entry: PageReport = {
    label: '04 — Course List', url: '', screenshot: '',
  ...
  const courseCards = page.locator('[data-testid="course-card"], .course-card, [class*="card"]')
  ...
  if (isEmpty) entry.notes.push('WARNING: Empty state shown — no courses loaded');
...
test('05 — Content Viewer — video player and transcript', async ({ page }) => {
  const entry: PageReport = {
    label: '05 — Content Viewer', url: '', screenshot: '',
    consoleErrors: [], consoleWarnings: [], networkErrors: [],
    loadedSuccessfully: false, notes: [],
  };
  attachErrorListeners(page, entry);
...
test('06 — Content Viewer — create an annotation', async ({ page }) => {
  const entry: PageReport = {
    label: '06 — Create Annotation', url: '', screenshot: '',
    consoleErrors: [], consoleWarnings: [], networkErrors: [],
    loadedSuccessfully: false, notes: [],
  };
  attachErrorListeners(page, entry);
  ...
  const addBtn = page.getByRole('button', { name: /Add/i })
    .or(page.locator('button:has(svg)').filter({ hasText: /Add/i })).first();
  ...
  if (!saveBtnVisible) {
    ...
    if (!annotationVisible) entry.notes.push('WARNING: Annotation text not visible after save');
