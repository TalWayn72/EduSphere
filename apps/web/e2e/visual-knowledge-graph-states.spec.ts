/**
 * Visual Regression Tests — Knowledge Graph States
 *
 * Covers knowledge graph at different states: default view, zoom, filters,
 * hover interactions, search panel, layout modes, and responsive viewports.
 * 20 visual assertions total.
 *
 * Snapshots stored in: apps/web/e2e/visual-knowledge-graph-states.spec.ts-snapshots/
 *
 * Update snapshots:
 *   pnpm --filter @edusphere/web exec playwright test visual-knowledge-graph-states --update-snapshots
 *
 * Run:
...
// ── Deterministic graph data for stable visual snapshots ────────────────────
  ...
  await page.route(`${GRAPHQL_URL}`, async (route) => {
    ...
    if (op.includes('KnowledgeGraph') || query.includes('knowledgeGraph')) {
      await route.fulfill({
        ...
        body: JSON.stringify({
          data: {
            knowledgeGraph: { nodes: GRAPH_NODES, edges: GRAPH_EDGES },
          },
        }),
      });
      return;
    ...
    if (op.includes('TopicCluster') || query.includes('topicCluster')) {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          data: { topicClusters: TOPIC_CLUSTERS },
        }),
      });
      return;
    }
          ...
          data: {
            personalGraph: {
              ...
              masteryScores: [
                ...
                { nodeId: 'n4', score: 0.90 },
                { nodeId: 'n5', score: 0.60 },
              ],
            },
          },
        }),
...
test.describe('Visual Regression — Knowledge Graph States @visual-kg', () => {
  ...
  test('knowledge graph — personal mastery scores overlay', async ({ page }) => {
    ...
    if (await mastery.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      // Fallback: screenshot full page which includes mastery data
  ...
  test('knowledge graph — hierarchical layout mode', async ({ page }) => {
    ...
    const layoutBtn = page.locator(
      '[data-testid="layout-hierarchical"], button:has-text("Hierarchical"), [data-testid="layout-mode"]',
    ).first();
  ...
  test('knowledge graph — radial layout mode', async ({ page }) => {
    await goTo(page, '/knowledge-graph');
    const layoutBtn = page.locator(
      '[data-testid="layout-radial"], button:has-text("Radial"), [data-testid="layout-mode"]',
    ).first();
    if (await layoutBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await layoutBtn.click();
      await page.waitForTimeout(2000);
