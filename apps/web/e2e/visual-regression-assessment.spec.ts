...
test.describe('Visual Regression — Assessment List @visual-assessment', () => {
  ...
  test('assessment list — header section', async ({ page }) => {
    await goTo(page, '/assessments');
    await elementOrPage(page, ['header', '[data-testid="page-header"]', 'h1'], 'assessment-list-header.png');
  });

  test('assessment list — main content', async ({ page }) => {
    await goTo(page, '/assessments');
    await elementOrPage(page, ['main', '[role="main"]', '[data-testid="main-content"]'], 'assessment-list-main.png');
  });

  test('assessment list — assessment cards', async ({ page }) => {
    await goTo(page, '/assessments');
    await elementOrPage(page, ['[data-testid="assessment-grid"]', '.grid', '[role="list"]'], 'assessment-list-cards.png');
  });

  test('assessment list — filter bar', async ({ page }) => {
    await goTo(page, '/assessments');
    await elementOrPage(page, ['[data-testid="filter-bar"]', '[role="search"]', '.filters'], 'assessment-list-filters.png');
  });
});
...
test.describe('Visual Regression — 360 Assessment @visual-assessment', () => {
  ...
  test('360 assessment — header section', async ({ page }) => {
    await goTo(page, '/assessments/360');
    await elementOrPage(page, ['header', '[data-testid="page-header"]', 'h1'], 'assessment-360-header.png');
  });

  test('360 assessment — radar chart', async ({ page }) => {
    await goTo(page, '/assessments/360');
    await elementOrPage(page, ['[data-testid="radar-chart"]', 'canvas', 'svg', '.recharts-wrapper'], 'assessment-360-radar.png', { maxDiffPixelRatio: 0.05, animations: 'disabled' as const });
  });

  test('360 assessment — feedback summary', async ({ page }) => {
    await goTo(page, '/assessments/360');
    await elementOrPage(page, ['[data-testid="feedback-summary"]', '.summary', 'main'], 'assessment-360-feedback.png');
  });

  test('360 assessment — evaluator list', async ({ page }) => {
    await goTo(page, '/assessments/360');
    await elementOrPage(page, ['[data-testid="evaluator-list"]', '[role="list"]', 'table'], 'assessment-360-evaluators.png');
  });
});
...
test.describe('Visual Regression — Assessment Campaigns @visual-assessment', () => {
  ...
  test('campaigns — header section', async ({ page }) => {
    await goTo(page, '/assessments/campaigns');
    await elementOrPage(page, ['header', '[data-testid="page-header"]', 'h1'], 'assessment-campaigns-header.png');
  });

  test('campaigns — campaign table', async ({ page }) => {
    await goTo(page, '/assessments/campaigns');
    await elementOrPage(page, ['table', '[data-testid="campaign-table"]', '[role="table"]', 'main'], 'assessment-campaigns-table.png');
  });

  test('campaigns — status indicators', async ({ page }) => {
    await goTo(page, '/assessments/campaigns');
    await elementOrPage(page, ['[data-testid="campaign-status"]', '.status-badge', 'section'], 'assessment-campaigns-status.png');
  });
});
...
test.describe('Visual Regression — Peer Review @visual-assessment', () => {
  ...
  test('peer review — header section', async ({ page }) => {
    await goTo(page, '/assessments/peer-review');
    await elementOrPage(page, ['header', '[data-testid="page-header"]', 'h1'], 'assessment-peerreview-header.png');
  });

  test('peer review — review queue', async ({ page }) => {
    await goTo(page, '/assessments/peer-review');
    await elementOrPage(page, ['[data-testid="review-queue"]', '[role="list"]', 'main'], 'assessment-peerreview-queue.png');
  });
});
...
test.describe('Visual Regression — Assessment Results @visual-assessment', () => {
  ...
  test('results — header section', async ({ page }) => {
    await goTo(page, '/assessments/results');
    await elementOrPage(page, ['header', '[data-testid="page-header"]', 'h1'], 'assessment-results-header.png');
  });

  test('results — score distribution', async ({ page }) => {
    await goTo(page, '/assessments/results');
    await elementOrPage(page, ['[data-testid="score-distribution"]', 'canvas', 'svg', 'main'], 'assessment-results-distribution.png', { maxDiffPixelRatio: 0.05, animations: 'disabled' as const });
  });

  test('results — results table', async ({ page }) => {
    await goTo(page, '/assessments/results');
    await elementOrPage(page, ['table', '[data-testid="results-table"]', '[role="table"]'], 'assessment-results-table.png');
  });

  test('assessment list — sidebar navigation', async ({ page }) => {
    await goTo(page, '/assessments');
    await elementOrPage(page, ['aside', '[data-testid="sidebar"]', 'nav'], 'assessment-list-sidebar.png');
  });

  test('360 assessment — competency breakdown', async ({ page }) => {
    await goTo(page, '/assessments/360');
    await elementOrPage(page, ['[data-testid="competency-breakdown"]', '.breakdown', 'section'], 'assessment-360-competency.png');
  });

  test('campaigns — create button area', async ({ page }) => {
    await goTo(page, '/assessments/campaigns');
    await elementOrPage(page, ['[data-testid="create-campaign"]', 'button', '.actions'], 'assessment-campaigns-create.png');
  });

  test('peer review — rubric section', async ({ page }) => {
    await goTo(page, '/assessments/peer-review');
    await elementOrPage(page, ['[data-testid="rubric"]', '.rubric', 'section'], 'assessment-peerreview-rubric.png');
  });
});
