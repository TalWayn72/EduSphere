...
async function waitForQuizPageReady(
  ...
  await page.goto(`${BASE}/quiz/${contentId}`, { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2_000);

  if (await page.locator('.bg-primary.h-1\\.5.rounded-full').isVisible()) return 'player';
  if ((await page.getByText('This content item is not a quiz').count()) > 0) return 'not-quiz';
  if ((await page.getByText('Quiz data is missing or invalid').count()) > 0) return 'missing-data';
  if ((await page.getByText(/Failed to load quiz/i).count()) > 0) return 'error';
  return 'loading';
}
...
test.describe('Quiz — Page load', () => {
  test('navigates to /quiz/:contentId without crash', async ({ page }) => {
    await page.goto(`${BASE}/quiz/demo-quiz-id`, { waitUntil: 'domcontentloaded' });
  ...
  test('shows Layout navigation shell when quiz page loads', async ({ page }) => {
    await page.goto(`${BASE}/quiz/demo-quiz-id`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1_500);
  ...
  test('renders one of the expected quiz page states', async ({ page }) => {
    ...
    expect(['player', 'not-quiz', 'missing-data', 'error', 'loading']).toContain(state);
  });
  ...
  test('QuizPlayer shows question counter when content is a valid quiz', async ({
    ...
    await expect(page.getByText(/Question \d+ of \d+/)).toBeVisible({ timeout: 8_000 });
  });

  test('QuizPlayer progress bar is rendered for a valid quiz', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
...
test.describe('Quiz — Multiple choice interaction', () => {
  ...
  test('question text is visible as a paragraph or heading', async ({ page }) => {
  ...
  test('clicking a multiple-choice option marks it selected', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (label && !navLabels.test(label)) {
        ...
        const selectedOption = page.locator('button.ring-2, button[aria-pressed="true"]').first();
  ...
  test('clicking Next advances to question 2 when there are multiple questions', async ({
    ...
    const counterBefore = await page.getByText(/Question \d+ of \d+/).textContent();
...
test.describe('Quiz — Fill-in-the-blank interaction', () => {
  test('fill-blank input accepts typed text', async ({ page }) => {
    await page.goto(`${BASE}/quiz/demo-fill-blank`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2_000);
    ...
    const input = page
      .locator('input[aria-label="Fill in the blank"], input[aria-label="Your answer"]')
      .first();
  ...
  test('fill-blank placeholder text is visible before input', async ({ page }) => {
    await page.goto(`${BASE}/quiz/demo-fill-blank`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2_000);
...
test.describe('Quiz — Likert scale interaction', () => {
  test('Likert radiogroup is rendered with scale labels', async ({ page }) => {
    await page.goto(`${BASE}/quiz/demo-likert`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2_000);
...
test.describe('Quiz — Submit and result view', () => {
  test('Submit Quiz button is visible on the last question', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
  ...
  test('QuizResultView renders score percentage after submit', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
    ...
    const scoreEl = page.locator('.font-mono.font-bold').filter({ hasText: /%/ }).first();
  ...
  test('Try Again button appears in QuizResultView and resets quiz', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
    ...
    await expect(page.getByText(/Question 1 of/)).toBeVisible({ timeout: 5_000 });
  });
});
...
test.describe('Quiz — Keyboard navigation', () => {
  test('Tab moves focus from Previous to Next/Submit button', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
  ...
  test('Enter key on a focused option button triggers click handler', async ({ page }) => {
    const state = await waitForQuizPageReady(page);
    if (state !== 'player') {
      test.skip();
      return;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (label && !navLabels.test(label)) {
        ...
        // No crash expected
        await expect(page.getByText(/Question \d+ of \d+/)).toBeVisible({ timeout: 3_000 });
        break;
      }
    }
  });
...
test.describe('Quiz — Visual regression @visual', () => {
  ...
  test('visual: quiz page fallback state (non-quiz content)', async ({ page }) => {
    await page.emulateMedia({ reducedMotion: 'reduce' });
    // Use a content ID known to return non-quiz state for a stable snapshot
    await page.goto(`${BASE}/quiz/not-a-quiz-content-id`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2_000);
