...
async function mockGamificationAPIs(page: import('@playwright/test').Page): Promise<void> {
  await routeGraphQL(page, (op) => {
    if (op === 'GetGamificationConfig' || op.toLowerCase().includes('gamification')) {
      return JSON.stringify({
        data: {
          gamificationConfig: {
            enabled: true,
            xpRules: { courseCompletion: 100, quizPass: 50, lessonView: 10, dailyLogin: 5 },
            customBadges: [
              { id: 'b1', name: 'Course Champion', iconUrl: '/badges/champion.png', criteria: 'Complete 10 courses' },
              { id: 'b2', name: 'Quiz Master', iconUrl: '/badges/quiz.png', criteria: 'Pass 50 quizzes' },
            ],
          },
          leaderboard: [
    ...
    if (op === 'ToggleGamification' || op.toLowerCase().includes('toggle')) {
      return JSON.stringify({ data: { toggleGamification: { enabled: false } } });
    }
    if (op === 'CreateCustomBadge' || op.toLowerCase().includes('badge')) {
      return JSON.stringify({ data: { createCustomBadge: { id: 'b-new', name: 'New Badge' } } });
    }
...
test.describe('Gamification Config — Structure', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}${GAMIFICATION_ROUTE}`, { waitUntil: 'domcontentloaded' });
    await page.locator('[data-testid="gamification-config"]').waitFor({ timeout: 15_000 });
  });

  test('page loads with gamification toggle', async ({ page }) => {
    await expect(page.locator('[data-testid="toggle-gamification"]')).toBeVisible({ timeout: 10_000 });
  });

  test('custom badges section is visible', async ({ page }) => {
    await expect(page.getByText('Course Champion')).toBeVisible({ timeout: 10_000 });
  ...
  test('XP configuration fields are visible', async ({ page }) => {
    await expect(page.locator('[data-testid="xp-course-completion"]')).toBeVisible({ timeout: 10_000 });
    await expect(page.locator('[data-testid="xp-quiz-pass"]')).toBeVisible();
  });

  test('create badge button is present', async ({ page }) => {
    await expect(page.locator('[data-testid="btn-create-badge"]')).toBeVisible({ timeout: 10_000 });
  });
});
...
test.describe('Gamification Config — Interactions', () => {
  ...
  test('create badge opens modal form', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="badge-creation-modal"]')).toBeVisible({ timeout: 5_000 });
  });
});
...
test.describe('Gamification Config — Visual', () => {
  test('visual regression — gamification page', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}${GAMIFICATION_ROUTE}`, { waitUntil: 'domcontentloaded' });
    await page.locator('[data-testid="gamification-config"]').waitFor({ timeout: 15_000 });
    await expect(page).toHaveScreenshot('gamification-config.png', { maxDiffPixelRatio: 0.05 });
  });
});
