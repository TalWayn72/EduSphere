...
test.describe('AI Course Builder — CTA Panel on /courses/new', () => {
  ...
  test('AI builder CTA panel is visible on step 1', async ({ page }) => {
    await expect(page.locator('[data-testid="ai-builder-cta"]')).toBeVisible({
      timeout: 10_000,
    });
  });

  test('AI builder panel heading mentions "AI Course Builder"', async ({
    page,
  }) => {
    const cta = page.locator('[data-testid="ai-builder-cta"]');
    await expect(cta.getByText(/AI Course Builder/i)).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('AI builder CTA panel describes the 10-minute promise', async ({
    page,
  }) => {
    const cta = page.locator('[data-testid="ai-builder-cta"]');
...
test.describe('AI Course Builder — Modal Open State', () => {
  ...
  test('"Launch AI Builder" button opens the modal dialog', async ({
    page,
  }) => {
    await expect(page.getByRole('dialog')).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('modal title is "AI Course Creator"', async ({ page }) => {
    ...
    await expect(
      dialog.getByRole('heading', { name: /AI Course Creator/i })
    ).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('modal shows Audience Level dropdown', async ({ page }) => {
    const dialog = page.getByRole('dialog');
    await expect(dialog.getByText(/Audience Level/i)).toBeVisible({
      timeout: 10_000,
    });
    await expect(dialog.locator('select')).toBeVisible({ timeout: 10_000 });
  });

  test('audience level dropdown has Beginner, Intermediate, Advanced options', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    ...
    for (const option of ['Beginner', 'Intermediate', 'Advanced']) {
      await expect(
        select.locator(`option[value="${option.toLowerCase()}"]`)
      ).toBeAttached();
    }
  });
  ...
  test('modal shows Estimated Hours input', async ({ page }) => {
    const dialog = page.getByRole('dialog');
    await expect(dialog.getByText(/Estimated Hours/i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test('"Generate Course" button is present but disabled when textarea is empty', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    const generateBtn = dialog.getByRole('button', {
      name: /Generate Course/i,
    });
...
test.describe('AI Course Builder — Generating / Skeleton State', () => {
  ...
  test('typing in textarea enables the Generate button', async ({ page }) => {
    ...
    await dialog
      .getByRole('textbox')
      .fill('Introduction to Machine Learning for high school students');
    const generateBtn = dialog.getByRole('button', {
      name: /Generate Course/i,
    });
  ...
  test('clicking Generate shows "Generating..." spinner state', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await dialog
  ...
  test('audience level select is disabled while generating', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await dialog
...
test.describe('AI Course Builder — Generated Course Preview', () => {
  ...
  test('generated course title appears in the outline view', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
  ...
  test('generated modules appear in the preview — Module 1 visible', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
  ...
  test('generated modules appear in the preview — Module 2 visible', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
  ...
  test('content item titles inside modules are visible', async ({ page }) => {
    const dialog = page.getByRole('dialog');
    await expect(dialog.getByText('Linear Regression')).toBeVisible({
      timeout: 10_000,
    });
    await expect(dialog.getByText('Decision Trees')).toBeVisible({
      timeout: 10_000,
    });
  });

  test('"Create Draft Course" button is visible in the outline view', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
  ...
  test('"Regenerate" button is visible in the outline view', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
  ...
  test('"Regenerate" button hides the outline and shows input form again', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
  ...
  test('visual screenshot of modal in outline/preview state', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
    await expect(
...
test.describe('AI Course Builder — Modal Close & Memory Safety', () => {
  ...
  test('closing modal via Cancel button removes the dialog from DOM', async ({
    page,
  }) => {
    await clickDialogButton(page, 'Cancel');
    await expect(page.getByRole('dialog')).not.toBeVisible({ timeout: 5_000 });
  ...
  test('reopening modal after close resets the textarea to empty', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
  ...
  test('closing modal during generation stops it cleanly (no console errors)', async ({
    page,
  }) => {
  ...
  test('modal state is fully reset: audience level cleared after close and reopen', async ({
    page,
  }) => {
    const dialog = page.getByRole('dialog');
  ...
  test('no raw error strings after closing and reopening modal', async ({
    page,
  }) => {
    await clickDialogButton(page, 'Cancel');
    await expect(page.getByRole('dialog')).not.toBeVisible({ timeout: 5_000 });
...
test.describe('AI Course Builder — Error Handling', () => {
  test('shows error message when generateCourseFromPrompt mutation fails', async ({
    page,
  }) => {
    await routeGraphQL(page, (op) => {
    ...
    await expect(dialog.getByText(/Failed to start generation/i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test('error state shows AlertTriangle icon (accessible error indicator)', async ({
    page,
  }) => {
    ...
    await expect(dialog.locator('.bg-destructive\\/10').first()).toBeVisible({
      timeout: 10_000,
    });
  });

  test('shows consent-specific error when CONSENT_REQUIRED is returned', async ({
    page,
  }) => {
    ...
    await dialog
      .getByRole('textbox')
      .fill('Test topic to trigger consent error');
    await clickDialogButton(page, 'Generate Course');

    // Consent-specific error message should appear
    await expect(dialog.getByText(/consent|privacy/i)).toBeVisible({
      timeout: 10_000,
    });
