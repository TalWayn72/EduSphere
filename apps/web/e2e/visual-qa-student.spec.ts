...
test.afterAll(() => {
  ...
  for (const entry of report) {
    ...
    if (entry.consoleWarnings.length > 0) {
      ...
      entry.consoleWarnings.slice(0, 3).forEach((w) => console.log(`      • ${w}`));
    }
  ...
  const totalErrors = report.reduce((sum, e) => sum + e.consoleErrors.length + e.networkErrors.length, 0);
...
test('01 — Login page renders with EduSphere branding', async ({ page }) => {
  ...
  await page.waitForFunction(
    ...
    entry.notes.push('BUG: App stuck on "Initializing authentication..." after 12s — Keycloak init may be hanging');
  });
  ...
  if (bodyText?.includes('Initializing authentication')) {
    entry.notes.push('STUCK: Login page still showing "Initializing authentication..." — Keycloak init is blocking UI render');
  }
  ...
  if (cspErrors.length > 0) {
    entry.notes.push(`CSP ERROR: Keycloak iframe blocked by CSP frame-ancestors — silent SSO check failed`);
  ...
  if (spinnerVisible) entry.notes.push('BUG: Keycloak init spinner still visible — login page not rendering');
  ...
  if (!headingVisible || !btnVisible) {
    console.log(`[QA BUG-01] Login page not rendered — stuck on "${bodyText?.substring(0, 200)}"`);
  }
  if (spinnerVisible) {
    console.log('[QA BUG-01] CRITICAL: Keycloak init is hanging — spinner never resolves');
  }
});
...
test('02 — Keycloak login flow completes and lands on app', async ({ page }) => {
  ...
  if (!notOnLogin) entry.notes.push('BUG: Still on /login after successful auth — redirect failed');
  ...
  if (devFallback) entry.notes.push(`WARNING: DEV MODE fallback occurred — Keycloak may not have initialized properly`);
...
test('04 — Course List page — courses grid', async ({ page }) => {
  ...
  const courseCards = page.locator('[data-testid="course-card"], .course-card, [class*="card"]').filter({ has: page.getByRole('heading') });
  ...
  if (isEmpty) entry.notes.push('WARNING: Empty state shown — no courses loaded (GraphQL may be failing)');
...
test('05 — Content Viewer — video player and transcript', async ({ page }) => {
  ...
  if (!transcriptVisible) entry.notes.push('MISSING: Transcript panel not visible');

  // Check annotations panel
  const annotationsPanel = page.getByText('Annotations').first();
  const annotationsPanelVisible = await annotationsPanel.isVisible().catch(() => false);
  if (!annotationsPanelVisible) entry.notes.push('MISSING: Annotations panel not visible');
  ...
  if (hasCrashed) entry.notes.push('CRITICAL: Error boundary triggered — component crashed');
...
test('06 — Content Viewer — create an annotation', async ({ page }) => {
  ...
  if (!textareaVisible) {
    entry.notes.push('BUG: Annotation form did not open after clicking Add button');
    entry.url = page.url();
  ...
  await textarea.fill('Visual QA Test Annotation — automated student session test');
  ...
  if (!saveBtnVisible) {
    ...
    const annotationText = page.getByText('Visual QA Test Annotation — automated student session test');
    const annotationVisible = await annotationText.isVisible().catch(() => false);
    if (!annotationVisible) {
      entry.notes.push('WARNING: Annotation text not visible after save — may not have saved, or GraphQL mutation failed');
    } else {
...
test('08 — Knowledge Graph page', async ({ page }) => {
  ...
  if (!graphVisible) {
    entry.notes.push('INFO: No canvas/svg graph element visible — may be text-based fallback or loading');
  }
...
test('09 — Search Page — semantic search', async ({ page }) => {
  ...
  if (inputVisible) {
    ...
    // Check for results
    const results = page.locator('[class*="card"], [class*="result"], [class*="CardContent"]').filter({
    ...
    if (resultCount === 0) {
      ...
      if (emptyVisible) {
        entry.notes.push('INFO: Empty state shown — search returned no results');
      }
    }
  }
...
test('10 — AI Agents Page — Chavruta mode and chat', async ({ page }) => {
  ...
  const headingVisible = await heading.isVisible().catch(() => false);
  if (!headingVisible) entry.notes.push('MISSING: "AI Learning Agents" heading not visible');
  ...
  const greeting = page.getByText(/Chavruta partner/i).or(page.getByText(/שלום/)).or(page.getByText(/debate/i));
  const greetingVisible = await greeting.first().isVisible().catch(() => false);
  if (!greetingVisible) entry.notes.push('WARNING: Chavruta greeting message not visible in chat');
  ...
  if (inputVisible) {
    ...
    const userMsg = page.getByText('What is free will from a Talmudic perspective?');
    const userMsgVisible = await userMsg.isVisible().catch(() => false);
    if (!userMsgVisible) entry.notes.push('BUG: User message not visible after sending');
    ...
    if (bubbleCount < 2) {
      entry.notes.push('WARNING: AI response did not appear after message — streaming may be broken');
    }
  }
...
test('11 — Collaboration Page', async ({ page }) => {
  ...
  const crashed = page.getByText(/something went wrong/i);
  const hasCrashed = await crashed.isVisible().catch(() => false);
  if (hasCrashed) entry.notes.push('CRITICAL: Collaboration page crashed with error boundary');
...
test('12 — Profile Page', async ({ page }) => {
  ...
  if (!emailVisible) entry.notes.push('INFO: Student email not visible on profile page (may require GraphQL me query)');
...
test('13 — User Menu and Logout flow', async ({ page }) => {
  ...
  const userMenuBtn = page
    ...
    .or(page.locator('button').filter({ has: page.locator('[class*="avatar"], [class*="Avatar"]') }))
    .first();
  ...
  if (!userMenuVisible) {
    ...
    if (!altVisible) {
      entry.notes.push('MISSING: UserMenu button not found in header — cannot test logout');
      entry.url = page.url();
  ...
  if (!logoutVisible) {
    ...
    if (!onLoginOrKeycloak) {
      entry.notes.push(`BUG: After logout, user is still on ${finalUrl} instead of /login or Keycloak`);
    } else {
      entry.notes.push('SUCCESS: Logout redirected correctly to login/Keycloak');
    }
...
test('14 — Navigation sidebar — all links reachable', async ({ page }) => {
  ...
  for (const link of expectedLinks) {
    const el = nav.getByRole('link', { name: link.name }).or(page.getByRole('link', { name: link.name })).first();
    const visible = await el.isVisible().catch(() => false);
    if (!visible) {
