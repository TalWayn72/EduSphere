...
test('01 — Login page renders with EduSphere branding', async ({ page }) => {
  ...
  await page
    ...
    .catch(() => {
      entry.notes.push(
        'BUG: App stuck on "Initializing authentication..." after 12s'
      );
    });
  await page.waitForLoadState('domcontentloaded');
  await page.waitForTimeout(500);
  ...
  if (bodyText?.includes('Initializing authentication'))
    entry.notes.push(
      'STUCK: Login page still showing "Initializing authentication..."'
    );
  ...
  if (cspErrors.length > 0) {
    entry.notes.push(
      'CSP ERROR: Keycloak iframe blocked by CSP frame-ancestors'
    );
  ...
  if (spinnerVisible)
    entry.notes.push('BUG: Keycloak init spinner still visible');
  if (!headingVisible)
  ...
  if (!headingVisible || !btnVisible)
    console.warn(`[QA BUG-01] Login page not rendered`);
  if (spinnerVisible)
    console.warn('[QA BUG-01] CRITICAL: Keycloak init is hanging');
});
...
test('02 — Keycloak login flow completes and lands on app', async ({
  ...
  if (!notOnLogin)
    entry.notes.push('BUG: Still on /login after successful auth');
...
test('03 — Dashboard page — stats cards and user profile', async ({ page }) => {
  ...
  const headingVisible = await heading.isVisible().catch(() => false);
  if (!headingVisible)
    entry.notes.push('MISSING: Dashboard main heading not visible');
  const statsToCheck = [
    'Study Time',
    'Concepts Mastered',
    'Active Courses',
