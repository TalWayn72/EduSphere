...
test('01 — Public: Login page', async ({ page }) => {
  ...
  // DEV_MODE shows "Sign In (Dev Mode)" button; LIVE shows "Sign In with Keycloak"
  const hasKeycloakBtn = await page.getByRole('button', { name: /sign in with keycloak/i }).isVisible().catch(() => false);
  const hasDevBtn = await page.locator('[data-testid="dev-login-btn"]').isVisible().catch(() => false);
  const hasBtn = hasKeycloakBtn || hasDevBtn;
  r.notes.push(`Sign-in button: ${hasBtn} (keycloak: ${hasKeycloakBtn}, dev: ${hasDevBtn})`);
  r.ok = hasBtn;
});
