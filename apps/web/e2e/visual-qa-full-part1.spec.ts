...
test.afterAll(() => {
  fs.writeFileSync(
    path.join(RESULTS_DIR, 'visual-qa-full-part1.json'),
...
test('01 — Public: Login page', async ({ page }) => {
  ...
  const hasKeycloakBtn = await page
    .getByRole('button', { name: /sign in with keycloak/i })
    .isVisible()
    .catch(() => false);
  const hasDevBtn = await page
    .locator('[data-testid="dev-login-btn"]')
    .isVisible()
    .catch(() => false);
  const hasBtn = hasKeycloakBtn || hasDevBtn;
  r.notes.push(
    `Sign-in button: ${hasBtn} (keycloak: ${hasKeycloakBtn}, dev: ${hasDevBtn})`
  );
  r.ok = hasBtn;
});
...
test('10 — Auth: Instructor Keycloak login', async ({ page }) => {
  ...
  const loggedIn =
    ...
    r.url.includes(BASE.replace(/https?:\/\//, '').split('/')[0]);
