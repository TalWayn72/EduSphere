...
const USERS = { student: { email: 'student@example.com', password: 'Demo1234' } };
...
test.afterAll(() => { fs.writeFileSync(path.join(RESULTS_DIR, 'visual-qa-full-part4.json'), JSON.stringify(results, null, 2)); });
...
test('70 — Student: Dashboard', async ({ page }) => {
  ...
  const r = await visitRoute(page, '/dashboard', '70 — Student Dashboard', '70-student-dashboard', 4000);
  r.ok = r.ok && !r.url.includes('/login');
});

test('71 — Student: Courses', async ({ page }) => {
  await login(page, USERS.student);
  const r = await visitRoute(page, '/courses', '71 — Student Courses', '71-student-courses', 4000);
  r.ok = r.ok && !r.url.includes('/login');
});

test('72 — Student: Content viewer', async ({ page }) => {
  await login(page, USERS.student);
  const r = await visitRoute(page, `/learn/${SEED.contentId}`, '72 — Student Content Viewer', '72-student-content-viewer', 5000);
...
test('73 — Student: Agents', async ({ page }) => {
  await login(page, USERS.student);
  const r = await visitRoute(page, '/agents', '73 — Student Agents', '73-student-agents', 4000);
  r.ok = r.ok && !r.url.includes('/login');
});

test('74 — Student: My Badges', async ({ page }) => {
  await login(page, USERS.student);
  const r = await visitRoute(page, '/my-badges', '74 — Student My Badges', '74-student-my-badges', 3000);
  r.ok = r.ok && !r.url.includes('/login');
});

test('75 — Student: Search', async ({ page }) => {
  await login(page, USERS.student);
  const r = await visitRoute(page, '/search', '75 — Student Search', '75-student-search', 3000);
  r.ok = r.ok && !r.url.includes('/login');
});
