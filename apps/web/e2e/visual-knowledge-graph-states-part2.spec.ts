...
test.describe('Visual Regression — Knowledge Graph States Part 2 @visual-kg', () => {
  ...
  test('knowledge graph — tablet canvas area', async ({ page }) => {
    ...
    const canvas = page
      ...
      .first();
    await screenshotElOrPage(canvas, page, 'kg-tablet-canvas.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — tablet legend', async ({ page }) => {
    ...
    const legend = page
      ...
      .first();
    await screenshotElOrPage(legend, page, 'kg-tablet-legend.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — mobile canvas area', async ({ page }) => {
    ...
    const canvas = page
      ...
      .first();
    await screenshotElOrPage(canvas, page, 'kg-mobile-canvas.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — wide desktop canvas', async ({ page }) => {
    ...
    const canvas = page
      ...
      .first();
    await screenshotElOrPage(canvas, page, 'kg-wide-canvas.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — reset zoom button', async ({ page }) => {
    ...
    const resetBtn = page
      .locator(
        '[data-testid="reset-zoom"], button:has-text("Reset"), [aria-label*="reset"]'
      )
      .first();
    if (await resetBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      const zoomIn = page
        .locator('[data-testid="zoom-in"], button:has-text("+")')
        .first();
  ...
  test('knowledge graph — fullscreen mode', async ({ page }) => {
    ...
    const fsBtn = page
      .locator(
        '[data-testid="fullscreen"], button[aria-label*="fullscreen"], button:has-text("Fullscreen")'
      )
      .first();
  ...
  test('knowledge graph — minimap/overview', async ({ page }) => {
    ...
    const minimap = page
      .locator(
        '[data-testid="graph-minimap"], [data-testid="graph-overview"], .minimap'
      )
      .first();
    await screenshotElOrPage(minimap, page, 'kg-minimap.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — toolbar controls', async ({ page }) => {
    ...
    const toolbar = page
      .locator(
        '[data-testid="graph-toolbar"], [role="toolbar"], .graph-controls'
      )
      .first();
    await screenshotElOrPage(toolbar, page, 'kg-toolbar-controls.png', {
      ...LOOSE_OPTS,
      fullPage: false,
    });
  });
  ...
  test('knowledge graph — header section', async ({ page }) => {
    ...
    if (await header.isVisible({ timeout: 3000 }).catch(() => false)) {
      await expect(header).toHaveScreenshot('kg-header.png', {
        ...LOOSE_OPTS,
        fullPage: false,
      });
    }
  ...
  test('knowledge graph — filtered by Source type', async ({ page }) => {
    ...
    const typeFilter = page
      .locator(
        '[data-testid="filter-type-source"], button:has-text("Source"), .type-filter:has-text("Source")'
      )
      .first();
  ...
  test('knowledge graph — filtered by Term type', async ({ page }) => {
    ...
    const typeFilter = page
      .locator(
        '[data-testid="filter-type-term"], button:has-text("Term"), .type-filter:has-text("Term")'
      )
      .first();
    if (await typeFilter.isVisible({ timeout: 3000 }).catch(() => false)) {
      await typeFilter.click();
