...
} from './courses-discovery.queries';
...
describe('courses-discovery.queries', () => {
  it('exports COURSES_DISCOVERY_QUERY as a query DocumentNode', () => {
    ...
    expect(COURSES_DISCOVERY_QUERY.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = COURSES_DISCOVERY_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports SEARCH_COURSES_DISCOVERY_QUERY as a query DocumentNode', () => {
    ...
    expect(
      SEARCH_COURSES_DISCOVERY_QUERY.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = SEARCH_COURSES_DISCOVERY_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
