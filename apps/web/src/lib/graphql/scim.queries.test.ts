...
describe('scim.queries', () => {
  it('exports SCIM_TOKENS_QUERY as a query DocumentNode', () => {
    ...
    const def = SCIM_TOKENS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports SCIM_SYNC_LOG_QUERY as a query DocumentNode', () => {
    ...
    const def = SCIM_SYNC_LOG_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports GENERATE_SCIM_TOKEN_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      GENERATE_SCIM_TOKEN_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = GENERATE_SCIM_TOKEN_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports REVOKE_SCIM_TOKEN_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REVOKE_SCIM_TOKEN_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REVOKE_SCIM_TOKEN_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
