...
import {
  INSTRUCTOR_ANALYTICS_OVERVIEW_QUERY,
  INSTRUCTOR_AI_USAGE_QUERY,
...
describe('analytics.queries', () => {
  it('exports INSTRUCTOR_ANALYTICS_OVERVIEW_QUERY as a query DocumentNode', () => {
    ...
    expect(
      INSTRUCTOR_ANALYTICS_OVERVIEW_QUERY.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = INSTRUCTOR_ANALYTICS_OVERVIEW_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports INSTRUCTOR_AI_USAGE_QUERY as a query DocumentNode', () => {
    ...
    expect(INSTRUCTOR_AI_USAGE_QUERY.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = INSTRUCTOR_AI_USAGE_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
