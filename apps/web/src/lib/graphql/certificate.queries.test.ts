...
describe('certificate.queries', () => {
  it('exports MY_CERTIFICATES_QUERY as a query DocumentNode', () => {
    ...
    const def = MY_CERTIFICATES_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports CERTIFICATE_DOWNLOAD_URL_QUERY as a query DocumentNode', () => {
    ...
    expect(
      CERTIFICATE_DOWNLOAD_URL_QUERY.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = CERTIFICATE_DOWNLOAD_URL_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
