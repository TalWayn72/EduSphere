...
import {
  TENANT_SOCIAL_LINKS_QUERY,
  UPDATE_TENANT_SOCIAL_LINKS_MUTATION,
} from './social-links.queries';
...
describe('social-links.queries', () => {
  it('exports TENANT_SOCIAL_LINKS_QUERY as a query DocumentNode', () => {
    ...
    expect(TENANT_SOCIAL_LINKS_QUERY.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = TENANT_SOCIAL_LINKS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports UPDATE_TENANT_SOCIAL_LINKS_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      UPDATE_TENANT_SOCIAL_LINKS_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = UPDATE_TENANT_SOCIAL_LINKS_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
