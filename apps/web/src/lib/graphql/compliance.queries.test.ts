...
describe('compliance.queries', () => {
  it('exports COMPLIANCE_COURSES_QUERY as a query DocumentNode', () => {
    ...
    expect(COMPLIANCE_COURSES_QUERY.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = COMPLIANCE_COURSES_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports GENERATE_COMPLIANCE_REPORT_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      GENERATE_COMPLIANCE_REPORT_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = GENERATE_COMPLIANCE_REPORT_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports UPDATE_COURSE_COMPLIANCE_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      UPDATE_COURSE_COMPLIANCE_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = UPDATE_COURSE_COMPLIANCE_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
