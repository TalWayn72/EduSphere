...
describe('manager.queries', () => {
  it('exports MY_TEAM_OVERVIEW_QUERY as a query DocumentNode', () => {
    ...
    const def = MY_TEAM_OVERVIEW_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports ADD_TEAM_MEMBER_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(ADD_TEAM_MEMBER_MUTATION.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = ADD_TEAM_MEMBER_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports REMOVE_TEAM_MEMBER_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REMOVE_TEAM_MEMBER_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REMOVE_TEAM_MEMBER_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
