...
describe('bi-export.queries', () => {
  it('exports BI_API_TOKENS_QUERY as a query DocumentNode', () => {
    ...
    const def = BI_API_TOKENS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports GENERATE_BI_API_KEY_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      GENERATE_BI_API_KEY_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = GENERATE_BI_API_KEY_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports REVOKE_BI_API_KEY_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REVOKE_BI_API_KEY_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REVOKE_BI_API_KEY_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
