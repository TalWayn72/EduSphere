...
describe('crm.queries', () => {
  it('exports CRM_CONNECTION_QUERY as a query DocumentNode', () => {
    ...
    const def = CRM_CONNECTION_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports CRM_SYNC_LOG_QUERY as a query DocumentNode', () => {
    ...
    const def = CRM_SYNC_LOG_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports DISCONNECT_CRM_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(DISCONNECT_CRM_MUTATION.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = DISCONNECT_CRM_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
