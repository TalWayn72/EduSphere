...
import {
  MY_TENANT_LANGUAGE_SETTINGS_QUERY,
  UPDATE_TENANT_LANGUAGE_SETTINGS_MUTATION,
} from './tenant-language.queries';
...
describe('tenant-language.queries', () => {
  it('exports MY_TENANT_LANGUAGE_SETTINGS_QUERY as a query DocumentNode', () => {
    ...
    expect(
      MY_TENANT_LANGUAGE_SETTINGS_QUERY.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = MY_TENANT_LANGUAGE_SETTINGS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports UPDATE_TENANT_LANGUAGE_SETTINGS_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      UPDATE_TENANT_LANGUAGE_SETTINGS_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = UPDATE_TENANT_LANGUAGE_SETTINGS_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
