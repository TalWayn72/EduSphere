...
describe('xapi.queries', () => {
  it('exports XAPI_TOKENS_QUERY as a query DocumentNode', () => {
    ...
    const def = XAPI_TOKENS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports XAPI_STATEMENTS_QUERY as a query DocumentNode', () => {
    ...
    const def = XAPI_STATEMENTS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports GENERATE_XAPI_TOKEN_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      GENERATE_XAPI_TOKEN_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = GENERATE_XAPI_TOKEN_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports REVOKE_XAPI_TOKEN_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REVOKE_XAPI_TOKEN_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REVOKE_XAPI_TOKEN_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
