...
import {
  CONTENT_TRANSLATION_QUERY,
  REQUEST_CONTENT_TRANSLATION_MUTATION,
...
describe('translation.queries', () => {
  it('exports CONTENT_TRANSLATION_QUERY as a query DocumentNode', () => {
    ...
    expect(CONTENT_TRANSLATION_QUERY.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = CONTENT_TRANSLATION_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports REQUEST_CONTENT_TRANSLATION_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REQUEST_CONTENT_TRANSLATION_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REQUEST_CONTENT_TRANSLATION_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
