...
import {
  REGISTER_WHATSAPP_MUTATION,
  VERIFY_WHATSAPP_MUTATION,
...
describe('whatsapp.queries', () => {
  it('exports REGISTER_WHATSAPP_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      REGISTER_WHATSAPP_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = REGISTER_WHATSAPP_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports VERIFY_WHATSAPP_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(VERIFY_WHATSAPP_MUTATION.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = VERIFY_WHATSAPP_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
