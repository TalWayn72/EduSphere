...
describe('collaboration.queries', () => {
  it('exports DISCUSSIONS_QUERY as a query DocumentNode', () => {
    ...
    const def = DISCUSSIONS_QUERY.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports CREATE_DISCUSSION_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(
      CREATE_DISCUSSION_MUTATION.definitions.length
    ).toBeGreaterThanOrEqual(1);
    const def = CREATE_DISCUSSION_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
  ...
  it('exports LEAVE_DISCUSSION_MUTATION as a mutation DocumentNode', () => {
    ...
    expect(LEAVE_DISCUSSION_MUTATION.definitions.length).toBeGreaterThanOrEqual(
      1
    );
    const def = LEAVE_DISCUSSION_MUTATION.definitions[0] as {
      kind: string;
      operation: string;
      name?: { value: string };
    };
