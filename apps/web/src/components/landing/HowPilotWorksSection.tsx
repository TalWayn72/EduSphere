          ...
          {STEP_DEFS.map(({ icon: Icon, num, key }, i) => (
            <React.Fragment key={num}>
              <div className="flex-1 flex flex-col items-center text-center">
                ...
                <div className="text-xs font-bold text-indigo-600 uppercase tracking-widest mb-1">{t('landing.howPilotWorks.step', { num })}</div>
                <h3 className="font-bold text-slate-900 text-lg mb-2">{t(`landing.howPilotWorks.${key}.title`)}</h3>
                <p className="text-sm text-slate-600 leading-relaxed max-w-xs">{t(`landing.howPilotWorks.${key}.desc`)}</p>
              </div>
