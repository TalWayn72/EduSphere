...
const LEARNERS = [
  { learnerId: 'l1', courseId: 'c1', riskScore: 85, daysSinceLastActivity: 14, progressPercent: 20, riskFactors: [{ key: 'low_activity', description: 'Low activity' }] },
];
...
describe('AtRiskLearnersPanel', () => {
  ...
  it('shows empty state when no learners', () => {
    vi.mocked(urql.useQuery).mockReturnValue([{ data: { atRiskLearners: [] }, fetching: false, error: undefined }, vi.fn()] as never);
    render(<AtRiskLearnersPanel courseId='c1' />);
    expect(screen.getByText(/No at-risk learners/i)).toBeInTheDocument();
  });

  it('renders learner table when data available', () => {
    vi.mocked(urql.useQuery).mockReturnValue([{ data: { atRiskLearners: LEARNERS }, fetching: false, error: undefined }, vi.fn()] as never);
    render(<AtRiskLearnersPanel courseId='c1' />);
