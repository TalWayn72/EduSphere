...
vi.mock('@tiptap/pm/view', () => {
  const decorations: Array<{ from: number; to: number; attrs: Record<string, string> }> = [];
  return {
    Decoration: {
...
describe('createAnnotationDecorationsPlugin', () => {
  ...
  it('creates plugin with correct key', async () => {
    const { createAnnotationDecorationsPlugin, annotationPluginKey } = await import(
      '@/components/annotation/AnnotationDecorationsPlugin'
    );
    const plugin = createAnnotationDecorationsPlugin(() => [], () => null);
  ...
  it('builds empty DecorationSet when no annotations', async () => {
    ...
    const { createAnnotationDecorationsPlugin } = await import(
      '@/components/annotation/AnnotationDecorationsPlugin'
    );
  ...
  it('creates decorations for valid annotations', async () => {
    ...
    const mockDoc = { content: { size: 1000 } };
    const annotations = [
      { id: 'ann-1', layer: AnnotationLayer.PERSONAL, textRange: { from: 10, to: 50 } },
      { id: 'ann-2', layer: AnnotationLayer.SHARED, textRange: { from: 100, to: 150 } },
    ];
    const plugin = createAnnotationDecorationsPlugin(() => annotations, () => null);
    plugin.state?.init?.({} as never, { doc: mockDoc } as never);
  ...
  it('skips annotations where from >= to', async () => {
    ...
    const mockDoc = { content: { size: 1000 } };
    const annotations = [
      { id: 'bad-1', layer: AnnotationLayer.PERSONAL, textRange: { from: 50, to: 10 } },
      { id: 'bad-2', layer: AnnotationLayer.SHARED, textRange: { from: 30, to: 30 } },
    ];
  ...
  it('adds focused class to focused annotation', async () => {
    ...
    const mockDoc = { content: { size: 1000 } };
    const annotations = [
      { id: 'ann-focus', layer: AnnotationLayer.INSTRUCTOR, textRange: { from: 5, to: 20 } },
    ];
    vi.mocked(Decoration.inline).mockClear();
    const plugin = createAnnotationDecorationsPlugin(() => annotations, () => 'ann-focus');
    plugin.state?.init?.({} as never, { doc: mockDoc } as never);
    expect(Decoration.inline).toHaveBeenCalledWith(
      5, 20,
      expect.objectContaining({ class: expect.stringContaining('annotation-highlight--focused') }),
    );
  });
  ...
  it('clamps out-of-bounds positions to document size', async () => {
    ...
    const annotations = [
      { id: 'oob', layer: AnnotationLayer.PERSONAL, textRange: { from: -10, to: 999 } },
    ];
    vi.mocked(Decoration.inline).mockClear();
    const plugin = createAnnotationDecorationsPlugin(() => annotations, () => null);
    plugin.state?.init?.({} as never, { doc: mockDoc } as never);
  ...
  it('creates extension with addProseMirrorPlugins', async () => {
    ...
    const { createAnnotationExtension } = await import(
      '@/components/annotation/AnnotationDecorationsPlugin'
    );
    createAnnotationExtension(() => [], () => null);
