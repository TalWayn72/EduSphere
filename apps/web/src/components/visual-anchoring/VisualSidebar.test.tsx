...
vi.mock('./CrossFadeImage', () => ({
  default: ({ src, alt }: { src: string | null; alt?: string }) => (
    <img data-testid="cross-fade-image" src={src ?? undefined} alt={alt} />
  ),
}));
...
describe('VisualSidebar', () => {
  ...
  it('shows empty state when activeAnchorId is null', () => {
    render(<VisualSidebar anchors={[makeAnchor()]} activeAnchorId={null} />);
  ...
  it('shows "No image assigned" when anchor has no visualAsset', () => {
    ...
    render(<VisualSidebar anchors={[anchor]} activeAnchorId="a1" />);
  ...
  it('shows anchor text in footer when active anchor is set', () => {
    const anchor = makeAnchor({
      id: 'a1',
      anchorText: 'Important section text',
    });
    render(<VisualSidebar anchors={[anchor]} activeAnchorId="a1" />);
  ...
  it('does not render footer when activeAnchorId is null', () => {
    ...
    render(<VisualSidebar anchors={[anchor]} activeAnchorId={null} />);
    expect(screen.queryByText(/Should not appear/)).toBeNull();
  });

  it('renders with border-l class for RTL layout', () => {
    render(<VisualSidebar anchors={[]} activeAnchorId={null} isRTL={true} />);
  ...
  it('renders with border-r class for LTR layout (default)', () => {
    render(<VisualSidebar anchors={[]} activeAnchorId={null} isRTL={false} />);
    const sidebar = screen.getByTestId('visual-sidebar');
