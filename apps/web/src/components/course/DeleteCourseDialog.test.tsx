...
function mockMutation(execute = NOOP_EXECUTE, fetching = false) {
  vi.mocked(urql.useMutation).mockReturnValue([{ fetching }, execute] as never);
}
...
describe('DeleteCourseDialog', () => {
  ...
  it('renders dialog with correct title and description', () => {
    ...
    expect(screen.getAllByText('Delete Course').length).toBeGreaterThanOrEqual(
      1
    );
  ...
  it('published course shows published warning', () => {
    ...
    expect(
      screen.getByText(
        'This course is currently published. Deleting it will remove access for all enrolled students.'
      )
    ).toBeInTheDocument();
  });
  ...
  it('draft course shows draft warning', () => {
    ...
    expect(
      screen.getByText(
        'This course is a draft. Deleting it will remove all modules and content items.'
      )
    ).toBeInTheDocument();
  });
