...
export function useSketchCanvas({
  ...
  const redraw = useCallback(
    (canvas: HTMLCanvasElement, previewPath?: SketchPath) => {
      const ctx = canvas.getContext('2d');
      if (!ctx) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      for (const p of pathsRef.current)
        renderPath(ctx, p, canvas.width, canvas.height);
      if (previewPath)
        renderPath(ctx, previewPath, canvas.width, canvas.height);
    },
    []
  );
  ...
  const continueDraw = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      if (tool === 'freehand' || tool === 'eraser') {
        ...
        redraw(canvas, {
          points: currentFreehandRef.current,
          color,
          width: strokeWidth,
          tool,
        });
        ...
        redraw(canvas, {
          points: [shapeStartRef.current, pos],
          color,
          width: strokeWidth,
          tool,
        });
