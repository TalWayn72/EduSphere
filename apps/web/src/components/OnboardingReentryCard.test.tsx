...
describe('OnboardingReentryCard', () => {
  it('renders when onboardingSkipped is true', () => {
    render(
      <MemoryRouter>
        <OnboardingReentryCard onboardingSkipped={true} />
      </MemoryRouter>
    );
    expect(screen.getByTestId('onboarding-reentry-card')).toBeInTheDocument();
    expect(screen.getByText('Complete Your Profile')).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /complete onboarding/i })
  ...
  it('renders nothing when onboarding was not skipped', () => {
    const { container } = render(
      <MemoryRouter>
        <OnboardingReentryCard onboardingSkipped={false} />
      </MemoryRouter>
    );
  ...
  it('dismiss button hides the card', () => {
    ...
    expect(
      screen.queryByTestId('onboarding-reentry-card')
