...
function ToolbarButton({ active = false, onClick, title, children, disabled }: ToolbarButtonProps) {
  return (
    <Button
...
export function EditorToolbar({ editor, onImageUpload, hasMath = false }: EditorToolbarProps) {
  const insertTable = () =>
    editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run();

  const insertMath = () => editor.chain().focus().insertContent('$$...$$').run();
  ...
  return (
    <div className="flex items-center gap-0.5 px-2 py-1.5 border-b bg-muted/40 flex-wrap">
      ...
      <ToolbarButton active={editor.isActive('table')} onClick={insertTable} title="Insert table">
        <Table className="h-3.5 w-3.5" />
      </ToolbarButton>
      {onImageUpload && (
        <ToolbarButton active={false} onClick={onImageUpload} title="Upload image">
          <ImageIcon className="h-3.5 w-3.5" />
        </ToolbarButton>
      )}
      {hasMath && (
        <ToolbarButton active={false} onClick={insertMath} title="Insert math (LaTeX)">
