...
describe('ShareBadgeDialog', () => {
  it('renders badge info when open', () => {
    render(<ShareBadgeDialog open={true} onOpenChange={vi.fn()} badge={MOCK_BADGE} />);
    expect(screen.getByText('TypeScript Expert')).toBeInTheDocument();
    expect(screen.getByText('Demonstrated advanced TypeScript skills')).toBeInTheDocument();
  ...
  it('renders without image when imageUrl not provided', () => {
    ...
    render(<ShareBadgeDialog open={true} onOpenChange={vi.fn()} badge={badgeNoImg} />);
  ...
  it('does not render content when closed', () => {
    render(<ShareBadgeDialog open={false} onOpenChange={vi.fn()} badge={MOCK_BADGE} />);
