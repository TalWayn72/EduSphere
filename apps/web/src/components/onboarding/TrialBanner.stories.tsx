...
function TrialBannerPreview({ daysRemaining, isTrialing }: { daysRemaining: number; isTrialing: boolean }) {
  if (!isTrialing) return null;
  const urgency = daysRemaining <= 3 ? 'bg-destructive text-destructive-foreground' : 'bg-amber-100 text-amber-900 dark:bg-amber-900 dark:text-amber-100';
  return (
    <div className={`flex items-center justify-between px-4 py-2 text-sm ${urgency}`}>
