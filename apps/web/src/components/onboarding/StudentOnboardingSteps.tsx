...
export function StudentOnboardingSteps({
  currentStep,
  onComplete,
  onSkip,
}: StudentOnboardingStepsProps) {
  ...
  const handleNext = useCallback(async () => {
    await updateStep({
      input: { step, data: { displayName, selectedTopics } },
    });
    if (step < totalSteps) {
      setStep(step + 1);
    } else {
            ...
            {(['Beginner', 'Intermediate', 'Advanced'] as const).map(
              (level) => (
                <Button
                  ...
                  onClick={async () => {
                    await updateStep({
                      input: { step, data: { knowledgeLevel: level } },
                    });
                    setStep(step + 1);
                  }}
                >
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border p-4 bg-indigo-50 dark:bg-indigo-950/20">
            <p className="text-sm text-indigo-700 dark:text-indigo-300">
              Your courses are ready! Click Continue to see them on your
              dashboard.
            </p>
          </div>
        </div>
      ...
      {step === 5 && (
        <div className="space-y-4 text-center">
          ...
          <p className="text-muted-foreground">
            Your learning journey begins now. Explore courses, earn XP, and
            track your progress.
          </p>
        </div>
      )}
