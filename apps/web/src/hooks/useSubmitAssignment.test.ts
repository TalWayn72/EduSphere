...
// ── Import after mocks ─────────────────────────────────────────────────────────
import { useSubmitAssignment, type SubmittedAssignment } from './useSubmitAssignment';
import * as urql from 'urql';

// ── Helpers ───────────────────────────────────────────────────────────────────

function makeAssignment(overrides: Partial<SubmittedAssignment> = {}): SubmittedAssignment {
  return {
    id: 'assign-1',
...
describe('useSubmitAssignment', () => {
  ...
  it('returns loading=false and error=null initially', () => {
    const { result } = renderHook(() => useSubmitAssignment('item-1', 'course-1'));
  ...
  it('calls the mutation with contentItemId, courseId, and textContent', async () => {
    ...
    const { result } = renderHook(() => useSubmitAssignment('item-abc', 'course-xyz'));
  ...
  it('returns null and sets error when the mutation returns an error', async () => {
    const mutationError = { message: 'Assignment submission failed' } as urql.CombinedError;
  ...
  it('correctly passes both contentItemId and courseId to the mutation', async () => {
    const mockExecute = vi.fn().mockResolvedValue({
      data: { submitTextAssignment: makeAssignment({ contentItemId: 'item-99' }) },
      error: undefined,
    });
    vi.mocked(urql.useMutation).mockReturnValue([
      { fetching: false, data: undefined, error: undefined } as unknown as ReturnType<typeof urql.useMutation>[0],
    ...
    const { result } = renderHook(() => useSubmitAssignment('item-99', 'course-88'));
    ...
    expect(mockExecute).toHaveBeenCalledWith(
      expect.objectContaining({ contentItemId: 'item-99', courseId: 'course-88' })
    );
  });
  ...
  it('clears the previous error at the start of each new submit call', async () => {
    const mockExecute = vi.fn()
      ...
      .mockResolvedValueOnce({ data: { submitTextAssignment: makeAssignment() }, error: undefined });
