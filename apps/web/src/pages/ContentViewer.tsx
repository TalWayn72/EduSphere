...
import { ANNOTATIONS_QUERY, CREATE_ANNOTATION_MUTATION } from '@/lib/graphql/annotation.queries';
import { START_AGENT_SESSION_MUTATION, SEND_AGENT_MESSAGE_MUTATION } from '@/lib/graphql/agent.queries';
import { mockVideo, mockTranscript, mockBookmarks, TranscriptSegment } from '@/lib/mock-content-data';
import { getThreadedAnnotations, filterAnnotationsByLayers } from '@/lib/mock-annotations';
import { mockGraphData } from '@/lib/mock-graph-data';
import { Annotation, AnnotationLayer } from '@/types/annotations';
import {
  Play, Pause, Volume2, VolumeX, Maximize,
  FileText, Network, Search,
  BookOpen, Bot, ChevronRight, Clock, Plus, Send,
  Eye, EyeOff
} from 'lucide-react';
...
const LAYER_META: Record<string, { label: string; color: string; bg: string }> = {
  PERSONAL:      { label: 'Personal',    color: 'text-violet-700', bg: 'bg-violet-50 border-violet-200' },
  SHARED:        { label: 'Shared',      color: 'text-blue-700',   bg: 'bg-blue-50   border-blue-200'   },
  INSTRUCTOR:    { label: 'Instructor',  color: 'text-green-700',  bg: 'bg-green-50  border-green-200'  },
  AI_GENERATED:  { label: 'AI',          color: 'text-amber-700',  bg: 'bg-amber-50  border-amber-200'  },
};
...
export function ContentViewer() {
  ...
  const [activeLayers, setActiveLayers] = useState<AnnotationLayer[]>([
    AnnotationLayer.PERSONAL, AnnotationLayer.SHARED,
    AnnotationLayer.INSTRUCTOR, AnnotationLayer.AI_GENERATED,
  ]);
  ...
  const baseAnnotations: Annotation[] = DEV_MODE || annotationsResult.error
  ...
  const annotations: Annotation[] = DEV_MODE
    ? [...filterAnnotationsByLayers(localAnnotations, activeLayers), ...baseAnnotations]
    : baseAnnotations;

  const transcript: TranscriptSegment[] = DEV_MODE ? mockTranscript : (contentResult.data?.contentItem?.transcript?.segments ?? mockTranscript);
  const bookmarks  = mockBookmarks;
  const videoUrl   = DEV_MODE ? mockVideo.url : (contentResult.data?.contentItem?.mediaAsset?.url ?? mockVideo.url);
  const videoTitle = DEV_MODE ? mockVideo.title : (contentResult.data?.contentItem?.title ?? mockVideo.title);
  ...
  const handleAddAnnotation = async () => {
    ...
    if (DEV_MODE) {
      ...
      await createAnnotation({
        input: { assetId: contentId, content: newAnnotation, layer: newLayer, timestampStart: currentTime },
      });
    }
    setNewAnnotation('');
  ...
  const handleSendChat = async () => {
    ...
    if (!DEV_MODE) {
      ...
      if (!sid) {
        const res = await startSession({ input: { templateType: 'CHAVRUTA', contextContentId: contentId } });
      ...
      if (sid) {
        ...
        if (reply) setChatMessages((prev) => [...prev, { id: reply.id, role: 'agent', content: reply.content }]);
      }
    } else {
      ...
      setTimeout(() => {
        ...
        setChatMessages((prev) => [...prev, { id: Date.now().toString(), role: 'agent', content: reply }]);
          ...
          <Card className="flex-shrink-0">
            <CardContent className="p-0">
              <div className="relative bg-black rounded-t-lg" style={{ aspectRatio: '16/9' }}>
                ...
                {duration > 0 && bookmarks.map((bm) => (
                  <div
                    ...
                    style={{ left: `${(bm.timestamp / duration) * 100}%`, backgroundColor: bm.color ?? '#3b82f6' }}
                    title={bm.label}
              ...
              <div className="px-3 py-2 flex items-center gap-3 bg-muted/40 rounded-b-lg">
                ...
                {/* Seek bar */}
                <div className="flex-1 relative h-2 bg-muted rounded-full cursor-pointer" onClick={(e) => {
                  const rect = e.currentTarget.getBoundingClientRect();
                  seekTo(((e.clientX - rect.left) / rect.width) * duration);
                }}>
                  <div className="h-2 bg-primary rounded-full" style={{ width: `${duration ? (currentTime / duration) * 100 : 0}%` }} />
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1">
              {transcript.map((seg, idx) => (
                ...
                >
                  ...
                  <span className={idx === activeSegment ? 'font-medium' : ''}>{seg.text}</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card className="flex-1 overflow-hidden flex flex-col">
            <div className="px-4 py-2 border-b">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setShowAnnotationForm(!showAnnotationForm)}>
              ...
              <div className="flex flex-wrap gap-1">
                {(Object.keys(LAYER_META) as AnnotationLayer[]).map((layer) => (
                  ...
                  >
                    {activeLayers.includes(layer) ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}
            ...
            {showAnnotationForm && (
              <div className="px-4 py-3 border-b bg-muted/30 space-y-2">
                ...
                <div className="flex gap-2 justify-end">
                  <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setShowAnnotationForm(false)}>Cancel</Button>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              {annotations.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-6">No annotations visible. Enable layers above.</p>
              )}
              {annotations.map((ann) => (
                <div
                  key={ann.id}
                  ...
                  onClick={() => ann.contentTimestamp !== undefined && seekTo(ann.contentTimestamp)}
                >
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-semibold ${LAYER_META[ann.layer]?.color ?? ''}`}>
          ...
          <Card className="flex-shrink-0">
            <Tabs defaultValue="graph">
              <TabsList className="w-full rounded-none border-b bg-transparent px-2 h-9">
                <TabsTrigger value="graph" className="text-xs flex-1"><Network className="h-3 w-3 mr-1" />Graph</TabsTrigger>
                <TabsTrigger value="search" className="text-xs flex-1"><Search className="h-3 w-3 mr-1" />Search</TabsTrigger>
              </TabsList>
                  ...
                  {mockGraphData.nodes.slice(0, 4).map((node) => (
                    <div key={node.id}
                      ...
                      <span className={`h-2 w-2 rounded-full flex-shrink-0 ${
                        ...
                        node.type === 'SOURCE' ? 'bg-purple-500' : 'bg-orange-500'
                      }`} />
              ...
              <TabsContent value="search" className="m-0 px-4 py-3 space-y-2">
                ...
                {searchQuery.trim().length > 1 ? (
                  ...
                  <p className="text-xs text-muted-foreground">Search transcripts, annotations &amp; concepts</p>
                )}
              </TabsContent>
            </Tabs>
          </Card>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col overflow-hidden">
          <Card className="flex-1 flex flex-col overflow-hidden">
            <div className="px-4 py-2 border-b flex items-center gap-2 flex-shrink-0">
              ...
              <div className="ml-auto flex gap-1">
                {['CHAVRUTA', 'QUIZ', 'EXPLAIN'].map((mode) => (
                  <span key={mode} className="text-xs px-1.5 py-0.5 rounded bg-muted/60 text-muted-foreground hover:bg-primary/10 hover:text-primary cursor-pointer transition-colors">
                    {mode}
                  </span>
                ))}
              </div>
            ...
            <div className="px-4 py-3 flex gap-2 flex-shrink-0">
              ...
              />
              <Button size="sm" className="h-9 w-9 p-0 flex-shrink-0" onClick={handleSendChat}>
