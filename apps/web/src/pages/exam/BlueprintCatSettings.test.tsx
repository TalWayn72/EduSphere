...
vi.mock('@/components/ui/form', () => ({
  ...
  FormField: ({ render: renderFn }: { render: (props: { field: Record<string, unknown> }) => React.ReactNode }) =>
    <div>{renderFn({ field: { value: '', onChange: vi.fn(), onBlur: vi.fn(), name: '', ref: vi.fn() } })}</div>,
...
describe('BlueprintCatSettings', () => {
  it('renders without crash', () => {
    const { container } = render(
      <BlueprintCatSettings form={mockForm as never} enableCat={false} onEnableCatChange={vi.fn()} />
    );
