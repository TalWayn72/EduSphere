...
export function AdminNotificationAnalyticsPage() {
  ...
  const analytics: Analytics | null =
    data?.notificationDeliveryAnalytics ?? null;
      ...
      {fetching ? (
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard
              label={t('notifications.totalSent')}
              value={analytics.totalSent}
            />
            <StatCard
              label={t('notifications.totalDelivered')}
              value={analytics.totalDelivered}
            />
            <StatCard
              label={t('notifications.totalFailed')}
              value={analytics.totalFailed}
            />
          </div>
          <section>
            <h2 className="text-lg font-semibold mb-3">
              {t('notifications.byChannel')}
            </h2>
            <DataTable
              headers={[
              ...
              rows={analytics.byChannel.map((r) => [
                r.channel,
                r.sent,
                r.delivered,
                r.failed,
