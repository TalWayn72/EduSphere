...
function useFeatures(): Feature[] {
  ...
  return FEATURE_DEFS.map((def) => {
    const steps = t(`features.${def.i18nKey}.steps`, {
      returnObjects: true,
      defaultValue: [],
    });
    const benefits = t(`features.${def.i18nKey}.benefits`, {
      returnObjects: true,
      defaultValue: [],
    });
    return {
      ...
      howItWorks: Array.isArray(steps)
        ? (steps as { title: string; desc: string }[]).map((s, i) => ({
            step: i + 1,
            title: s.title,
            desc: s.desc,
          }))
...
export function FeaturesPage() {
  ...
  const howToSchemas = FEATURES.filter((f) => f.howItWorks.length > 0).map(
    (feature) => ({
