...
export function SrsReviewPage() {
  ...
  const { currentCard, totalDue, fetching, error, submitRating, sessionComplete, stats } =
  ...
  if (error) {
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-32 gap-4" data-testid="error-state">
          <p className="text-destructive font-medium">{t('errorLoading', 'Failed to load review cards')}</p>
          <p className="text-sm text-muted-foreground">{error.message}</p>
          <Button asChild variant="outline">
            <Link to="/dashboard">{t('backToDashboard', 'Back to Dashboard')}</Link>
          </Button>
        </div>
      </Layout>
    );
  ...
  if (sessionComplete) {
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-32 gap-6" data-testid="session-complete">
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          <p className="text-muted-foreground text-lg" data-testid="no-cards-due">
            {t('noCardsDue', 'No cards due')}
          </p>
          <p className="text-sm text-muted-foreground">
            {t('noCardsDueHint', 'Great job! Check back later or explore courses to generate new review cards.')}
          </p>
          <Button asChild variant="outline">
