...
export function EmbeddingDashboardPage() {
  ...
  const [reindexResult, reindex] = useMutation(REINDEX_COURSE_EMBEDDINGS_MUTATION);

  const handleReindexConfirm = useCallback(async () => {
    const res = await reindex({});
    setDialogOpen(false);
    if (res.error) {
      toast.error(t('embeddings.reindexError', 'Reindex failed. Please try again.'));
    } else {
      toast.success(t('embeddings.reindexSuccess', 'Reindex started successfully.'));
  ...
  return (
    <AdminLayout
      ...
      description={t('embeddings.description', 'Monitor and manage content indexing status')}
    >
      <div data-testid="embedding-dashboard-page" className="space-y-6">
        <h1 className="sr-only">{t('embeddings.title', 'Embedding Dashboard')}</h1>

        {/* Action bar */}
        <div className="flex items-center justify-between">
          <Badge variant="outline" className="border-blue-400 text-blue-700 dark:border-blue-500 dark:text-blue-300">
          ...
          <div className="flex gap-2">
            ...
            >
              <RefreshCw className={`h-4 w-4 mr-1 ${fetching ? 'animate-spin' : ''}`} aria-hidden="true" />
        ...
        {/* Loading skeleton */}
        {fetching && !stats && (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4" data-testid="embedding-skeleton" aria-busy="true" aria-label={t('embeddings.loadingStats', 'Loading embedding statistics')}>
        ...
        {error && !fetching && (
          <Card>
            <CardContent role="alert" className="py-8 text-center text-destructive text-sm">
