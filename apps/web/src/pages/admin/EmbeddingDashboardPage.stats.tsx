  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <StatCard
        ...
        description={t(
          'embeddings.allKnowledgeSources',
      ...
      <CardContent>
        <Table aria-label={t('embeddings.courseBreakdown', 'Course Breakdown')}>
          <TableHeader>
            <TableRow>
              <TableHead scope="col">
                {t('embeddings.course', 'Course')}
              </TableHead>
              <TableHead scope="col" className="text-center">
                {t('embeddings.sources', 'Sources')}
              </TableHead>
              <TableHead scope="col" className="text-center">
                {t('embeddings.indexed', 'Indexed')}
              </TableHead>
              <TableHead scope="col" className="text-center">
                {t('embeddings.chunks', 'Chunks')}
              </TableHead>
              <TableHead scope="col" className="text-center">
                {t('embeddings.status', 'Status')}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {courses.map((c) => (
              <TableRow
                key={c.courseId}
                data-testid={`course-row-${c.courseId}`}
              >
