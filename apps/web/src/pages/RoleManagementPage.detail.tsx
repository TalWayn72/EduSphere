...
export function RoleDetailPanel({ role, onDuplicate, onEdit, onDelete }: Props) {
  const colorClass = ROLE_COLORS[role.name] ?? 'bg-muted text-muted-foreground border';
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="space-y-1">
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              <span className={`px-2 py-0.5 rounded text-sm font-mono border ${colorClass}`}>
                {role.name}
              </span>
            </CardTitle>
                  ...
                  {group.permissions.map(perm => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className={has ? 'text-foreground' : ''}>{perm.label}</span>
                      </label>
                    );
                  })}
                </div>
