...
const courses = [
  { id: 'c-1', title: 'Course A', description: '', thumbnailUrl: null, estimatedHours: 2, isPublished: true, instructorName: 'A', enrollmentCount: 0 },
  { id: 'c-2', title: 'Course B', description: '', thumbnailUrl: null, estimatedHours: 3, isPublished: false, instructorName: 'B', enrollmentCount: 5 },
];
...
describe('CourseGrid', () => {
  ...
  it('renders course cards', () => {
    render(<MemoryRouter><CourseGrid {...baseProps} /></MemoryRouter>);
    expect(screen.getAllByTestId('course-card')).toHaveLength(2);
  });

  it('shows loading state when fetching', () => {
    render(<MemoryRouter><CourseGrid {...baseProps} fetching={true} /></MemoryRouter>);
    expect(screen.getByText('loadingCourses')).toBeInTheDocument();
  });

  it('shows empty state when no courses', () => {
    render(<MemoryRouter><CourseGrid {...baseProps} courses={[]} /></MemoryRouter>);
