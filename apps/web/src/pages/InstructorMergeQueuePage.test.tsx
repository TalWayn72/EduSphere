...
const MOCK_PROPOSALS = [
  {
    id: 'mr-1',
    annotationId: 'ann-42',
    content: 'Maimonides uses overflow',
  ...
  },
  {
    id: 'mr-2',
    annotationId: 'ann-17',
    content: 'Kantian imperative parallels',
    description: 'Cross-course connection',
  ...
  },
  {
    id: 'mr-3',
    annotationId: 'ann-91',
    content: 'Compatibilism in Frankfurt cases',
    description: 'Useful clarification',
...
vi.mock('urql', async (importOriginal) => {
  ...
  return {
    ...
    useMutation: vi.fn((doc: unknown) => {
      if (String(doc).includes('approve'))
        return [{ fetching: false }, mockApproveFn];
...
describe('InstructorMergeQueuePage', () => {
  ...
  it('renders all 3 pending requests', () => {
    ...
    const proposalDivs = queue.querySelectorAll(
  ...
  it('shows course name in each request', () => {
    renderPage();
    expect(screen.getAllByText('Jewish Philosophy 101').length).toBeGreaterThan(
      0
    );
    expect(
      screen.getAllByText('Early Modern Philosophy').length
    ).toBeGreaterThan(0);
  });
});
