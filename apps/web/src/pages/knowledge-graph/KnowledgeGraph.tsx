...
import { Search, Loader2, RefreshCw, Network, User } from 'lucide-react';
          ...
          <div>
            <div className="mb-2">
              ...
              <p className="text-sm text-muted-foreground">
                Your annotations across all courses, connected by shared
                concepts.
              </p>
            </div>
            <PersonalGraphView
              ...
              {courseId ? (
                <>
                  <p className="text-xs text-muted-foreground mb-0.5">
                    {t('title')} &rsaquo;{' '}
                    <span
                      data-testid="kg-course-context-badge"
            ...
            <div className="flex gap-2 items-center">
              {(['CONCEPT', 'PERSON', 'SOURCE'] as const).map((tp) => (
                <button
                  key={tp}
                  onClick={() =>
                    graph.setTypeFilter(graph.typeFilter === tp ? null : tp)
                  }
...
const EDGE_LEGEND_STYLES: Record<string, React.CSSProperties> =
  Object.fromEntries(
