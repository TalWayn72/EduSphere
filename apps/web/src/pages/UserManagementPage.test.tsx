...
// Mock shadcn Select as a native <select> with real <option> children so that
// fireEvent.change can set element.value and trigger onChange properly.
// SelectTrigger / SelectValue render null — not needed for these tests.
...
describe('UserManagementPage', () => {
  ...
  it('shows error toast when resetPassword fails', async () => {
    ...
    // Discriminate by mutation string — robust against any number of re-renders.
    vi.mocked(useMutation).mockImplementation((mutation: unknown) => {
      const isReset = String(mutation).includes('resetUserPassword');
