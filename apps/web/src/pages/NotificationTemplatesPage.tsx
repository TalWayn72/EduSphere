...
const DEFAULTS: NotificationTemplate[] = [
  ...
  },
  {
    ...
    bodyHtml:
      '<h2>We miss you!</h2><p>Hi {{user.name}},</p><p>We noticed you have not logged in recently. Come back and continue your learning journey on <strong>{{tenant.name}}</strong>.</p>',
    variables: ['user.name', 'tenant.name'],
    isActive: false,
...
export function NotificationTemplatesPage() {
  ...
  const handleSave = (id: string, subject: string, bodyHtml: string) => {
    setTemplates((prev) =>
      prev.map((t) =>
        t.id === id
          ? { ...t, subject, bodyHtml, updatedAt: new Date().toISOString() }
          : t
      )
    );
    setSavedId(id);
  ...
  const handleReset = (id: string) => {
    ...
    setTemplates((prev) =>
      prev.map((t) =>
        t.id === id ? { ...def, updatedAt: new Date().toISOString() } : t
      )
    );
    setSavedId(null);
  };
