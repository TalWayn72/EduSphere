  ...
  return (
    <PublicLayout navVariant="minimal">
      ...
      <div className="mx-auto max-w-6xl px-6 py-16">
        <h1 className="mb-4 text-center text-4xl font-bold text-gray-900 dark:text-gray-100">{t('solutionsPage.heading')}</h1>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {SOLUTIONS.map(({ Icon, title, description, features }) => (
            <article key={title} className="flex flex-col rounded-xl bg-white p-8 shadow-sm dark:bg-gray-900">
              ...
              <ul className="mb-6 flex-1 space-y-2">
                {features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-200">
                    <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-indigo-500 dark:bg-indigo-600" aria-hidden="true" />
                    {f}
                  </li>
                ))}
              </ul>
