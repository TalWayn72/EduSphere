    ...
    <AdminLayout>
      <PageShell size="xl">
        ...
        <div data-testid="pilot-requests-page">
          ...
          {error && (
            <p className="text-destructive text-sm py-4">
              Failed to load pilot requests. Please try again.
            </p>
          )}
          ...
          {requests.length > 0 && (
            <div className="overflow-x-auto">
              <table
                data-testid="pilot-requests-table"
                  ...
                  {requests.map((req) => (
                    <tr key={req.id} className="border-b hover:bg-muted/30">
                      ...
                      <td className="py-3 pr-4">
                        {req.estimatedUsers.toLocaleString()}
                      </td>
                      <td className="py-3 pr-4 text-muted-foreground">
                      ...
                      <td className="py-3 pr-4">
                        <span
                          className={`inline-flex items-center rounded border px-2 py-0.5 text-xs font-medium ${STATUS_BADGE[req.status] ?? ''}`}
                        >
                          {req.status}
                        </span>
                      </td>
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Approve Pilot Request</DialogTitle>
              <DialogDescription className="sr-only">
                Set seat limit and approve pilot request.
              </DialogDescription>
            </DialogHeader>
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Reject Pilot Request</DialogTitle>
              <DialogDescription className="sr-only">
