...
export function extractResults(
  ...
  assets: { sourceUrl?: string | null; fileUrl?: string | null }[]
) {
  ...
  // Transcript (ASR)
  const transcript =
    getString(asr?.outputData, 'transcript') ??
  ...
  // Ingestion
  const ingestedUrl =
    getString(ingestion?.outputData, 'sourceUrl') ??
    getString(ingestion?.outputData, 'fileUrl') ??
    assets?.[0]?.sourceUrl ??
    assets?.[0]?.fileUrl;

  // NER / Source linking
  const entities = getArray<{ text: string; type: string }>(
    nerLinking?.outputData,
    'entities'
  );
  const linkedSources = getArray<{ title: string; url?: string }>(
    nerLinking?.outputData,
    'linkedSources'
  );
  ...
  // Citation verifier
  const verifiedCount =
    (citations?.outputData?.['verifiedCitations'] as unknown[] | undefined)
      ?.length ?? 0;
  const failedCount =
    (citations?.outputData?.['failedCitations'] as unknown[] | undefined)
      ?.length ?? 0;
  const matchReport = getString(citations?.outputData, 'matchReport');

  // QA Gate
  const qaScore =
    qa?.outputData?.['overallScore'] ?? qa?.outputData?.['qaScore'];
  const fixList = getArray<{ description: string; severity: string }>(
    qa?.outputData,
    'fixList'
  );
