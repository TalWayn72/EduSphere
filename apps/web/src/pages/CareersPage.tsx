    ...
    <PublicLayout navVariant="minimal">
      ...
      <section className="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white py-24 text-center dark:text-white">
        <div className="mx-auto max-w-3xl px-6">
          ...
          <p className="mt-4 text-lg text-indigo-100 dark:text-indigo-300">{t('careersPage.subtitle')}</p>
        </div>
      </section>
      ...
      <section className="py-16 bg-slate-50 dark:bg-slate-800">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefits.map((b) => (
              <div key={b.label} className="bg-white rounded-xl p-6 shadow-sm text-center dark:bg-gray-900">
                <b.icon className="h-8 w-8 mx-auto text-indigo-600 mb-3 dark:text-indigo-400" />
                <h3 className="font-semibold">{b.label}</h3>
                <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">{b.detail}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {positions.map((pos) => (
              <div key={pos.title} className="border rounded-xl p-6 hover:shadow-md transition-shadow">
                ...
                <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{pos.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-indigo-50 text-center dark:bg-indigo-950">
        <div className="mx-auto max-w-2xl px-6">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-300">
            Send your resume to{' '}
            <a href="mailto:careers@edusphere.dev" className="text-indigo-600 underline font-medium dark:text-indigo-400">
