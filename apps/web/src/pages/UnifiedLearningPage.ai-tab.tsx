...
export function AiTab({ chat }: Props) {
  ...
  const { messages, chatInput, setChatInput, sendMessage, chatEndRef, isStreaming } = chat;
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-3 py-2 border-b flex items-center gap-2 flex-shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">{t('dialecticalPartner')}</p>
        </div>
      ...
      <div className="px-3 py-2 flex gap-2 flex-shrink-0">
        ...
        <Button size="sm" className="h-8 w-8 p-0 flex-shrink-0"
          onClick={handleSend} disabled={isStreaming}>
