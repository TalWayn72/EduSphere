...
const CHANNELS = ['inApp', 'pushWeb', 'pushMobile', 'email', 'whatsapp'] as const;
type Channel = (typeof CHANNELS)[number];

const CHANNEL_LABELS: Record<Channel, string> = {
  inApp: 'In-App', pushWeb: 'Push Web', pushMobile: 'Push Mobile',
  email: 'Email', whatsapp: 'WhatsApp',
};

interface NotifType { key: string; label: string }
interface Category { name: string; types: NotifType[] }
...
export function NotificationPreferencesPage() {
  ...
  const isEnabled = useCallback((type: string, ch: Channel) =>
    prefs.find((p) => p.notificationType === type && p.channel === ch)?.enabled ?? false,
  [prefs]);

  const toggle = useCallback((type: string, ch: Channel, current: boolean) => {
    void executeMutation({ input: { notificationType: type, channel: ch, enabled: !current } });
  }, [executeMutation]);
      ...
      {fetching ? <p>Loading...</p> : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm" role="grid" aria-label="Notification preferences">
            <thead>
              <tr>
                <th className="text-left py-2 pr-4 font-medium">Notification</th>
                {CHANNELS.map((ch) => (
                  <th key={ch} className="text-center py-2 px-2 font-medium">{CHANNEL_LABELS[ch]}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {CATEGORIES.map((cat) => (
                <React.Fragment key={cat.name}>
                  <tr><td colSpan={6} className="pt-4 pb-1 font-semibold text-muted-foreground">{cat.name}</td></tr>
                  {cat.types.map((t) => (
                        ...
                        <td key={ch} className="text-center py-2 px-2">
                          {ch === 'whatsapp' && !whatsappSetup ? (
                            ...
                            <>
                              ...
                              <Switch id={`${t.key}-${ch}`} checked={isEnabled(t.key, ch)} onCheckedChange={() => toggle(t.key, ch, isEnabled(t.key, ch))} />
                            </>
                          )}
                        </td>
                      ))}
