...
function setup(prefs = MOCK_PREFS, fetching = false) {
  ...
  vi.mocked(urql.useMutation).mockReturnValue([{ fetching: false, stale: false, error: undefined }, mutateFn] as never);
  return { mutateFn };
}
...
describe('NotificationPreferencesPage', () => {
  ...
  it('shows "Set up" link when whatsapp not configured', () => {
    setup([{ notificationType: 'COURSE_ENROLLED', channel: 'inApp', enabled: true }]);
    render(<MemoryRouter><NotificationPreferencesPage /></MemoryRouter>);
    const setupLinks = screen.getAllByText('Set up');
    expect(setupLinks.length).toBeGreaterThan(0);
    expect(setupLinks[0].closest('a')).toHaveAttribute('href', '/settings/whatsapp');
  });
  ...
  it('has accessible table role', () => {
    setup();
    render(<MemoryRouter><NotificationPreferencesPage /></MemoryRouter>);
    expect(screen.getByRole('grid', { name: /notification preferences/i })).toBeInTheDocument();
  });
});
