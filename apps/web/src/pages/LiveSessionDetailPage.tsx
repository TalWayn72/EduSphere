    ...
    <Layout>
      <PageShell size="xl" spacing="compact">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate('/sessions')} data-testid="back-btn">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]" style={{ height: 'calc(100vh - 18rem)' }}>
          <Card className="flex flex-col overflow-hidden">
            <CardContent className="flex-1 flex flex-col p-0">
              <VideoArea session={session} isLive={isLive} isEnded={isEnded} />
              {isInstructor && !isEnded && (
                <div className="border-t px-4 py-2 flex items-center gap-3 bg-muted/30" data-testid="instructor-controls">
                  <p className="text-xs text-muted-foreground flex-1">Instructor controls</p>
                  <Button size="sm" variant="outline" className="h-7 text-xs">Start Recording</Button>
                </div>
              )}
            </CardContent>
          </Card>
...
function SessionHeader({
  session, isEnded, isInstructor, joined, joinFetching, endFetching, isLive,
  onJoin, onLeave, onEnd, t,
}: {
  session: LiveSession; isEnded: boolean; isInstructor: boolean; joined: boolean;
  joinFetching: boolean; endFetching: boolean; isLive: boolean;
  onJoin: () => void; onLeave: () => void; onEnd: () => void;
  t: (key: string) => string;
}) {
  return (
    <div className="flex items-center justify-between gap-4">
      <div>
        <h1 className="text-xl font-bold" data-testid="session-detail-title">{session.meetingName}</h1>
        <p className="text-sm text-muted-foreground mt-0.5">{session.participantCount ?? 0} participants</p>
      </div>
      <div className="flex items-center gap-2">
        <StatusBadge status={session.status} />
        {!isEnded && !joined && !isInstructor && (
          <Button size="sm" onClick={onJoin} disabled={joinFetching} data-testid="join-btn">
            {joinFetching ? <Loader2 className="h-4 w-4 animate-spin" /> : t('joinSession')}
          </Button>
        )}
        {!isEnded && joined && !isInstructor && (
          <Button size="sm" variant="outline" onClick={onLeave} data-testid="leave-btn">Leave</Button>
        )}
        {isInstructor && isLive && (
          <Button size="sm" variant="destructive" onClick={onEnd} disabled={endFetching} data-testid="end-session-btn">
            {endFetching ? <Loader2 className="h-4 w-4 animate-spin" /> : (
              <><StopCircle className="h-4 w-4 mr-1" />{t('endSession')}</>
            )}
          </Button>
        )}
      </div>
...
function VideoArea({ session, isLive, isEnded }: { session: LiveSession; isLive: boolean; isEnded: boolean }) {
  return (
    <div className="flex-1 bg-zinc-900 flex flex-col items-center justify-center gap-3 rounded-t-lg dark:bg-zinc-100" data-testid="video-area">
      {isEnded ? (
        <div className="text-center space-y-2" data-testid="session-ended-state">
          ...
          {session.recordingUrl && (
            <a href={session.recordingUrl} target="_blank" rel="noopener noreferrer"
              className="text-primary text-sm underline" data-testid="recording-link">
              Watch Recording
            </a>
          )}
        </div>
