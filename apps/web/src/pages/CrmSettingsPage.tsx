...
import {
  CRM_CONNECTION_QUERY, CRM_SYNC_LOG_QUERY, DISCONNECT_CRM_MUTATION,
} from '@/lib/graphql/crm.queries';
import { Link, Copy, CheckCircle, XCircle, Loader2, Unplug } from 'lucide-react';

const ADMIN_ROLES = new Set(['ORG_ADMIN', 'SUPER_ADMIN']);

interface CrmConnectionData {
  id: string; provider: string; instanceUrl: string; isActive: boolean; createdAt: string;
}
interface CrmSyncEntry {
  id: string; operation: string; externalId: string | null;
...
export function CrmSettingsPage() {
  ...
  const [connResult, refetchConn] = useQuery<{ crmConnection: CrmConnectionData | null }>({
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Sync course completions to Salesforce and accept enrollment webhooks
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>OAuth 2.0 connection to your Salesforce org</CardDescription>
          </CardHeader>
          <CardContent>
            {connResult.fetching ? (
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Instance: <span className="font-mono">{conn.instanceUrl}</span>
                </p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configure this URL in Salesforce to push enrollment events to EduSphere
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2 p-3 bg-muted rounded-md font-mono text-sm">
              <span className="flex-1 truncate">{webhookUrl}</span>
              <Button variant="ghost" size="sm" onClick={() => void handleCopyWebhook()}>
                <Copy className="h-4 w-4" />
                <span className="ml-1">{copied ? 'Copied!' : 'Copy'}</span>
              </Button>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Set <span className="font-mono">X-Salesforce-HMAC-SHA256</span> header using your
              <span className="font-mono"> SALESFORCE_WEBHOOK_SECRET</span> env var.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Last 20 CRM synchronization operations</CardDescription>
          </CardHeader>
          <CardContent>
            {logResult.fetching ? (
              <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <div className="divide-y text-sm">
                {logEntries.map((entry) => (
                  <div key={entry.id} className="py-2 flex items-center gap-3 flex-wrap">
                    <span className={
                    ...
                    {entry.externalId !== null && (
                      <span className="text-xs text-muted-foreground">{entry.externalId}</span>
                    )}
