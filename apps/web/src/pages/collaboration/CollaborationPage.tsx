...
import { MY_DISCUSSIONS_QUERY, CREATE_DISCUSSION_MUTATION } from '@/lib/graphql/collaboration.queries';
    ...
    <Layout>
      <div className="space-y-6">
        ...
        {error && !isSchemaValidationError && (
          <div role="alert" aria-live="polite" data-testid="collab-offline-banner" className="flex items-center justify-between gap-2 px-3 py-1.5 text-xs text-orange-800 bg-orange-50 border border-orange-200 rounded-md dark:text-orange-200 dark:bg-orange-950 dark:border-orange-700">
            <div className="flex items-center gap-2">
              <AlertTriangle className="h-3 w-3 flex-shrink-0" />
              <span>{t('loadError')}</span>
            </div>
            <button onClick={() => reexecute({ requestPolicy: 'network-only' })} className="underline hover:no-underline text-orange-900 font-medium shrink-0 dark:text-orange-100" data-testid="collab-offline-banner-retry">
              {t('common:retry')}
            </button>
          </div>
        )}
        ...
        {error && isSchemaValidationError && (
          <div role="status" aria-live="polite" data-testid="collab-schema-banner" className="flex items-center gap-2 px-3 py-1.5 text-xs text-blue-800 bg-blue-50 border border-blue-200 rounded-md dark:text-blue-200 dark:bg-blue-950 dark:border-blue-700">
            <AlertTriangle className="h-3 w-3 flex-shrink-0" />
