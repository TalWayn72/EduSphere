...
const DOC_LINES = [
  ...
  {
    text: 'neural networks. These systems learn to perform tasks by',
    heading: false,
  },
  {
    text: 'considering examples — without explicit task-specific rules.',
    heading: false,
  },
  { text: '' },
  { text: 'Key concepts:', heading: false },
  {
    text: '  Layers · Weights · Activation Functions · Backprop',
    highlight: true,
  },
  { text: '' },
  {
    text: 'The first layer receives raw input, processes it through',
    heading: false,
  },
      ...
      >
        <div
          style={{
            ...
            height: 8,
            borderRadius: '50%',
            background: role === 'instructor' ? '#10b981' : '#6366f1',
          }}
        />
      ...
      <div style={{ flex: 1 }}>
        {DOC_LINES.map((l, i) => (
          <div
            key={i}
            style={{
              fontSize: l.heading ? 14 : 12,
              fontWeight: l.heading ? 700 : 400,
              color: l.heading ? '#e2e8f0' : '#94a3b8',
              lineHeight: 1.85,
              paddingLeft: 8,
              background: l.highlight
                ? `rgba(251,191,36,${highlightOpacity * 0.22})`
                : 'transparent',
              borderLeft: l.highlight
                ? `3px solid rgba(251,191,36,${highlightOpacity})`
...
export function LiveCollab() {
  ...
  const leftIn = interpolate(frame, [0, 20], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const rightIn = interpolate(frame, [55, 75], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  const instrHighlight = interpolate(frame, [148, 168], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const studHighlight = interpolate(frame, [218, 238], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  const syncOpacity = interpolate(frame, [155, 170, 220, 235], [0, 1, 1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  const kgSlide = interpolate(frame, [550, 590], [280, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
    ...
    >
      ...
      >
        ...
        <span style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600 }}>
          EduSphere Live · Intro to Neural Networks
        </span>
      ...
      <div style={{ display: 'flex', gap: 18, flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, opacity: leftIn }}>
          <Panel
            ...
            role="instructor"
            highlightOpacity={instrHighlight}
            commentVisible={frame > 378}
        ...
        <div style={{ flex: 1, opacity: rightIn }}>
          <Panel
            title="Student View"
            role="student"
            highlightOpacity={studHighlight}
        ...
        {frame > 540 && (
          ...
          >
            ...
            {KG_NODES.map((concept, i) => {
              const op = interpolate(frame - 558 - i * 14, [0, 18], [0, 1], {
                extrapolateLeft: 'clamp',
                extrapolateRight: 'clamp',
              });
              return (
