...
const STEPS = [
  ...
  },
  {
    num: '02',
    title: 'Set Goals',
    desc: 'Choose your learning path',
    icon: '🎯',
    color: '#8b5cf6',
    delay: 85,
  },
  {
    ...
    desc: 'AI adapts to you instantly',
    icon: '🚀',
    color: '#06b6d4',
    delay: 145,
  },
...
function StepCard({ step, frame }: { step: (typeof STEPS)[0]; frame: number }) {
  ...
  const checkOpacity = interpolate(lf, [28, 42], [0, 1], {
...
function Connector({ frame, delay }: { frame: number; delay: number }) {
  ...
  const width = interpolate(lf, [0, 35], [0, 120], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  return (
...
export function OnboardingSpeed() {
  ...
  const timerOpacity = interpolate(frame, [220, 240], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
