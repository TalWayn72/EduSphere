...
(async () => {
  ...
  for (var i = 0; i < 20; i++) {
    ...
    foundSettings = await page.locator('h1:has-text("Settings")').isVisible().catch(function() { return false; });
    foundCrash = await page.locator('text=Something went wrong').isVisible().catch(function() { return false; });
  ...
  var hasHighlight = await page.locator('.animate-settings-highlight').isVisible().catch(function() { return false; });
  ...
  var passed = (foundCrash === false) && foundSettings && hasToggle && (tooltipError === false);
  console.log('');
