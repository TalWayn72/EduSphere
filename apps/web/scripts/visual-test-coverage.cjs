...
function classifyFile(filename) {
  ...
  if (lower.includes('dark-mode') || lower.includes('dark_mode'))
    return 'dark-mode';
  ...
  if (lower.includes('cross-browser') || lower.includes('xbrowser'))
...
for (const file of storyFiles) {
  ...
  const namedExports = countMatches(
    content,
...
console.log(
  `  Storybook Stories:      ${totalStories} (across ${storyFiles.length} files)`
);
...
console.log(
  `  Target: 500+ assertions | Current: ${totalAssertions >= 500 ? 'MET' : `${500 - totalAssertions} remaining`}`
);
console.log(divider);
console.log('');
