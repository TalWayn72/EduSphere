...
async function run() {
  ...
  try {
    ...
    const kcBtn = page
      .locator('button:has-text("Keycloak"), button:has-text("Sign In")')
      .first();
    ...
    if (page.url().includes('keycloak') || page.url().includes(':8080')) {
      ...
      await page.waitForURL((u) => !u.href.includes('keycloak'), {
        timeout: 30000,
      });
      await page.waitForTimeout(2000);
    }
                ...
                modules: [
                  {
                    ...
                    contentItemTitles: ['What is ML?', 'Types'],
                  },
                  {
                    title: 'Supervised Learning',
                    description: 'Classification',
                    contentItemTitles: ['Regression', 'Trees'],
                  },
                  {
                    title: 'Neural Networks',
                    description: 'Deep learning',
                    contentItemTitles: ['Perceptrons', 'CNNs'],
                  },
                ],
              },
            },
    ...
    const aiBtn = page
      .locator(
        'button:has-text("AI"), button:has-text("Builder"), button:has-text("בונה")'
      )
      .first();
    await aiBtn.click();
    ...
    await dialog
      .locator('textarea')
      .first()
      .fill('Introduction to Machine Learning');
    await page.waitForTimeout(300);

    const genBtn = dialog
      .locator('button:has-text("Generate"), button:has-text("צור")')
      .first();
    ...
    for (let i = 0; i < 15; i++) {
      ...
      statusTexts.forEach((t) => {
        if (t.trim()) seenTexts.add(t.trim());
      });

      const spinCount = await dialog
      ...
      console.log(
        `   [t+${i + 1}s] status: ${JSON.stringify(statusTexts.map((t) => t.trim()))} btn: "${btnText.trim()}" disabled: ${btnDisabled}`
      );
      ...
      if (i === 0 || i === 3 || i === 6 || i === 9 || i === 12) {
        await page.screenshot({
          path: path.join(
            SCREENSHOTS_DIR,
            `progress-gen-${String(i).padStart(2, '0')}.png`
          ),
        });
      }
    ...
    console.log(
      `   ProgressStatus renders: ${progressWorking ? 'YES ✓' : 'NO ✗'}`
    );
    console.log(
      `   Text cycling (>1 unique): ${seenTexts.size > 1 ? 'YES ✓' : 'NO ✗'}`
    );

    // Take a screenshot showing the progress state
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'progress-generating-proof.png'),
    });
    ...
    const outlineVisible = await dialog
      .locator('h3')
      .isVisible()
      .catch(() => false);
    if (outlineVisible) {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'progress-07-success.png'),
    });
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'progress-final.png'),
    });
    ...
    console.log(
      `   Text cycling detected: ${seenTexts.size > 1 ? 'YES ✓' : 'NO ✗'}`
    );
    ...
    await page
      .screenshot({ path: path.join(SCREENSHOTS_DIR, 'progress-ERROR.png') })
      .catch(() => {});
  } finally {
    await browser.close();
  }
