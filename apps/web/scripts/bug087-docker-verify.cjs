...
(async () => {
  ...
  if (
    await kcBtn.isVisible({ timeout: 5000 }).catch(function () {
      return false;
    })
  ) {
    await kcBtn.click();
  ...
  if (
    await usernameField.isVisible({ timeout: 10000 }).catch(function () {
      return false;
    })
  ) {
  ...
  await page.screenshot({
    path: '../../docs/screenshots/bug087-docker-verified.png',
    fullPage: true,
  });
