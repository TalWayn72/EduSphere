...
function makeUpdateChain(captured: { set?: unknown } = {}) {
  ...
  const set = vi.fn().mockImplementation((data: unknown) => {
    captured.set = data;
    return { where };
  });
    ...
    new Promise<T>((_, reject) =>
      setTimeout(
        () =>
          reject(
            new Error(
              'Agent execution timeout: ' + label + ' exceeded ' + ms + 'ms'
            )
          ),
        ms
      )
...
describe('AgentService fire-and-forget timeout (memory safety)', () => {
  ...
  it('marks execution as FAILED after 5-minute timeout using Promise.race', async () => {
    ...
    const guardedExecution = withTimeout(
      neverResolves,
      FIVE_MINUTES_MS,
      executionId
    ).catch((err: Error) => {
  ...
  it('records an error message containing timeout in DB output field', async () => {
    ...
    const returning = vi.fn().mockResolvedValue([]);
    const where = vi.fn().mockReturnValue({ returning });
    const set = vi.fn().mockImplementation((data: { output?: unknown }) => {
    ...
    expect(typeof (capturedOutput as Record<string, unknown>).error).toBe(
      'string'
    );
    expect((capturedOutput as Record<string, unknown>).error as string).toMatch(
      /timeout/i
    );
  });
  ...
  it('does NOT reject before the 5-minute mark', async () => {
    ...
    withTimeout(neverResolves, 300_000, 'exec-early-test').catch(() => {
      timedOut = true;
    });
  ...
  it('marks execution as FAILED when processExecution rejects immediately', async () => {
    ...
    const runningWhere = vi
      .fn()
      .mockReturnValue({ returning: runningReturning });
    ...
    const failSet = vi.fn().mockImplementation((data: unknown) => {
      failedPayload = data;
      return { where: failWhere };
    });
    mockUpdate
  ...
  it('includes the execution label in the timeout error message', async () => {
    ...
    let errorMessage = '';
    const guarded = withTimeout(neverResolves, 300_000, label).catch(
      (err: Error) => {
