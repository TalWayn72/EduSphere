...
const mockWorkflow = {
  invoke: vi.fn(() => Promise.resolve({ evaluation: null })),
};
...
describe('RoleplaySessionService', () => {
  ...
  describe('startSession', () => {
    it('checks THIRD_PARTY_LLM consent before any LLM call', async () => {
      const template = {
        id: 'tpl-1',
        character_persona: 'Angry customer',
        scene_description: 'De-escalate',
        evaluation_rubric: [],
        max_turns: 8,
        is_active: true,
      };
      ...
      expect(mockConsentGuard.assertConsent).toHaveBeenCalledWith(
        'user-1',
        expect.any(Boolean)
      );
    });
    ...
    it('throws CONSENT_REQUIRED when consent guard rejects', async () => {
      ...
      await expect(
        service.startSession('tpl-1', 'user-1', 'tenant-1')
    ...
    it('throws NotFoundException when scenario not found', async () => {
      ...
      await expect(
        service.startSession('nonexistent', 'user-1', 'tenant-1')
  ...
  describe('sendMessage', () => {
    it('throws NotFoundException when session not found', async () => {
      ...
      await expect(
        service.sendMessage('nonexistent', 'Hello', 'user-1', 'tenant-1')
      ).rejects.toThrow(NotFoundException);
    });
