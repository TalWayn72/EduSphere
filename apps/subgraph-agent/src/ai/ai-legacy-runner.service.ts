...
} from '../workflows/chavruta.workflow.js';
import {
  createQuizWorkflow as createLegacyQuizWorkflow,
  type QuizContext,
} from '../workflows/quiz.workflow.js';
import {
  createSummarizerWorkflow,
  type SummarizerContext,
} from '../workflows/summarizer.workflow.js';
import {
...
export class AiLegacyRunnerService {
  ...
  async runQuiz(
    ...
    const result = await createLegacyQuizWorkflow(model, locale).step(
      ctx,
      input.userAnswer
    );
    return {
  ...
  async executeStream(
    ...
    if (agent.template === 'CHAVRUTA_DEBATE') {
      return createChavrutaWorkflow(model, locale).stream(
        this.buildChavrutaCtx(input)
      );
    }
    if (agent.template === 'QUIZ_ASSESS') {
      return createLegacyQuizWorkflow(model, locale).stream(
        this.buildQuizCtx(input),
        input.userAnswer
      );
    }
    if (agent.template === 'SUMMARIZE') {
      return createSummarizerWorkflow(model, locale).stream(
        this.buildSummarizerCtx(input)
      );
    }
    const system = injectLocale(this.resolveSystemPrompt(agent), locale);
    const prompt = this.buildUserPrompt(input);
  ...
  buildSummarizerCtx(input: ExecutionInput): SummarizerContext {
    ...
    return {
      sessionId: input.sessionId ?? 'default',
      content: (input.context?.['content'] as string) ?? input.message ?? '',
