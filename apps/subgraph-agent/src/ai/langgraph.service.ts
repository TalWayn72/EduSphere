...
export class LangGraphService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (dbUrl) {
      try {
        ...
        this.logger.log(
          'LangGraph: using PostgresSaver for durable checkpointing'
        );
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        this.logger.warn(
          `LangGraph: PostgresSaver init failed (${msg}), falling back to MemorySaver`
        );
  ...
  private scheduleMemoryTrim(): void {
    this.cleanupInterval = setInterval(() => {
      ...
      const storage = (saver as unknown as { storage?: Map<string, unknown> })
        .storage;
  ...
  async onModuleDestroy(): Promise<void> {
    ...
    if (this.pool !== null) {
      try {
        ...
        this.logger.error(
          `LangGraph: pool.end() failed: ${err instanceof Error ? err.message : String(err)}`
        );
      }
      this.pool = null;
    }
  ...
  getCheckpointer(): Checkpointer {
    if (!this.checkpointer) {
      throw new Error(
        'LangGraphService not initialized — onModuleInit() has not been called'
      );
    }
