...
const MOCK_ANNOTATION = {
  id: 'ann-gdpr', asset_id: 'asset-1', user_id: 'user-owner',
  tenant_id: 'tenant-1', layer: 'PERSONAL', content: { text: 'private note' },
...
describe('AnnotationService — GDPR compliance', () => {
  ...
  describe('Soft delete — never hard delete', () => {
    it('delete() calls UPDATE with deleted_at timestamp (soft delete)', async () => {
      ...
      mockReturning.mockResolvedValue([{ ...MOCK_ANNOTATION, deleted_at: deletedAt }]);
      const result = await service.delete('ann-gdpr', ownerAuth);
      // update().set() was called — soft-delete pattern, not a DB row removal
      expect(mockUpdate).toHaveBeenCalled();
      expect(mockSet).toHaveBeenCalledWith(expect.objectContaining({ deleted_at: expect.any(Date) }));
    ...
    it('soft-deleted annotation is excluded from findAll results', async () => {
      mockOrderBy.mockReturnValue({ limit: mockLimit.mockReturnValue({ offset: vi.fn().mockResolvedValue([]) }) });
  ...
  describe('PERSONAL layer — owner-only visibility', () => {
    it('findAll with PERSONAL layer passes owner userId in tenant context', async () => {
      ...
      mockOrderBy.mockReturnValue({ limit: mockLimit.mockReturnValue({ offset: vi.fn().mockResolvedValue([MOCK_ANNOTATION]) }) });
      await service.findAll({ layer: 'PERSONAL', limit: 10, offset: 0 }, ownerAuth);
  ...
  describe('GDPR erasure — nullify annotation content', () => {
    it('update() can nullify content field (GDPR erasure pattern)', async () => {
      ...
      const result = await service.update('ann-gdpr', { content: null }, ownerAuth);
      expect(mockSet).toHaveBeenCalledWith(expect.objectContaining({ content: null }));
      expect(result).toBeDefined();
    });

    it('update() accepts empty string content as erasure value', async () => {
      mockLimit.mockResolvedValue([MOCK_ANNOTATION]);
      mockReturning.mockResolvedValue([{ ...MOCK_ANNOTATION, content: { text: '' } }]);
      const result = await service.update('ann-gdpr', { content: { text: '' } }, ownerAuth);
      expect(result).toBeDefined();
      expect(mockSet).toHaveBeenCalledWith(expect.objectContaining({ content: { text: '' } }));
    });
  });
});
