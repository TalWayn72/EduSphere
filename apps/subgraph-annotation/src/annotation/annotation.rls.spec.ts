...
const capturedContexts: Array<{ tenantId: string; userId: string; userRole: string }> = [];
...
const tenantAAuth: AuthContext = {
  userId: 'user-a', email: 'a@a.com', username: 'userA',
  tenantId: 'tenant-A', roles: ['STUDENT'], scopes: [], isSuperAdmin: false,
};
const tenantBAuth: AuthContext = {
  userId: 'user-b', email: 'b@b.com', username: 'userB',
  tenantId: 'tenant-B', roles: ['STUDENT'], scopes: [], isSuperAdmin: false,
};
const MOCK_ANNOTATION = {
  id: 'ann-1', asset_id: 'asset-1', user_id: 'user-a',
  tenant_id: 'tenant-A', layer: 'PERSONAL', content: { text: 'hi' },
  ...
  describe('createAnnotation — tenantId from AuthContext', () => {
    it('sets tenantId from AuthContext, not from client input', async () => {
      ...
      await service.create(
        { assetId: 'asset-1', annotationType: 'TEXT', layer: 'PERSONAL', content: { text: 'x' } },
        tenantAAuth
      );
    ...
    it('withTenantContext receives correct userId and userRole', async () => {
      mockReturning.mockResolvedValue([MOCK_ANNOTATION]);
      await service.create(
        { assetId: 'asset-1', annotationType: 'TEXT', layer: 'PERSONAL', content: {} },
        tenantAAuth
      );
      expect(withTenantContext).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ tenantId: 'tenant-A', userId: 'user-a', userRole: 'STUDENT' }),
        expect.any(Function)
      );
    });
  });
