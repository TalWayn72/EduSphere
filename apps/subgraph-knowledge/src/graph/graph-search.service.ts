...
export class GraphSearchService {
  ...
  async semanticSearch(
    query: string, limit: number, tenantId: string, userId: string, role: string
    ...
    return withTenantContext(db, { tenantId, userId, userRole: toUserRole(role) }, async () => {
      ...
      try {
        ...
        type Row = { id: string; segment_id: string; transcript_id: string; text: string; similarity: string };
        ...
        vectorResults = rows.map((r) => ({
          id: r.segment_id, text: r.text,
          similarity: parseFloat(r.similarity),
          entityType: 'transcript_segment', entityId: r.transcript_id,
        }));
        this.logger.debug({ hits: vectorResults.length }, 'pgvector search complete');
      } catch (err) {
        this.logger.warn(`pgvector search unavailable (${String(err)}) — using ILIKE fallback`);
      }
      ...
      if (remaining > 0) {
        ...
        const segments = await db
          .select({ id: transcript_segments.id, text: transcript_segments.text, transcript_id: transcript_segments.transcript_id })
        ...
        textResults = segments
          ...
          .map((seg) => ({
            ...
            entityType: 'transcript_segment', entityId: seg.transcript_id,
          }));
      }

      const conceptResults = await this.searchConceptsByText(query, tenantId, Math.max(1, Math.floor(limit / 4)));
  ...
  async generateEmbedding(
    ...
    return withTenantContext(db, { tenantId, userId, userRole: toUserRole(role) }, async () => {
      if (entityType !== 'transcript_segment') {
        this.logger.warn(`generateEmbedding: unsupported entityType=${entityType}`);
        return false;
      }
      try {
        ...
        this.logger.error(`generateEmbedding failed for ${entityId}: ${String(err)}`);
        return false;
      }
    });
  }
  ...
  private async searchConceptsByText(query: string, tenantId: string, limit: number): Promise<SemanticResult[]> {
    try {
      const concepts = await this.cypher.findAllConcepts(tenantId, limit * 3);
      const q = query.toLowerCase();
      return (concepts as GraphConceptNode[])
        .filter((c) => c.name?.toLowerCase().includes(q) || c.definition?.toLowerCase().includes(q))
        .slice(0, limit)
        .map((c) => ({
          id: c.id, text: c.definition ?? c.name,
          similarity: this.computeTextSimilarity(`${c.name} ${c.definition ?? ''}`, query),
          entityType: 'concept', entityId: c.id,
        }));
    } catch (err) {
