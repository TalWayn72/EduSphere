...
describe('GraphService extended', () => {
  ...
  it('linkConcepts returns relationship info', async () => {
    ...
    mc.findConceptById.mockResolvedValueOnce(RAW).mockResolvedValueOnce({ ...RAW, id: 'c-2' });
    var r = await svc.linkConcepts('c-1','c-2','RELATES_TO',0.8,'desc','t-1','u-1','INSTRUCTOR');
  ...
  it('findTopicClustersByCourse delegates to cypherService', async () => {
    ...
    expect(await svc.findTopicClustersByCourse('course-1','ten-1','u-1','STUDENT')).toHaveLength(1);
  });
  ...
  it('generateEmbedding returns false', async () => {
    expect(await svc.generateEmbedding('text','Concept','c-1','t-1','u-1','INSTRUCTOR')).toBe(false);
  });
});