...
import { executeCypher, createConcept, findRelatedConcepts, createRelationship } from '@edusphere/db';

var RAW = { id: 'c-1', tenant_id: 't-1', name: 'Free Will', definition: 'Ability', source_ids: '[]' };
var PERSON = { id: 'p-1', tenant_id: 't-1', name: 'Maimonides', bio: 'Philosopher' };
var TERM = { id: 'term-1', tenant_id: 't-1', name: 'Torah', definition: 'Scriptures' };
var SOURCE = { id: 'src-1', tenant_id: 't-1', title: 'Guide', type: 'BOOK', url: null };
var CLUSTER = { id: 'cl-1', tenant_id: 't-1', name: 'Cluster', description: null };
describe('CypherService', function() {
  ...
  describe('findConceptByNameCaseInsensitive', function() {
    it('finds concept matching case-insensitively', async function() {
      (executeCypher as any).mockResolvedValue([RAW]);
      const result = await service.findConceptByNameCaseInsensitive('free will', 't-1');
      expect(result).toEqual(RAW);
    });
    it('returns null when no match', async function() {
      (executeCypher as any).mockResolvedValue([]);
      expect(await service.findConceptByNameCaseInsensitive('Unknown', 't-1')).toBeNull();
    });
  ...
  describe('createConcept', function() {
    it('delegates to createConcept helper', async function() {
      ...
      var result = await service.createConcept({ tenant_id: 't-1', name: 'X', definition: 'Y', source_ids: [] });
  ...
  describe('updateConcept', function() {
    it('updates and returns result', async function() {
      (executeCypher as any).mockResolvedValue([RAW]);
      expect(await service.updateConcept('c-1', 't-1', { name: 'Updated' })).toEqual(RAW);
    });
    it('returns null when not found', async function() {
      (executeCypher as any).mockResolvedValue([]);
  ...
  describe('findRelatedConcepts', function() {
    it('delegates to helper', async function() {
      ...
      expect(await service.findRelatedConcepts('c-1', 't-1', 2, 10)).toEqual([{ id: 'r-1' }]);
    });
  });
  ...
  describe('linkConcepts', function() {
    it('delegates to createRelationship', async function() {
      ...
      expect(createRelationship).toHaveBeenCalledWith(expect.anything(), 'f-1', 't-1', 'RELATES_TO', { strength: 0.8 });
    });
  });
  describe('findPersonById', function() {
    it('returns person', async function() { (executeCypher as any).mockResolvedValue([PERSON]); expect(await service.findPersonById('p-1','t-1')).toEqual(PERSON); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findPersonById('x','t-1')).toBeNull(); });
  });
  describe('findPersonByName', function() {
    it('returns person', async function() { (executeCypher as any).mockResolvedValue([PERSON]); expect(await service.findPersonByName('Maimonides','t-1')).toEqual(PERSON); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findPersonByName('X','t-1')).toBeNull(); });
  });
  describe('createPerson', function() {
    it('returns created person', async function() { (executeCypher as any).mockResolvedValue([PERSON]); expect(await service.createPerson('Maimonides','Bio','t-1')).toEqual(PERSON); });
    it('passes null bio', async function() { (executeCypher as any).mockResolvedValue([PERSON]); await service.createPerson('Name',null,'t-1'); expect((executeCypher as any).mock.calls[0][3].bio).toBeNull(); });
  });

  describe('findTermById', function() {
    it('returns term', async function() { (executeCypher as any).mockResolvedValue([TERM]); expect(await service.findTermById('term-1','t-1')).toEqual(TERM); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findTermById('x','t-1')).toBeNull(); });
  });
  describe('findTermByName', function() {
    it('returns term', async function() { (executeCypher as any).mockResolvedValue([TERM]); expect(await service.findTermByName('Torah','t-1')).toEqual(TERM); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findTermByName('X','t-1')).toBeNull(); });
  });
  describe('createTerm', function() {
    it('returns created term', async function() { (executeCypher as any).mockResolvedValue([TERM]); expect(await service.createTerm('Torah','Scriptures','t-1')).toEqual(TERM); });
  });

  describe('findSourceById', function() {
    it('returns source', async function() { (executeCypher as any).mockResolvedValue([SOURCE]); expect(await service.findSourceById('src-1','t-1')).toEqual(SOURCE); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findSourceById('x','t-1')).toBeNull(); });
  });
  describe('createSource', function() {
    it('returns created source', async function() { (executeCypher as any).mockResolvedValue([SOURCE]); expect(await service.createSource('Title','BOOK',null,'t-1')).toEqual(SOURCE); });
    it('passes null url', async function() { (executeCypher as any).mockResolvedValue([SOURCE]); await service.createSource('T','BOOK',null,'t-1'); expect((executeCypher as any).mock.calls[0][3].url).toBeNull(); });
  });

  describe('findTopicClusterById', function() {
    it('returns cluster', async function() { (executeCypher as any).mockResolvedValue([CLUSTER]); expect(await service.findTopicClusterById('cl-1','t-1')).toEqual(CLUSTER); });
    it('returns null', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findTopicClusterById('x','t-1')).toBeNull(); });
  });
  describe('findTopicClustersByCourse', function() {
    it('returns clusters', async function() { (executeCypher as any).mockResolvedValue([CLUSTER]); expect(await service.findTopicClustersByCourse('course-1','t-1')).toHaveLength(1); });
    it('returns empty array', async function() { (executeCypher as any).mockResolvedValue([]); expect(await service.findTopicClustersByCourse('x','t-1')).toEqual([]); });
  });
  describe('createTopicCluster', function() {
    it('returns created cluster', async function() { (executeCypher as any).mockResolvedValue([CLUSTER]); expect(await service.createTopicCluster('Cluster',null,'t-1')).toEqual(CLUSTER); });
    it('passes null description', async function() { (executeCypher as any).mockResolvedValue([CLUSTER]); await service.createTopicCluster('C',null,'t-1'); expect((executeCypher as any).mock.calls[0][3].description).toBeNull(); });
  });
