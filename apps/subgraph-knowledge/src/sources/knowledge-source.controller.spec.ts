...
const { mockValidate, mockExtractToken, mockCreateAndProcess } = vi.hoisted(
  () => {
    ...
    const mockCreateAndProcess = vi
      .fn()
      .mockResolvedValue({ id: 'ks-new', status: 'PENDING' });
...
describe('KnowledgeSourceController', () => {
  ...
  it('throws UnauthorizedException when Authorization header is missing', async () => {
    ...
    await expect(
      controller.upload(file, VALID_BODY, undefined)
  ...
  it('throws UnauthorizedException when token has no tenant claim', async () => {
    ...
    await expect(
      controller.upload(file, VALID_BODY, VALID_AUTH)
    ).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('throws BadRequestException when no file is uploaded', async () => {
    await expect(
      controller.upload(undefined as never, VALID_BODY, VALID_AUTH)
  ...
  it('throws BadRequestException when courseId is missing', async () => {
    ...
    await expect(
      controller.upload(file, { courseId: undefined as never }, VALID_AUTH)
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('throws BadRequestException for unsupported file type', async () => {
    await expect(
      controller.upload(
        makeFile('slides.pptx', 'application/vnd.ms-powerpoint'),
        VALID_BODY,
        VALID_AUTH
      )
    ).rejects.toThrow(BadRequestException);
  ...
  it('detects FILE_PDF from MIME type', async () => {
    await controller.upload(
      makeFile('thesis.pdf', 'application/pdf'),
      VALID_BODY,
      VALID_AUTH
    );
    expect(mockCreateAndProcess).toHaveBeenCalledWith(
      expect.objectContaining({ sourceType: 'FILE_PDF' })
    );
  });
  ...
  it('detects FILE_TXT from MIME type', async () => {
    ...
    expect(mockCreateAndProcess).toHaveBeenCalledWith(
      expect.objectContaining({ sourceType: 'FILE_TXT' })
    );
  });

  it('falls back to extension detection when MIME is generic', async () => {
    await controller.upload(
      makeFile('document.docx', 'application/octet-stream'),
      VALID_BODY,
      VALID_AUTH
    );
    expect(mockCreateAndProcess).toHaveBeenCalledWith(
      expect.objectContaining({ sourceType: 'FILE_DOCX' })
    );
  });

  it('uses file.originalname as title when body.title is omitted', async () => {
    await controller.upload(
      makeFile('my-notes.pdf', 'application/pdf'),
      { courseId: 'c-1' },
      VALID_AUTH
    );
    expect(mockCreateAndProcess).toHaveBeenCalledWith(
      expect.objectContaining({ title: 'my-notes.pdf' })
    );
  });
});
