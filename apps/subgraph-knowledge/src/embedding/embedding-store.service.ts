...
type SimRow = { id: string; segment_id: string; type: string; similarity: string };
...
export class EmbeddingStoreService implements OnModuleDestroy {
  ...
  async upsertContentEmbedding(segmentId: string, vector: number[]): Promise<EmbeddingRecord> {
  ...
  async searchByVector(vecStr: string, limit: number, minSimilarity = 0): Promise<SearchResult[]> {
    if (minSimilarity > 0) {
