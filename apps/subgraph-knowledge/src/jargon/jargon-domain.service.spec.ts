...
vi.mock('@edusphere/db', () => ({
  ...
  schema: {
    jargon_domains: {
      ...
      language: 'language', deleted_at: 'deleted_at',
    },
    jargon_terms: { domain_id: 'domain_id', deleted_at: 'deleted_at' },
    domain_proximity: {
      tenant_id: 'tenant_id', domain_a_id: 'domain_a_id',
      domain_b_id: 'domain_b_id', proximity_score: 'proximity_score',
    },
  },
  eq: vi.fn((a, b) => ({ eq: [a, b] })),
...
describe('JargonDomainService', () => {
  ...
  describe('listDomains', () => {
    it('returns all active domains for a tenant', async () => {
      ...
      const rows = (await mockDb.select().from({}).where({}).orderBy({})) as JargonDomain[];
  ...
  describe('countTerms', () => {
    it('returns term count for a domain', async () => {
      ...
      const rows = (await mockDb.select().from({}).where({}).orderBy({})) as Array<{ count: number }>;
  ...
  describe('createDomain', () => {
    it('creates a domain and returns it', async () => {
      ...
      const rows = (await mockDb.insert({}).values(input).returning()) as JargonDomain[];
      expect(rows[0].name).toBe('קבלה');
    ...
    it('defaults language to "he" when not provided', async () => {
      ...
      const rows = (await mockDb.insert({}).values({}).returning()) as JargonDomain[];
      expect(rows[0].language).toBe('he');
    });
    ...
    it('throws ConflictException on duplicate domain name for tenant', async () => {
      ...
      await expect(async () => {
        ...
        if (err.code === '23505') throw new ConflictException('Domain already exists');
      }).rejects.toThrow(ConflictException);
    });

    it('creates domain with parent_domain_id when parentDomainId provided', async () => {
      const child: JargonDomain = { ...DOMAIN_TALMUD, parent_domain_id: DOMAIN_KABBALAH.id };
  ...
  describe('detectDomains', () => {
    it('delegates detection to JargonDomainDetectService', async () => {
      ...
      const allDomains = (await mockDb.select().from({}).where({}).orderBy({})) as JargonDomain[];
      const results = await mockDetectService.detectDomains('ספירות עשר...', allDomains, TENANT_ID);
      expect(mockDetectDomains).toHaveBeenCalledWith('ספירות עשר...', allDomains, TENANT_ID);
    ...
    it('returns empty array when no domains configured for tenant', async () => {
      ...
      const results = await mockDetectService.detectDomains('any text', [], TENANT_ID);
  ...
  describe('computeProximity', () => {
    it('skips computation when fewer than 2 domains', async () => {
      buildSelectChain([DOMAIN_KABBALAH]);
      const domains = (await mockDb.select().from({}).where({}).orderBy({})) as JargonDomain[];
      if (domains.length < 2) {
    ...
    it('computes proximity for each unique domain pair', async () => {
      ...
      buildInsertChain([]);
      await mockDetectService.computeProximityPair(DOMAIN_KABBALAH.id, DOMAIN_TALMUD.id);
      expect(mockComputeProximityPair).toHaveBeenCalledWith(DOMAIN_KABBALAH.id, DOMAIN_TALMUD.id);
    });
  });

  describe('getRelatedDomains', () => {
    it('returns related domains sorted by descending proximity score', async () => {
      buildSelectChain([{ domain_b_id: DOMAIN_TALMUD.id, proximity_score: '0.82' }]);
  ...
  describe('error handling', () => {
    it('propagates unexpected DB errors', async () => {
      ...
      await expect(mockDb.insert({}).values({}).returning()).rejects.toThrow('connection refused');
    });
  });
});
