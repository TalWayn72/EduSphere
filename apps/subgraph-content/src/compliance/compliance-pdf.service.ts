...
export class CompliancePdfService {
  ...
  generatePdf(input: CompliancePdfInput): Promise<Buffer> {
    this.logger.debug(`Generating compliance PDF: tenant=${input.tenantName} rows=${input.rows.length}`);
  ...
  private drawHeader(doc: PDFKit.PDFDocument, input: CompliancePdfInput): void {
    ...
    doc.fontSize(10).fillColor('#555555')
      .text(`As of: ${input.asOf.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, { align: 'center' });
    doc.moveDown(0.8);
    doc.fillColor('#000000');
  }

  private drawSummary(doc: PDFKit.PDFDocument, s: CompliancePdfInput['summary']): void {
  ...
  private drawTable(doc: PDFKit.PDFDocument, rows: ComplianceReportRow[]): void {
    const colWidths = [100, 120, 80, 110, 60, 60, 40, 70, 65];
    const headers = ['Name', 'Email', 'Department', 'Course', 'Enrolled', 'Completed', 'Score', 'Status', 'Due Date'];
  ...
  private drawRow(doc: PDFKit.PDFDocument, cells: string[], widths: number[], isHeader: boolean): void {
    ...
    if (isHeader) {
      doc.rect(startX, y, widths.reduce((a, b) => a + b, 0), rowHeight).fill('#e8e8e8').fillColor('#000000');
    }

    cells.forEach((cell, i) => {
      doc.text(cell.slice(0, 20), x + 2, y + 2, { width: (widths[i] ?? 60) - 4, lineBreak: false });
      x += widths[i] ?? 60;
    });
    doc.moveDown();
  ...
  private shadeRow(doc: PDFKit.PDFDocument, widths: number[]): void {
    const startX = doc.page.margins.left;
    doc.rect(startX, doc.y, widths.reduce((a, b) => a + b, 0), 14).fill('#f9f9f9').fillColor('#000000');
  }

  private drawFooter(doc: PDFKit.PDFDocument, tenantName: string): void {
    doc.moveDown(1);
    doc.fontSize(8).fillColor('#888888')
      .text(`Tenant: ${tenantName} | Generated: ${new Date().toISOString()} | EduSphere Platform`, { align: 'center' });
  }
}
