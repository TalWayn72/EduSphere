...
describe('QuizGraderService', () => {
  ...
  beforeEach(() => { grader = new QuizGraderService(); });

  it('MULTIPLE_CHOICE: correct answer returns correct=true, score 100%', () => {
    const quiz: QuizContent = {
      items: [{ type: 'MULTIPLE_CHOICE', question: 'Q?',
        options: [{ id: 'a', text: '3' }, { id: 'b', text: '4' }],
        correctOptionIds: ['b'] }],
      randomizeOrder: false, showExplanations: true, passingScore: 70,
    };
  ...
  it('MULTIPLE_CHOICE: wrong answer returns correct=false', () => {
    const quiz: QuizContent = {
      items: [{ type: 'MULTIPLE_CHOICE', question: 'Q?',
        ...
        correctOptionIds: ['b'] }],
      randomizeOrder: false, showExplanations: false, passingScore: 70,
    };
  ...
  it('DRAG_ORDER: correct order returns correct=true', () => {
    const quiz: QuizContent = {
      items: [{ type: 'DRAG_ORDER', question: 'Order',
  ...
  it('MATCHING: all correct pairs gives 100% score', () => {
    const quiz: QuizContent = {
      items: [{ type: 'MATCHING', question: 'Match',
        leftItems: [{ id: 'l1', text: 'Dog' }, { id: 'l2', text: 'Cat' }],
        rightItems: [{ id: 'r1', text: 'Bark' }, { id: 'r2', text: 'Meow' }],
        correctPairs: [{ leftId: 'l1', rightId: 'r1' }, { leftId: 'l2', rightId: 'r2' }] }],
      randomizeOrder: false, showExplanations: false, passingScore: 70,
    };
    const result = grader.grade(quiz, { 0: [{ leftId: 'l1', rightId: 'r1' }, { leftId: 'l2', rightId: 'r2' }] });
    expect(result.itemResults[0]?.correct).toBe(true);
  ...
  it('FILL_BLANK: exact match case insensitive returns correct=true', () => {
    const quiz: QuizContent = {
      items: [{ type: 'FILL_BLANK', question: 'Capital',
        correctAnswer: 'Paris', useSemanticMatching: false, similarityThreshold: 0.85 }],
      randomizeOrder: false, showExplanations: false, passingScore: 70,
    };
  ...
  it('LIKERT: always returns correct=true regardless of value', () => {
    const quiz: QuizContent = {
      ...
      randomizeOrder: false, showExplanations: false, passingScore: 0,
    };
  ...
  it('passes when score meets passingScore threshold', () => {
    const quiz: QuizContent = {
      ...
      randomizeOrder: false, showExplanations: false, passingScore: 50,
    };
