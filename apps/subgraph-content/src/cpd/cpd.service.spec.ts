...
const { mockCloseAllPools, mockWithTenantContext, mockNatsConnect, mockInsert, mockSelect } = vi.hoisted(() => {
  ...
  return { mockCloseAllPools, mockWithTenantContext, mockNatsConnect, mockInsert, mockSelect };
});
...
vi.mock('@edusphere/db', () => ({
  ...
  schema: {
    courseCpdCredits: { courseId: 'courseId', tenantId: 'tenantId', creditTypeId: 'creditTypeId', creditHours: 'creditHours' },
    userCpdLog: { userId: 'userId', tenantId: 'tenantId', creditTypeId: 'creditTypeId', completionDate: 'completionDate' },
    cpdCreditTypes: { id: 'id', tenantId: 'tenantId', name: 'name', regulatoryBody: 'regulatoryBody', isActive: 'isActive' },
  },
  eq: vi.fn((_a, _b) => 'eq'),
  and: vi.fn((...args) => args),
  gte: vi.fn((_a, _b) => 'gte'),
...
const mockGenerateReport = vi.fn().mockResolvedValue('https://presigned-url/report.csv');
vi.mock('./cpd-export.service.js', () => ({
  CpdExportService: vi.fn().mockImplementation(function CpdExportServiceCtor() { return { generateReport: mockGenerateReport }; }),
}));
...
const sampleLogRows = [
  { id: 'log-1', courseId: 'course-1', earnedHours: '2.50', completionDate: new Date('2025-01-15'), creditTypeName: 'NASBA CPE', regulatoryBody: 'NASBA' },
  { id: 'log-2', courseId: 'course-2', earnedHours: '1.00', completionDate: new Date('2025-02-10'), creditTypeName: 'NASBA CPE', regulatoryBody: 'NASBA' },
];
...
describe('CpdService', () => {
  ...
  it('test 1: NATS consumer processes course.completed and creates CPD log entry', async () => {
    ...
    mockWithTenantContext
      .mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
        fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(credits) }) }) }),
      )
      .mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
        fn({ insert: vi.fn().mockReturnValue({ values: vi.fn().mockResolvedValue([]) }) }),
      );

    const svc = makeService();
    const payload = { courseId: 'c-1', userId: 'u-1', tenantId: 't-1', completionDate: new Date().toISOString() };

    // Access private method via type assertion for testing
    await (svc as unknown as { handleCourseCompleted: (p: typeof payload) => Promise<void> }).handleCourseCompleted(payload);
  ...
  it('test 2: getUserCpdReport aggregates hours by credit type', async () => {
    mockWithTenantContext.mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
      fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ innerJoin: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(sampleLogRows) }) }) }) }),
    );
  ...
  it('test 3: getUserCpdReport filters by date range', async () => {
    ...
    mockWithTenantContext.mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
      fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ innerJoin: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(filteredRows) }) }) }) }),
    );
  ...
  it('test 4: getUserCpdReport returns empty when no credits assigned', async () => {
    mockWithTenantContext.mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
      fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ innerJoin: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([]) }) }) }) }),
    );
  ...
  it('test 5: exportCpdReport calls generateReport for CSV format', async () => {
    ...
    expect(mockGenerateReport).toHaveBeenCalledWith(expect.any(Object), 'user-1', 'tenant-1', 'CSV');
  ...
  it('test 6: exportCpdReport calls generateReport for NASBA format', async () => {
    ...
    expect(mockGenerateReport).toHaveBeenCalledWith(expect.any(Object), 'user-1', 'tenant-1', 'NASBA');
    expect(url).toBe('https://presigned-url/report.csv');
  });
  ...
  it('test 8: listCreditTypes returns only active types', async () => {
    const activeCreditTypes = [
      { id: 'ct-1', name: 'NASBA CPE', regulatoryBody: 'NASBA', creditHoursPerHour: '1.00', isActive: true },
    ];
    mockWithTenantContext.mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
      fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(activeCreditTypes) }) }) }),
    );
