...
export class CpdExportService {
  ...
  async generateReport(
    ...
    this.logger.log({ userId, tenantId, format, reportId }, 'CPD report generated');
    return url;
  }

  private buildCsvReport(data: CpdReport, format: string): string {
    const title = sanitizeCsvCell(`${format} CPD Report — Total Hours: ${data.totalHours.toFixed(2)}`);
    const generated = sanitizeCsvCell(`Generated: ${new Date().toISOString()}`);
    const headers = ['Course ID', 'Credit Type', 'Earned Hours', 'Completion Date'].join(',');
  ...
  private buildPdfReport(
    ...
    return new Promise((resolve, reject) => {
      ...
      const formatLabel = format === 'NASBA'
      ...
      data.byType.forEach((t) => {
        doc.text(`  ${t.name} (${t.regulatoryBody}): ${t.totalHours.toFixed(2)} hrs`);
      });
      doc.moveDown(0.8);
      ...
      this.drawPdfRow(doc, ['Credit Type', 'Hours', 'Completion Date'], [180, 60, 130], true);
      doc.font('Helvetica');
      data.entries.forEach((e: CpdLogEntry, i: number) => {
        if (i % 2 === 0) {
          doc.rect(doc.page.margins.left, doc.y, 370, 14).fill('#f9f9f9').fillColor('#000000');
        }
        this.drawPdfRow(doc, [
          e.creditTypeName,
      ...
      doc.fontSize(8).fillColor('#888888')
        .text(`EduSphere Platform | ${format} Regulatory Report`, { align: 'center' });
      doc.end();
    });
  }

  private drawPdfRow(doc: PDFKit.PDFDocument, cells: string[], widths: number[], isHeader: boolean): void {
    ...
    cells.forEach((cell, i) => {
      doc.text(cell.slice(0, 28), x + 2, y + 2, { width: (widths[i] ?? 60) - 4, lineBreak: false });
      x += widths[i] ?? 60;
    });
    doc.moveDown();
