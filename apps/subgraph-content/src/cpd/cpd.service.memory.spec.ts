...
vi.mock('./cpd-export.service.js', () => ({
  CpdExportService: vi.fn().mockImplementation(function CpdExportServiceCtor() { return { generateReport: vi.fn().mockResolvedValue('https://url') }; }),
}));
  ...
  it('test 3: concurrent completions handled without duplicate CPD entries per insert', async () => {
    ...
    vi.mocked(withTenantContext)
      ...
      .mockImplementationOnce(async (_d: unknown, _c: unknown, fn: (tx: unknown) => unknown) =>
        fn({ select: vi.fn().mockReturnValue({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([{ creditTypeId: 'ct-1', creditHours: '2.00' }]) }) }) }),
      )
    ...
    await Promise.all([
      (svc as unknown as { handleCourseCompleted: (p: typeof payload) => Promise<void> }).handleCourseCompleted(payload),
      (svc as unknown as { handleCourseCompleted: (p: typeof payload) => Promise<void> }).handleCourseCompleted(payload),
    ]);
