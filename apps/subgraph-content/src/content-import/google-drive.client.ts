...
export class GoogleDriveClient {
  ...
  async listFolderContents(
    ...
    if (res.status === 403) {
      throw new BadRequestException(
        'Insufficient Google Drive permissions. Please reconnect your account.'
      );
    }
    if (!res.ok) {
  ...
  async downloadFile(fileId: string, accessToken: string): Promise<Buffer> {
    ...
    if (!res.ok) {
      throw new BadRequestException(
        `Failed to download file ${fileId}: ${res.status}`
      );
    }
  ...
  async exchangeCode(code: string): Promise<string> {
    ...
    if (!data.access_token)
      throw new BadRequestException('No access_token in Google OAuth response');
