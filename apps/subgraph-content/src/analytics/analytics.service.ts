...
import type { ContentItemMetric, FunnelStep, CourseAnalytics } from './analytics.types.js';
...
export class AnalyticsService implements OnModuleDestroy {
  ...
  async getCourseAnalytics(
    ...
    this.logger.log(`getCourseAnalytics courseId=${courseId} tenant=${ctx.tenantId}`);

    const [enrollmentCount, activeCount, completionRate, contentMetrics, funnel] =
      await Promise.all([
  ...
  private async getEnrollmentCount(courseId: string, ctx: TenantContext): Promise<number> {
  ...
  private async getActiveLearners(courseId: string, ctx: TenantContext): Promise<number> {
  ...
  private async getCompletionRate(courseId: string, ctx: TenantContext): Promise<number> {
    return withTenantContext(this.db, ctx, async (tx) => {
  ...
  private async getContentItemMetrics(courseId: string, ctx: TenantContext): Promise<ContentItemMetric[]> {
    return withTenantContext(this.db, ctx, async (tx) => {
      const rows = await tx
        .select({
        ...
        .leftJoin(schema.userProgress, eq(schema.userProgress.contentItemId, schema.contentItems.id))
      ...
      return rows.map((r) => {
        ...
        return {
          ...
          completionRate: viewCount > 0 ? Math.round((completedCount / viewCount) * 1000) / 10 : 0,
        };
      });
    });
  }

  private async getDropOffFunnel(courseId: string, ctx: TenantContext): Promise<FunnelStep[]> {
    return withTenantContext(this.db, ctx, async (tx) => {
      ...
      for (const mod of modRows) {
        const [startedRow] = await tx
          ...
          .innerJoin(schema.contentItems, eq(schema.userProgress.contentItemId, schema.contentItems.id))
        ...
        steps.push({
          ...
          dropOffRate: started > 0 ? Math.round(((started - completed) / started) * 1000) / 10 : 0,
        });
      }
      return steps;
    });
