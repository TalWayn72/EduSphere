...
function setupDbChain(returnValue: unknown) {
  ...
  const thenableWhere = Object.assign(Promise.resolve(arr), {
...
describe('LtiService', () => {
  ...
  it('initiateLogin generates valid redirect URL with all required params', async () => {
    ...
    const result = await service.initiateLogin({
      ...
      login_hint: 'user123',
      target_link_uri: 'https://edusphere.com/lti/launch',
      client_id: 'client-123',
    });
  ...
  it('initiateLogin throws NotFoundException for unknown platform', async () => {
    ...
    await expect(
      service.initiateLogin({
        ...
        login_hint: 'u1',
        target_link_uri: 'https://edusphere.com',
      })
  ...
  it('handleCallback rejects expired token when jwtVerify throws', async () => {
    ...
    await expect(service.handleCallback(token, 'state-1')).rejects.toThrow(
  ...
  it('handleCallback rejects mismatched nonce', async () => {
    ...
    await expect(service.handleCallback(token, 'wrong-state')).rejects.toThrow(
      UnauthorizedException
    );
  });
  ...
  it('handleCallback accepts valid token and returns session', async () => {
    ...
    jwtVerifyMock.mockResolvedValueOnce({ payload: ltiClaims });
    const launchInsertReturning = vi
      .fn()
      .mockResolvedValue([{ id: 'launch-id-1' }]);
    const launchInsertValues = vi
      .fn()
      .mockReturnValue({ returning: launchInsertReturning });
    mockInsert
      .mockReturnValueOnce({ values: mockValues })
      .mockReturnValueOnce({ values: launchInsertValues });
    const { LtiService } = await import('./lti.service');
    const service = new LtiService();
    type WithStoreNonce = { storeNonce: (n: string, s: string) => void };
    (service as unknown as WithStoreNonce).storeNonce(
      'valid-nonce-001',
      'state-for-test'
    );
    const token = makeValidJwt(ltiClaims);
  ...
  it('toggleLtiPlatform updates isActive field', async () => {
    ...
    const { LtiService } = await import('./lti.service');
    const service = new LtiService();
    const dto = await service.togglePlatform(
      'platform-uuid',
      'tenant-uuid',
      false
    );
