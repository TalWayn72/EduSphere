...
export class AtRiskService implements OnModuleDestroy {
  ...
  async runNightlyDetection(): Promise<void> {
    this.logger.log({ action: 'AT_RISK_CRON_START' }, 'At-risk detection cron started');
    try {
  ...
  private async detectForTenant(tenantId: string): Promise<void> {
    ...
    const enrollments = await withTenantContext(this.db, ctx, async (tx) =>
      tx
        ...
        .innerJoin(schema.courses, eq(schema.courses.id, schema.userCourses.courseId))
        .where(
          and(
  ...
  private async processEnrollment(
    enrollment: { userId: string; courseId: string; enrolledAt: Date; estimatedHours: number | null },
    ctx: TenantContext,
    ...
    const existingFlag = await this.findActiveFlag(enrollment.userId, enrollment.courseId, ctx);
    if (isAtRisk && !existingFlag) {
      await this.createFlag(enrollment.userId, enrollment.courseId, ctx.tenantId, score, factors);
      await this.publishFlagEvent(enrollment.userId, enrollment.courseId, ctx.tenantId, score);
      return 'flagged';
    }
    if (!isAtRisk && existingFlag) {
  ...
  private async buildMetrics(
    ...
    const [progressRows, quizRows] = await Promise.all([
      withTenantContext(this.db, ctx, async (tx) =>
        tx.select({ lastAccessedAt: schema.userProgress.lastAccessedAt, isCompleted: schema.userProgress.isCompleted })
          .from(schema.userProgress)
          .innerJoin(schema.contentItems, eq(schema.contentItems.id, schema.userProgress.contentItemId))
          .innerJoin(schema.modules, eq(schema.modules.id, schema.contentItems.moduleId))
          .where(and(eq(schema.userProgress.userId, enrollment.userId), eq(schema.modules.course_id, enrollment.courseId))),
      ),
      withTenantContext(this.db, ctx, async (tx) =>
        tx.select({ passed: schema.quizResults.passed })
          .from(schema.quizResults)
          .innerJoin(schema.contentItems, eq(schema.contentItems.id, schema.quizResults.contentItemId))
          .innerJoin(schema.modules, eq(schema.modules.id, schema.contentItems.moduleId))
          .where(and(eq(schema.quizResults.userId, enrollment.userId), eq(schema.modules.course_id, enrollment.courseId))),
      ),
    ]);
    ...
    const daysSinceLastActivity = latestActivity
      ? Math.floor((now - latestActivity.getTime()) / 86400000) : 999;
    const estimatedDays = (enrollment.estimatedHours ?? 0) * 3;
    const elapsedDays = Math.floor((now - enrollment.enrolledAt.getTime()) / 86400000);
    const courseDaysRemaining = Math.max(0, estimatedDays - elapsedDays);
    const failedQuizzes = quizRows.filter((q) => !q.passed).length;
    const quizFailureRate = quizRows.length > 0 ? failedQuizzes / quizRows.length : 0;
    const activeInWindow = new Set(progressRows.filter((r) => r.lastAccessedAt >= sevenDaysAgo).map((r) => r.lastAccessedAt.toDateString())).size;
    return { daysSinceLastActivity, courseProgressPercent, courseDaysRemaining, quizFailureRate, weeklyActivityDays: activeInWindow };
  }

  private async findActiveFlag(learnerId: string, courseId: string, ctx: TenantContext) {
    const rows = await withTenantContext(this.db, ctx, async (tx) =>
      tx.select({ id: schema.atRiskFlags.id })
        .from(schema.atRiskFlags)
        .where(and(eq(schema.atRiskFlags.learnerId, learnerId), eq(schema.atRiskFlags.courseId, courseId), eq(schema.atRiskFlags.status, 'active')))
        .limit(1),
    );
    return rows[0] ?? null;
  }

  private async createFlag(learnerId: string, courseId: string, tenantId: string, riskScore: number, factors: object): Promise<void> {
    const sysCtx: TenantContext = { tenantId, userId: 'system', userRole: 'SUPER_ADMIN' };
    await withTenantContext(this.db, sysCtx, async (tx) =>
      tx.insert(schema.atRiskFlags).values({ learnerId, courseId, tenantId, riskScore, riskFactors: factors, status: 'active', flaggedAt: new Date() }),
    );
    this.logger.log({ learnerId, courseId, tenantId, riskScore }, 'Learner flagged as at-risk');
  }

  private async resolveFlag(flagId: string, ctx: TenantContext): Promise<void> {
    await withTenantContext(this.db, ctx, async (tx) =>
      tx.update(schema.atRiskFlags).set({ status: 'resolved', resolvedAt: new Date() }).where(eq(schema.atRiskFlags.id, flagId)),
    );
  }

  private async publishFlagEvent(learnerId: string, courseId: string, tenantId: string, riskScore: number): Promise<void> {
    try {
      if (!this.nc) {
        const natsUrl = process.env.NATS_URL ?? 'nats://localhost:4222';
        this.nc = await connect({ servers: natsUrl });
      }
      const payload = JSON.stringify({ learnerId, courseId, tenantId, riskScore, flaggedAt: new Date().toISOString() });
      this.nc.publish(NATS_SUBJECT, this.sc.encode(payload));
    } catch (err) {
      this.logger.warn({ learnerId, courseId, err }, 'Failed to publish at-risk NATS event');
    }
  }

  async getAtRiskLearners(courseId: string, ctx: TenantContext): Promise<AtRiskLearner[]> {
    const rows = await withTenantContext(this.db, ctx, async (tx) =>
      tx.select().from(schema.atRiskFlags)
        .where(and(eq(schema.atRiskFlags.courseId, courseId), eq(schema.atRiskFlags.status, 'active'))),
    );
    return rows.map((r) => ({
      id: r.id,
  ...
  async dismissFlag(flagId: string, ctx: TenantContext): Promise<boolean> {
    await withTenantContext(this.db, ctx, async (tx) =>
      tx.update(schema.atRiskFlags)
        .set({ status: 'dismissed', resolvedAt: new Date() })
        .where(and(eq(schema.atRiskFlags.id, flagId), eq(schema.atRiskFlags.tenantId, ctx.tenantId))),
    );
...
function toRiskFactorsList(factors: Record<string, boolean>): Array<{ key: string; description: string }> {
  ...
  return Object.entries(factors).filter(([, v]) => v).map(([key]) => ({
    key,
