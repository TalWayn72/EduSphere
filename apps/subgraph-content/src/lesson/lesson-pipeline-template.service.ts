...
const flatConfigSchema = z.record(
  z.string(),
...
export class LessonPipelineTemplateService {
  ...
  async findAll(tenantId: string) {
    const rows = await this.db
      .select()
      .from(this.tbl)
      .where(or(eq(this.tbl.tenant_id, tenantId), eq(this.tbl.is_system, true)));
  ...
  async create(input: CreatePipelineTemplateInput, tenantId: string, userId: string) {
    ...
    this.logger.log({ msg: 'Template created', tenantId, userId, templateId: row.id });
    return this.mapRow(row);
  }

  async update(id: string, input: UpdatePipelineTemplateInput, tenantId: string) {
    const existing = await this.findById(id);
    if (!existing) throw new NotFoundException(`Template "${id}" not found`);
    if (existing.isSystem) throw new BadRequestException('Cannot modify system templates');
    ...
    if (!updated) throw new NotFoundException(`Template "${id}" not found for tenant`);
  ...
  async delete(id: string, tenantId: string): Promise<boolean> {
    const existing = await this.findById(id);
    if (!existing) throw new NotFoundException(`Template "${id}" not found`);
    if (existing.isSystem) throw new BadRequestException('Cannot delete system templates');

    const deleted = await this.db
      .delete(this.tbl)
      .where(and(eq(this.tbl.id, id), eq(this.tbl.tenant_id, tenantId)))
      .returning();
    if (deleted.length === 0) throw new NotFoundException(`Template "${id}" not found for tenant`);
