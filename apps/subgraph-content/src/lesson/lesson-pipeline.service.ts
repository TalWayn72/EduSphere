...
import { schema, eq, and } from '@edusphere/db';
...
export class LessonPipelineService implements OnModuleDestroy {
  ...
  findByLesson(lessonId: string) {
    return this.queryService.findByLesson(lessonId);
  }
  findRunById(runId: string) {
    return this.queryService.findRunById(runId);
  }
  findCurrentRunByPipeline(pipelineId: string) {
    return this.queryService.findCurrentRunByPipeline(pipelineId);
  }
  findResultsByRunId(runId: string) {
    return this.queryService.findResultsByRunId(runId);
  }
  getNextRunNumber(lessonId: string) {
    return this.queryService.getNextRunNumber(lessonId);
  }
  findRunHistory(lessonId: string, limit = 10) {
    return this.queryService.findRunHistory(lessonId, limit);
  }
  ...
  async restoreRun(runId: string, tenantCtx: TenantContext) {
    ...
    const nextRun = lessonId
      ? await this.queryService.getNextRunNumber(lessonId)
      : 1;
  ...
  async startRun(pipelineId: string, tenantCtx: TenantContext) {
    ...
    const runNumber = lessonIdForRun
      ? await this.queryService.getNextRunNumber(lessonIdForRun)
      : 1;
