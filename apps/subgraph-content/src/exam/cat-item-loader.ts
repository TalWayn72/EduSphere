...
import type { TenantContext, ExamItem, ExamSession, DrizzleDB } from '@edusphere/db';
...
export class CatItemLoader {
  ...
  async loadEligibleItems(
    courseId: string, excludeIds: string[], ctx: TenantContext,
  ): Promise<ExamItem[]> {
    const all = await withTenantContext(this.db, ctx, (tx) =>
      tx.select().from(schema.examItems).where(
        and(
  ...
  async loadAdministeredItems(
    ids: string[], ctx: TenantContext,
  ): Promise<ExamItem[]> {
    if (ids.length === 0) return [];
    const all = await withTenantContext(this.db, ctx, (tx) =>
      tx.select().from(schema.examItems).where(isNull(schema.examItems.deletedAt)),
    );
    const idSet = new Set(ids);
    const map = new Map(all.filter((i) => idSet.has(i.id)).map((i) => [i.id, i]));
    return ids.map((id) => map.get(id)).filter(Boolean) as ExamItem[];
  }

  async saveCatState(
    sessionId: string, state: CatState, ctx: TenantContext,
  ): Promise<void> {
