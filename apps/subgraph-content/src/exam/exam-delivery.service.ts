...
export class ExamDeliveryService implements OnModuleDestroy {
  ...
  async startSession(
    blueprintId: string, userId: string, tenantId: string,
  ): Promise<ExamSession> {
    ...
    const assembly = await this.assemblyService.assembleExam(blueprintId, userId, tenantId);
    const attempts = await this.helpers.getAttemptCount(blueprintId, userId, ctx);

    const [session] = await withTenantContext(this.db, ctx, (tx) =>
      tx.insert(schema.examSessions).values({
        tenantId, blueprintId, userId,
        status: 'IN_PROGRESS',
    ...
    this.logger.log({ sessionId: session!.id, blueprintId, userId, tenantId }, 'Exam session started');
    return session!;
  }

  async submitAnswer(
    sessionId: string, itemId: string, answerData: unknown,
    ...
    const remaining = this.helpers.computeRemainingFromBlueprint(session, bp.timeLimitMinutes);
    ...
    await withTenantContext(this.db, ctx, async (tx) => {
      ...
      await tx.update(schema.examSessions)
  ...
  async flagQuestion(
    sessionId: string, itemId: string, userId: string, tenantId: string,
  ): Promise<boolean> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    const session = await this.helpers.loadSession(sessionId, ctx);
    this.helpers.assertActive(session, userId);
  ...
  async voidSession(sessionId: string, adminTenantId: string): Promise<boolean> {
    const ctx: TenantContext = { tenantId: adminTenantId, userId: 'system', userRole: 'ORG_ADMIN' };
    await withTenantContext(this.db, ctx, (tx) =>
      tx.update(schema.examSessions)
        .set({ status: 'VOIDED' })
        .where(eq(schema.examSessions.id, sessionId)),
