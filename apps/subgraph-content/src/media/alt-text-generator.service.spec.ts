...
const {
  ...
  const mockGenerateText = vi
    .fn()
    .mockResolvedValue({ text: 'A clear diagram' });
...
vi.mock('nats', () => ({
  ...
  StringCodec: vi.fn().mockReturnValue({
    decode: vi.fn((v: Uint8Array) => Buffer.from(v).toString()),
    encode: vi.fn((s: string) => Buffer.from(s)),
  }),
}));
vi.mock('@edusphere/db', () => ({
  createDatabaseConnection: vi.fn(() => ({ update: mockDbUpdate })),
...
vi.mock('@aws-sdk/client-s3', () => ({
  ...
  GetObjectCommand: vi.fn().mockImplementation(function (a) {
...
describe('AltTextGeneratorService', () => {
  ...
  it('skips non-image content types', async () => {
    await asP(service).handleMediaUploaded({
      ...IMG,
  ...
  it('handles generateText failure gracefully (non-fatal)', async () => {
    ...
    await expect(
      asP(service).handleMediaUploaded(IMG)
