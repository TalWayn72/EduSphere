...
const { mockUnsubscribe, mockDrain, mockConnect, mockCloseAllPools } = vi.hoisted(() => {
  const mockUnsubscribe = vi.fn();
  const mockDrain = vi.fn().mockResolvedValue(undefined);
  const makeNc = () => ({
    subscribe: vi.fn().mockReturnValue({ unsubscribe: mockUnsubscribe, [Symbol.asyncIterator]: async function* () {} }),
    drain: mockDrain,
    close: vi.fn().mockResolvedValue(undefined),
  });
  const mockConnect = vi.fn().mockImplementation(() => Promise.resolve(makeNc()));
  const mockCloseAllPools = vi.fn().mockResolvedValue(undefined);
  return { mockUnsubscribe, mockDrain, mockConnect, mockCloseAllPools };
});

vi.mock("nats", () => ({ connect: mockConnect, StringCodec: vi.fn().mockReturnValue({ decode: vi.fn(), encode: vi.fn() }) }));
vi.mock("@edusphere/db", () => ({ createDatabaseConnection: vi.fn(() => ({ update: vi.fn() })), closeAllPools: mockCloseAllPools, schema: { media_assets: {} } }));
...
vi.mock("@aws-sdk/client-s3", () => ({ S3Client: vi.fn().mockImplementation(function() { return {}; }), GetObjectCommand: vi.fn().mockImplementation((a: unknown) => a) }));
...
describe("AltTextGeneratorService — memory safety", () => {
  beforeEach(() => { vi.clearAllMocks(); mockDrain.mockResolvedValue(undefined); });
