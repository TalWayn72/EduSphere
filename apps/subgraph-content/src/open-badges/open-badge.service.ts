...
export class OpenBadgeService implements OnModuleInit, OnModuleDestroy {
    ...
    return withTenantContext(this.queryService.db, ctx, async (tx) => {
      ...
      if (!assertion)
        throw new InternalServerErrorException(
          'Assertion insert returned no record'
        );
      this.logger.log(
        { assertionId: assertion.id, userId: input.userId },
        'OpenBadge issued'
      );
      return this.queryService.mapAssertion(
        assertion,
        def.name,
        def.description
      );
  ...
  async verifyCredential(assertionId: string): Promise<VerificationResult> {
    ...
    const def = await this.queryService.getDefinitionById(
    ...
    const proof =
      assertion.proof as unknown as import('./open-badge.types.js').OpenBadgeProof;
    ...
    return {
      valid: true,
      assertion: this.queryService.mapAssertion(
        assertion,
        def.name,
        def.description
      ),
  ...
  async createBadgeDefinition(
    ...
    return withTenantContext(this.queryService.db, ctx, async (tx) => {
      ...
      if (!def)
        throw new InternalServerErrorException(
          'Badge definition insert returned no record'
        );
      this.logger.log(
  ...
  getAssertionById(assertionId: string) {
    return this.queryService.getAssertionById(assertionId);
  }
  getDefinitionById(definitionId: string) {
    return this.queryService.getDefinitionById(definitionId);
  }
