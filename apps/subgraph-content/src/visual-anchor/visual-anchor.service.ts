...
export class VisualAnchorService implements OnModuleDestroy {
  ...
  async createAnchor(
    ...
    const input = CreateVisualAnchorSchema.parse(
      rawInput
    ) as CreateVisualAnchorDto;
    ...
    this.logger.log(
      `[VisualAnchorService] Created anchor: id=${row.id} tenantId=${authCtx.tenantId}`
    );
  ...
  async updateAnchor(
    ...
    const input = UpdateVisualAnchorSchema.parse(
      rawInput
    ) as UpdateVisualAnchorDto;
    ...
    if (input.pageNumber !== undefined)
      setValues['page_number'] = input.pageNumber;
    ...
    if (input.posXEnd !== undefined)
      setValues['pos_x_end'] = input.posXEnd.toString();
    if (input.posYEnd !== undefined)
      setValues['pos_y_end'] = input.posYEnd.toString();
    if (input.documentOrder !== undefined)
      setValues['document_order'] = input.documentOrder;
    ...
    this.logger.log(
      `[VisualAnchorService] Updated anchor: id=${id} tenantId=${authCtx.tenantId}`
    );
  ...
  async deleteAnchor(id: string, authCtx: TenantContext): Promise<boolean> {
    ...
    const [deleted] = await withTenantContext(db, authCtx, (tx) =>
      tx
        ...
        .returning({
          id: schema.visualAnchors.id,
          media_asset_id: schema.visualAnchors.media_asset_id,
        })
    );
    ...
    this.logger.log(
      `[VisualAnchorService] Deleted anchor: id=${id} tenantId=${authCtx.tenantId}`
    );
    return true;
  }
  ...
  async assignAsset(
    ...
    const [anchor] = await withTenantContext(db, authCtx, (tx) =>
      tx
        ...
        .where(
          and(
            eq(schema.visualAnchors.id, anchorId),
            isNull(schema.visualAnchors.deleted_at)
          )
        )
    );
    if (!anchor)
      throw new NotFoundException(`Visual anchor ${anchorId} not found`);
    ...
    if (!mediaAsset)
      throw new NotFoundException(
        `Media asset ${anchor.media_asset_id} not found`
      );
    const [asset] = await withTenantContext(db, authCtx, (tx) =>
      tx
        ...
        .where(
          and(
            eq(schema.visualAssets.id, visualAssetId),
            isNull(schema.visualAssets.deleted_at)
          )
        )
    );
    if (!asset)
      throw new NotFoundException(`Visual asset ${visualAssetId} not found`);
    if (asset.course_id !== mediaAsset.course_id) {
      this.logger.warn(
        `[VisualAnchorService] Cross-course assignment blocked: anchorId=${anchorId} assetId=${visualAssetId}`
      );
      throw new BadRequestException(
        'Asset and anchor must belong to the same course'
      );
    }
    const [updated] = await withTenantContext(db, authCtx, (tx) =>
      tx
  ...
  async syncAnchors(
    ...
    if (!allowedRoles.includes(authCtx.userRole)) {
      this.logger.warn(
        `[VisualAnchorService] syncAnchors denied: userId=${authCtx.userId} role=${authCtx.userRole}`
      );
      throw new ForbiddenException(
        'Only instructors and admins can sync anchors'
      );
    }
    ...
    this.logger.log(
      `[VisualAnchorService] syncAnchors: mediaAssetId=${mediaAssetId} synced=${synced} broken=${broken}`
    );
  ...
  async confirmVisualAssetUpload(
    ...
    return runVisualAssetUploadPipeline({
      ...
      declaredSize,
      authCtx,
      db: this.queryService.db,
      s3: this.queryService.s3,
  ...
  private async publishNats(subject: string, payload: object): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `[VisualAnchorService] NATS publish failed for ${subject}: ${String(err)}`
      );
    } finally {
