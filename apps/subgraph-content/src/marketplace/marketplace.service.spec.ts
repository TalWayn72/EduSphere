...
vi.mock('nats', () => ({
  connect: vi.fn(() =>
    Promise.resolve({ publish: mockPublish, drain: vi.fn() })
  ),
  StringCodec: vi.fn(() => ({
    encode: vi.fn((s: string) => s),
    decode: vi.fn((s: string) => s),
  })),
}));
...
describe('MarketplaceService', () => {
  it('1. purchaseCourse creates Stripe PaymentIntent with correct amount', async () => {
    ...
    const result = await svc.purchaseCourse(
      COURSE_ID,
      USER_ID,
      TENANT_ID,
      'user@test.com',
  ...
  it('7. getInstructorEarnings delegates to earnings service', async () => {
    ...
    expect(mockEarningsService.getInstructorEarnings).toHaveBeenCalledWith(
      'instructor-1',
      TENANT_ID
    );
  });
