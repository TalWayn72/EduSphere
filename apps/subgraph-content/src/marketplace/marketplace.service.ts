...
import type { PurchaseResult, EarningsSummary, CourseListing, Purchase, InstructorPayout } from './marketplace.types.js';
...
export class MarketplaceService implements OnModuleDestroy {
  ...
  async createListing(
    ...
    const [listing] = await withTenantContext(this.db, ctx, async (tx) =>
      tx.insert(schema.courseListings)
        .values({ courseId, tenantId, priceCents, currency: validCurrency, revenueSplitPercent: split })
        .returning(),
    );
    this.logger.log({ courseId, priceCents, currency, tenantId }, 'Course listing created');
    return listing!;
  }
  ...
  async publishListing(courseId: string, tenantId: string): Promise<void> {
    ...
    await withTenantContext(this.db, ctx, async (tx) =>
      tx.update(schema.courseListings)
  ...
  async purchaseCourse(
    ...
    if (!listing) throw new BadRequestException('Course listing not found or not published');

    const stripeCustomer = await this.getOrCreateStripeCustomer(userId, tenantId, userEmail, userName);
    const intent = await this.stripeClient.createPaymentIntent(listing.priceCents, listing.currency, stripeCustomer);
    ...
    this.logger.log({ courseId, userId, tenantId, intentId: intent.id }, 'Purchase initiated');
  ...
  private async getOrCreateStripeCustomer(
    ...
    await withTenantContext(this.db, ctx, async (tx) =>
      tx.insert(schema.stripeCustomers).values({ userId, tenantId, stripeCustomerId: customer.id }),
    );
    return customer.id;
  }
  ...
  async processWebhook(event: Stripe.Event, tenantId: string): Promise<void> {
    ...
    if (event.type === 'payment_intent.succeeded') {
      ...
      this.logger.log({ intentId: intent.id, tenantId }, 'Purchase marked COMPLETE');
      ...
      this.logger.warn({ intentId: intent.id, tenantId }, 'Purchase marked FAILED');
    }
  }
  ...
  private async publishEnrollmentEvent(
    ...
    try {
      ...
      nc.publish(COURSE_ENROLLED_SUBJECT, this.sc.encode(JSON.stringify(payload)));
      this.logger.log({ courseId: purchase.courseId, userId: purchase.userId }, 'Enrollment event published');
    } catch (err) {
  ...
  async getUserPurchases(userId: string, tenantId: string): Promise<Purchase[]> {
    const ctx: TenantContext = { tenantId, userId, userRole: 'STUDENT' };
    return withTenantContext(this.db, ctx, async (tx) =>
      tx.select()
        .from(schema.purchases)
  ...
  async getInstructorEarnings(instructorId: string, tenantId: string): Promise<EarningsSummary> {
    return this.earningsService.getInstructorEarnings(instructorId, tenantId);
  }

  async requestPayout(instructorId: string, tenantId: string): Promise<InstructorPayout> {
    return this.earningsService.requestPayout(instructorId, tenantId, this.stripeClient);
  }
}
