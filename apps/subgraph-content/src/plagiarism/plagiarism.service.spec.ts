...
const { mockCloseAllPools, mockExecute, mockDbSelect, mockNatsConnect } =
  vi.hoisted(() => {
...
function makeSelectChain(data) {
  return {
    from: vi.fn().mockReturnValue({
      where: vi
        .fn()
        .mockReturnValue({ limit: vi.fn().mockResolvedValue(data) }),
    }),
  };
}
...
function makeTx(selectData, updateSetMock) {
  const setMock =
    updateSetMock ??
    vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([]) });
  return {
    execute: mockExecute,
    select: vi.fn().mockReturnValue(makeSelectChain(selectData)),
...
describe('PlagiarismService', () => {
  ...
  it('flags submission when highest similarity >= threshold (0.85)', async () => {
    ...
    mockDbSelect.mockReturnValue(
      makeSelectChain([{ settings: { plagiarism_threshold: 0.85 } }])
    );
    const setMock = vi
      .fn()
      .mockReturnValue({ where: vi.fn().mockResolvedValue([]) });
    vi.mocked(withTenantContext)
      .mockImplementationOnce(async (_d, _c, fn) =>
        fn(
          makeTx(
            [{ id: 'sub-1', textContent: 'text', isFlagged: false }],
            setMock
          )
        )
      )
  ...
  it('respects custom threshold of 0.65 from tenant settings', async () => {
    mockExecute
      ...
      .mockResolvedValueOnce([]);
    mockDbSelect.mockReturnValue(
      makeSelectChain([{ settings: { plagiarism_threshold: 0.65 } }])
    );
    const setMock = vi
      .fn()
      .mockReturnValue({ where: vi.fn().mockResolvedValue([]) });
    vi.mocked(withTenantContext)
      .mockImplementationOnce(async (_d, _c, fn) =>
        fn(
          makeTx([{ id: 'sub-1', textContent: 't', isFlagged: false }], setMock)
        )
      )
      .mockImplementationOnce(async (_d, _c, fn) => fn(makeTx([], setMock)))
      .mockImplementationOnce(async (_d, _c, fn) =>
  ...
  it('does not throw when submission is not found', async () => {
    ...
    const svc = new PlagiarismService(makeEmbeddingClient(ZERO_VEC));
    await expect(
      svc.processSubmission('missing', 'tenant-1', 'course-1')
  ...
  it('does not throw when embedding provider fails', async () => {
    ...
    await expect(
      svc.processSubmission('sub-1', 'tenant-1', 'course-1')
    ).resolves.toBeUndefined();
  });
});
