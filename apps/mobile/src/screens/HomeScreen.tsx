...
const HOME_QUERY = gql`
  query HomeData {
    ...
    myCourses(first: 3) { edges { node { id title progress lastAccessedAt } } }
  }
`;
...
type CourseItem = { id: string; title: string; progress: number; lastAccessed?: string; lastAccessedAt?: string; };
...
export default function HomeScreen() {
  ...
  const recentCourses: CourseItem[] = DEV_MODE
    ? MOCK_RECENT_COURSES
    : ((data?.myCourses?.edges ?? []) as Array<{ node: CourseItem }>).map((e) => e.node);
  return (
      ...
      <View style={styles.header}>
        <View>
          ...
          {loading && !DEV_MODE ? (
            ...
            <Text style={styles.userName}>{user?.firstName ?? 'Student'} {user?.lastName ?? ''}</Text>
          )}
        </View>
      ...
      {recentCourses.map((course) => (
        <TouchableOpacity key={course.id} style={styles.courseCard} onPress={() => navigation.navigate('CourseDetail', { courseId: course.id })}>
          <View style={styles.courseInfo}>
            ...
            <Text style={styles.courseAccessed}>{formatRelativeTime(course.lastAccessed ?? course.lastAccessedAt ?? '')}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 16 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, backgroundColor: 'white', padding: 20, borderRadius: 16 },
  greeting: { fontSize: 14, color: '#666' },
  userName: { fontSize: 24, fontWeight: 'bold', color: '#1a1a1a' },
  devBadge: { backgroundColor: '#FF9500', borderRadius: 4, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  statCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, width: '47%', borderTopWidth: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  statValue: { fontSize: 28, fontWeight: 'bold', marginBottom: 4 },
  statUnit: { fontSize: 14, fontWeight: 'normal' },
  statLabel: { fontSize: 13, color: '#666' },
  courseCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  courseInfo: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  courseTitle: { fontSize: 15, fontWeight: '600', flex: 1, marginRight: 8 },
  courseAccessed: { fontSize: 12, color: '#999' },
  progressContainer: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  progressBar: { flex: 1, height: 6, backgroundColor: '#f0f0f0', borderRadius: 3, overflow: 'hidden' },
