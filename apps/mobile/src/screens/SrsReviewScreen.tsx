  ...
  return (
    <View style={s.container}>
      ...
      <TouchableOpacity onPress={flipCard} activeOpacity={0.9}>
        <Animated.View
          style={[s.card, { transform: [{ rotateY: frontRotate }] }]}
        >
          <Text style={s.cardLabel}>Front</Text>
          <Text style={s.cardText}>{card.front}</Text>
        </Animated.View>
        <Animated.View
          style={[s.card, s.cardBack, { transform: [{ rotateY: backRotate }] }]}
        >
      ...
      {isFlipped && (
        <View style={s.ratingRow}>
          {(
            ...
            <TouchableOpacity
              key={label}
              style={[s.rateBtn, s[`rate${label}` as keyof typeof s] as object]}
