...
export function enqueue(mutation: Omit<QueuedMutation, 'retryCount'>): void {
  ...
  const count = (db.getFirstSync<{ c: number }>('SELECT COUNT(*) as c FROM offline_queue'))?.c ?? 0;
  if (count >= MAX_QUEUE_SIZE) {
    db.runSync('DELETE FROM offline_queue WHERE id = (SELECT id FROM offline_queue ORDER BY created_at ASC LIMIT 1)');
  }
  db.runSync(
    'INSERT OR REPLACE INTO offline_queue (id, operation_name, query, variables, tenant_id, user_id, created_at, retry_count) VALUES (?, ?, ?, ?, ?, ?, ?, 0)',
    [mutation.id, mutation.operationName, mutation.query, JSON.stringify(mutation.variables), mutation.tenantId, mutation.userId, mutation.createdAt],
  );
}
...
export function peek(limit = 10): QueuedMutation[] {
  const rows = getDb().getAllSync<{
    id: string; operation_name: string; query: string; variables: string;
    tenant_id: string; user_id: string; created_at: number; retry_count: number;
  }>('SELECT * FROM offline_queue ORDER BY created_at ASC LIMIT ?', [limit]);
  return rows.map((r) => ({
    id: r.id, operationName: r.operation_name, query: r.query,
    variables: JSON.parse(r.variables) as Record<string, unknown>,
    tenantId: r.tenant_id, userId: r.user_id, createdAt: r.created_at, retryCount: r.retry_count,
  }));
}

export function incrementRetry(id: string): void {
  getDb().runSync('UPDATE offline_queue SET retry_count = retry_count + 1 WHERE id = ?', [id]);
}

export function queueSize(): number {
  return (getDb().getFirstSync<{ c: number }>('SELECT COUNT(*) as c FROM offline_queue'))?.c ?? 0;
}
