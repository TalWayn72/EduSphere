...
describe('0012_live_sessions_enc_rename migration', () => {
  ...
  it('contains RENAME COLUMN for attendee_password to attendee_password_enc', () => {
    expect(sql).toMatch(
      /RENAME\s+COLUMN\s+attendee_password\s+TO\s+attendee_password_enc/i
    );
  });

  it('contains RENAME COLUMN for moderator_password to moderator_password_enc', () => {
    expect(sql).toMatch(
      /RENAME\s+COLUMN\s+moderator_password\s+TO\s+moderator_password_enc/i
    );
  });
  ...
  it('creates index on user_skill_mastery using correct column concept_id', () => {
    ...
    expect(sql).toMatch(
      /ON\s+user_skill_mastery\s*\(\s*user_id\s*,\s*concept_id\s*\)/i
    );
  });

  it('adds SI-3 comments on both encrypted columns', () => {
    expect(sql).toMatch(
      /COMMENT ON COLUMN live_sessions\.attendee_password_enc/i
    );
    expect(sql).toMatch(
      /COMMENT ON COLUMN live_sessions\.moderator_password_enc/i
    );
