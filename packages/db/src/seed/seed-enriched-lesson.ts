...
async function seedEnrichedLesson(): Promise<void> {
  ...
  await withBypassRLS(db, async (tx) => {
    ...
    await tx.insert(schema.visualAssets).values([
      {
        id: IDS.va1,
    ...
    await tx.insert(schema.media_assets).values({
      id: IDS.mediaAsset,
      tenant_id: DEMO_TENANT,
      course_id: COURSE_ID,
      module_id: MODULE_ID,
    ...
    await tx.insert(schema.lesson_assets).values({
      id: IDS.lessonAsset,
    ...
    await tx.insert(schema.transcript_segments).values(
      SEGMENTS.map((s) => ({
        id: IDS.seg(s.n),
    ...
    await tx.insert(schema.visualAnchors).values([
      {
        id: IDS.anchor1,
        tenant_id: DEMO_TENANT,
        media_asset_id: IDS.mediaAsset,
        ...
        page_number: null,
        pos_x: '0.1', pos_y: '0.2', pos_w: '0.8', pos_h: '0.05',
      ...
      },
      {
        ...
        page_number: null,
        pos_x: '0.1', pos_y: '0.3', pos_w: '0.8', pos_h: '0.05',
      ...
      },
      {
        ...
        page_number: null,
        pos_x: '0.1', pos_y: '0.4', pos_w: '0.8', pos_h: '0.05',
    ...
    await tx.insert(schema.lesson_citations).values([
      {
        id: IDS.cit1,
        lesson_id: IDS.lesson,
    ...
    await tx.insert(schema.enriched_transcript_blocks).values([
      ...
      },
      {
        ...
        content: { caption: 'תרשים הצמצום — חלל ורשימו', alt: 'Tzimtzum diagram' },
        start_time: '28.0',
        end_time: '80.0',
      },
      // CITATION: Etz Chaim quote about Tzimtzum
      {
        ...
        content: {
          quote: 'דע כי קודם שנאצלו הנאצלים ונבראו הנבראים היה אור עליון פשוט ממלא כל המציאות',
  ...
  console.log();
  console.log(`   Browser: http://localhost:5173/courses/${COURSE_ID}/lessons/${IDS.lesson}`);
}
