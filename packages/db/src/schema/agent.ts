import { pgTable, text, uuid, jsonb, boolean, timestamp } from 'drizzle-orm/pg-core';
...
export const agent_executions = pgTable('agent_executions', {
  id: pk(),
  agent_id: uuid('agent_id').notNull().references(() => agent_definitions.id, { onDelete: 'cascade' }),
