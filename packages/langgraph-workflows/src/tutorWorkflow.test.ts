...
import { AdaptiveTutorWorkflow, createTutorWorkflow, TutorState } from './tutorWorkflow';
...
describe('AdaptiveTutorWorkflow', () => {
  ...
  describe('initial state validation', () => {
    it('run() accepts minimal state with only question field', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('intermediate') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Explanation text') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Comprehension check question?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. Topic A\n2. Topic B\n3. Topic C') as never);

      const workflow = new AdaptiveTutorWorkflow();
      const result = await workflow.run({ question: 'What is photosynthesis?' });
    ...
    it('initial state has correct defaults', async () => {
      mockGenerateText
        ...
        .mockResolvedValueOnce(makeGenerateTextResponse('Simple explanation') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check question?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. Topic A\n2. Topic B\n3. Topic C') as never);
  ...
  describe('assessNode — student level detection', () => {
    it('sets studentLevel to "beginner" when LLM returns beginner', async () => {
      mockGenerateText
        ...
        .mockResolvedValueOnce(makeGenerateTextResponse('1. A\n2. B\n3. C') as never);
    ...
    it('sets studentLevel to "advanced" when LLM returns advanced', async () => {
      mockGenerateText
        ...
        .mockResolvedValueOnce(makeGenerateTextResponse('Deep explanation') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. A\n2. B\n3. C') as never);

      const workflow = new AdaptiveTutorWorkflow();
      const result = await workflow.run({ question: 'Explain Gödel incompleteness?' });
    ...
    it('defaults studentLevel to "intermediate" when LLM returns unknown text', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('   unknown_value   ') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Explanation') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. A\n2. B\n3. C') as never);
  ...
  describe('explainNode — generates explanation', () => {
    it('populates explanation field', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('intermediate') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('HTTP is a protocol...') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. REST\n2. WebSockets\n3. gRPC') as never);
    ...
    it('appends question and answer to conversationHistory', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('intermediate') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('My explanation here') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. X\n2. Y\n3. Z') as never);
    ...
    it('passes optional context to explain node', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('advanced') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Contextual explanation') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Comprehension Q?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. A\n2. B\n3. C') as never);
  ...
  describe('verifyNode — comprehension check', () => {
    it('populates comprehensionCheck field', async () => {
      mockGenerateText
        .mockResolvedValueOnce(makeGenerateTextResponse('intermediate') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Explanation text') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Can you describe the main cycle?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('1. A\n2. B\n3. C') as never);

      const workflow = new AdaptiveTutorWorkflow();
      const result = await workflow.run({ question: 'What is photosynthesis?' });

      expect(result.comprehensionCheck).toBe('Can you describe the main cycle?');
    });
  });
  ...
  describe('followupNode — follow-up suggestions', () => {
    it('parses numbered follow-up suggestions from LLM output', async () => {
      mockGenerateText
        ...
        .mockResolvedValueOnce(
          makeGenerateTextResponse('1. Topic A\n2. Topic B\n3. Topic C') as never
        );

      const workflow = new AdaptiveTutorWorkflow();
      const result = await workflow.run({ question: 'What is machine learning?' });
    ...
    it('handles LLM returning no numbered lines — yields empty suggestions', async () => {
      mockGenerateText
        ...
        .mockResolvedValueOnce(makeGenerateTextResponse('Explanation') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('Check?') as never)
        .mockResolvedValueOnce(makeGenerateTextResponse('No numbered list here.') as never);
  ...
  describe('error handling', () => {
    it('propagates LLM error from assessNode', async () => {
      mockGenerateText.mockRejectedValueOnce(new Error('LLM rate limit') as never);

      const workflow = new AdaptiveTutorWorkflow();
      await expect(workflow.run({ question: 'Will this fail?' })).rejects.toThrow('LLM rate limit');
    });
    ...
    it('propagates LLM error from explainNode', async () => {
      ...
      const workflow = new AdaptiveTutorWorkflow();
      await expect(workflow.run({ question: 'Will this fail?' })).rejects.toThrow('LLM unavailable');
    });
  });
