...
**תאריך עדכון:** 12 מרץ 2026 (Phase 59–64 ✅ — Marketplace + Badges + SCORM + Portal Builder + Compliance Library)

---

## FEAT-PHASE51-53-B2B-GTM | ✅ Complete | HIGH

**Status:** ✅ Complete — committed ff12192 + 19e2e75 + 171ebfe + E2E commit (2026-03-12)
...
| All 8 specs pass on chromium + mobile-chrome — commit `68a2f26` (2026-03-12) | ✅ |
...
- **GAP-8** `GraphGroundedCredentialService` (subgraph-content): `verifyKnowledgePathCoverage()` + `recordGraphCredential()`; `OpenBadgeService.issueGraphGroundedBadge()` with ≥70% coverage gate; `knowledgePathCoverage` query + `issueGraphGroundedBadge` mutation; 5 tests
- **DB**: migration 0029 — `chavruta_partner_sessions` (tenant+participant RLS), `knowledge_path_credentials` (tenant+owner RLS), `cohort_id` on `social_feed_items`
- **Frontend**: `ChavrutaPartnerPage` (/chavruta/partner, 7 tests), `MentorDiscoveryPage` (/mentor/discover/:courseId?, 7 tests), `CohortInsightsWidget` (5 tests), `GraphCredentialBadge` (5 tests), 3 GraphQL query files
